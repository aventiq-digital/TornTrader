...
assert.match(source, /items-cont\.all-items/); assert.match(source, /:scope > li\[data-group\]/);
...
assert.match(source, /if \(isContextCurrent\(\)\)/);
assert.match(source, /unchangedAtEnd >= 3/);
...
assert.equal(bazaarAddContext.getBazaarRowDisplayState({ ...displayBase, sellRule: 'keep-one', sellableQuantity: 3, sellableMarketValue: '986418' }).metadata, '3 sellable · $986,418');
assert.deepEqual({ ...bazaarAddContext.getBazaarRowDisplayState({ ...displayBase, sellRule: 'dont-sell', sellableQuantity: 0 }) }, { mode: 'protected', metadata: 'Protected', fillDisabled: true });
assert.equal(bazaarAddContext.getBazaarFillMessage({ state: 'loading' }), 'Loading Bazaar prices…');
assert.equal(bazaarAddContext.getBazaarFillMessage({ ok: false, reason: 'request-failed' }), 'Could not load Bazaar prices');
assert.equal(bazaarAddContext.getBazaarFillMessage({ ok: false, reason: 'no-listings' }), 'No Bazaar listings found');
assert.equal(bazaarAddContext.getBazaarFillMessage({ ok: false, reason: 'partial-aggregate' }), 'Partial quantity cannot be selected safely here');
...
assert.equal(selectionReadContext.getBazaarBulkSelectionMessage('rule-exceeded'), 'Selection exceeds sell rule');
...
assert.match(source, /Bazaar Sell Manager/);
assert.match(source, /Select sellable/);
assert.match(source, /Clear selection/);
assert.match(source, /Hide protected/);
assert.match(source, /Price adjustment:/);
assert.match(source, /Sort direction: \${descending \? 'descending' : 'ascending'}/);
assert.match(source, /Sellable: \${summary\.positions} positions/);
assert.match(source, /Market value partially unknown/);
...
assert.match(source, />Sell<\/span><select aria-label="Sell rule for/);
assert.match(source, /loading \? 'Loading…' : 'Fill'/);
assert.match(source, /Fill quantity and price for \${escapeAttribute\(model\.itemName\)}/);
...
assert.doesNotMatch(extractFunction('renderBazaarAddRowTools'), /No Bazaar price available/);
assert.match(source, /\$\{loading \? 'Loading…' : 'Fill'\}/);
assert.match(source, /void handleBazaarAddFill\(target\.dataset\.weav3rBazaarRowKey\)/);
assert.doesNotMatch(source, /No Bazaar price available/);
...
assert.match(source, /Fill prices for selected sellable positions/);
...
(async () => {
  ...
  assert.equal(bazaarAddContext.getBazaarFillMessage({ ok: true, proposal: xanaxProposal }), 'Price $829,968 prepared');
  ...
  assert.equal(bazaarAddContext.getBazaarFillMessage({ ok: false, proposal: xanaxProposal }), 'Could not set price');
  ...
  const lifecycleContext = {
    ...
    tornState: { bazaarManage: { root: null }, bazaarAdd: { lifecycleListenersAttached: false, lifecycleObserver: null, root: null, bulkPriceOperation: { cancelled: false } } },
    isTornBazaarAddRoute() { return addRoute; },
    isTornBazaarManageRoute() { return false; },
    initBazaarManage() {}, teardownBazaarManage() {}, findBazaarManagePanel() { return null; }, logDebug() {},
  ...
  const makeInventoryRow = ({ itemId, itemName, rowKey, quantity, armoryId = '', group = '', category = '', imageId = itemId, visibleName = itemName, imageAlt = itemName, withTornTools = false, factionLoan = false, equipped = false }) => {
    ...
    return { dataset: { item: String(itemId), sort: itemName, rowkey: rowKey, qty: String(quantity), armoryid: armoryId, group, category, equipped: String(equipped) }, matches(selector) { return selector === 'li[data-item][data-rowkey]'; }, querySelector(selector) { if (selector === '.title-wrap .name') return title; if (selector === '.tt-item-price') return tt; if (selector === '[data-action="return"][data-type="armoury"]') return factionLoan ? { dataset: { action: 'return', type: 'armoury', id: armoryId } } : null; return null; }, querySelectorAll(selector) { if (selector.startsWith('img[src')) return [image]; if (selector === 'img[alt]') return [image]; return []; } };
  };
  ...
  assert.deepEqual({ ...inventoryContext.parseTornInventoryRow(angleGrinder), sourceRow: undefined }, { itemId: 1509, itemName: 'Angle Grinder', category: 'Tool', quantity: 3, ownedQuantity: 3, ownership: 'owned', isFactionLoaned: false, rowKey: 'g1509', instanceKey: null, armoryId: null, groupKey: null, groupState: 'stack', isGroupParent: false, isGroupChild: false, sourceRow: undefined });
  ...
  const authenticFactionLoan = makeInventoryRow({ itemId: 233, itemName: 'BT MP9', rowKey: 'u11934147572', quantity: 1, armoryId: '11934147572', category: 'Secondary', factionLoan: true, equipped: true }); const parsedFactionLoan = inventoryContext.parseTornInventoryRow(authenticFactionLoan); assert.deepEqual({ itemId: parsedFactionLoan.itemId, instanceKey: parsedFactionLoan.instanceKey, armoryId: parsedFactionLoan.armoryId, rowKey: parsedFactionLoan.rowKey, ownership: parsedFactionLoan.ownership, ownedQuantity: parsedFactionLoan.ownedQuantity }, { itemId: 233, instanceKey: '11934147572', armoryId: '11934147572', rowKey: 'u11934147572', ownership: 'faction-loan', ownedQuantity: 0 });
  const equippedOwnedMp9 = makeInventoryRow({ itemId: 233, itemName: 'BT MP9', rowKey: 'u11934147573', quantity: 1, armoryId: '11934147573', category: 'Secondary', equipped: true }); assert.equal(inventoryContext.parseTornInventoryRow(equippedOwnedMp9).ownership, 'owned');
  const unequippedFactionLoan = makeInventoryRow({ itemId: 233, itemName: 'BT MP9', rowKey: 'u11934147574', quantity: 1, armoryId: '11934147574', category: 'Secondary', factionLoan: true }); assert.equal(inventoryContext.parseTornInventoryRow(unequippedFactionLoan).ownership, 'faction-loan');
  const mixedOwnershipScan = inventoryContext.scanTornInventoryList({ querySelectorAll: () => [equippedOwnedMp9, authenticFactionLoan] }); assert.equal(mixedOwnershipScan.allRows.length, 2); assert.equal(mixedOwnershipScan.rows.length, 1); assert.equal(mixedOwnershipScan.factionLoans.length, 1); assert.equal(mixedOwnershipScan.totalQuantity, 1);
  ...
  const inventoryHistoryContext = { ensureTornInventoryWahRow() { return historyHost; }, getTornInventoryWahArea() { return historyHost; }, document: { createElement() { return { dataset: {}, setAttribute(name, value) { this[name] = value; } }; } } }; vm.createContext(inventoryHistoryContext); vm.runInContext(extractFunction('ensureTornInventoryHistoryButton'), inventoryHistoryContext);
  ...
  const mixedOwnershipBlocks = basisContext.buildTornInventoryBasisBlocks(mixedOwnershipScan); assert.equal(mixedOwnershipBlocks.length, 1); assert.equal(mixedOwnershipBlocks[0].currentQuantity, 1); const mixedOwnershipBasis = basisContext.buildItemBasisSummary(233, mixedOwnershipBlocks[0].currentQuantity, basisContext.aggregateItemBasisEvents([basisEvent('owned-mp9-buy', 'in', 233, 1, '1000')])); assert.equal(mixedOwnershipBasis.reconstructionStatus, 'full'); assert.equal(mixedOwnershipBasis.trackedNetQuantity, 1);
  ...
  let basisNode = null; const basisHost = { appendChild(node) { basisNode = node; } }; const basisRow = { querySelector(selector) { if (selector.includes('[data-wah-item-basis]')) return basisNode; if (selector === '.title-wrap') return basisHost; return null; } }; const basisRenderContext = { ensureTornInventoryWahRow() { return basisHost; }, getTornInventoryWahArea() { return basisHost; }, document: { createElement() { return { dataset: {}, hidden: false }; } }, formatLedgerMoney: ledgerContext.formatLedgerMoney, formatBigIntRatioMoney: basisContext.formatBigIntRatioMoney }; vm.createContext(basisRenderContext); vm.runInContext(extractFunction('renderTornInventoryBasis'), basisRenderContext); const renderedBasis = basisRenderContext.renderTornInventoryBasis({ model: { itemId: 1084, sourceRow: basisRow } }, creditPartial); assert.equal(basisRenderContext.renderTornInventoryBasis({ model: { itemId: 1084, sourceRow: basisRow } }, creditPartial), renderedBasis); assert.match(renderedBasis.textContent, /3\/4 tracked/); assert.match(renderedBasis.title, /Item-ID-based/); assert.equal(renderedBasis.dataset.wahState, 'available'); assert.equal(renderedBasis.hidden, false); basisRenderContext.renderTornInventoryBasis({ model: { itemId: 1084, sourceRow: basisRow } }, { ...creditPartial, reconstructionStatus: 'untracked' }); assert.equal(renderedBasis.textContent, 'Basis —'); assert.equal(renderedBasis.dataset.wahState, 'unavailable'); assert.equal(renderedBasis.hidden, true); assert.match(renderedBasis.title, /No confirmed purchase basis/); basisRenderContext.renderTornInventoryBasis({ model: { itemId: 1084, sourceRow: basisRow } }, { ...creditPartial, reconstructionStatus: 'divergent' }); assert.equal(renderedBasis.hidden, false); assert.equal(renderedBasis.textContent, 'Basis —');
  let bazaarNode = null; const valuationHost = { appendChild(node) { bazaarNode = node; } }; const valuationRow = { querySelector(selector) { if (selector.includes('[data-wah-item-bazaar]')) return bazaarNode; if (selector === '.title-wrap') return valuationHost; return null; } }; const valuationRenderContext = { ensureTornInventoryWahRow() { return valuationHost; }, getTornInventoryWahArea() { return valuationHost; }, document: { createElement() { return { dataset: {} }; } }, normalizePositiveInt: offerContext.normalizePositiveInt, normalizeMoneyString: ledgerContext.normalizeMoneyString, formatLedgerMoney: ledgerContext.formatLedgerMoney }; vm.createContext(valuationRenderContext); vm.runInContext(`${extractFunction('buildInventoryBazaarValuation')}\n${extractFunction('renderTornInventoryBazaar')}`, valuationRenderContext); const renderedValuation = valuationRenderContext.renderTornInventoryBazaar({ model: { itemId: 1084, sourceRow: valuationRow }, currentQuantity: 4 }, recommendedMinusOne); assert.equal(valuationRenderContext.renderTornInventoryBazaar({ model: { itemId: 1084, sourceRow: valuationRow }, currentQuantity: 4 }, recommendedMinusOne), renderedValuation); assert.match(renderedValuation.textContent, /\$300 · current/); assert.match(renderedValuation.title, /Not guaranteed proceeds/); assert.equal(renderedValuation.dataset.wahState, 'available'); assert.equal(renderedValuation.dataset.wahFreshness, 'current');
  ...
  const oldNow = quoteNow + 180 * 24 * 60 * 60 * 1000; const oldResult = quoteContext.resolveInventoryTraderQuote(367, quoteIndex, 4, oldNow); assert.equal(oldResult.status, 'unverified'); assert.ok(oldResult.quote); assert.match(quoteContext.formatStoredQuoteAge(oldResult.quote.lastSeenAt, oldNow), /5mo ago/);
  ...
  let traderNode = null; const traderHost = { appendChild(node) { traderNode = node; } }; const traderRow = { querySelector(selector) { if (selector.includes('[data-wah-item-trader]')) return traderNode; if (selector === '.title-wrap') return traderHost; return null; } }; const traderRenderContext = { ensureTornInventoryWahRow() { return traderHost; }, getTornInventoryWahArea() { return traderHost; }, document: { createElement() { return { dataset: {}, hidden: false }; } }, normalizePositiveInt: offerContext.normalizePositiveInt, normalizeMoneyString: ledgerContext.normalizeMoneyString, formatLedgerMoney: ledgerContext.formatLedgerMoney, formatStoredQuoteAge: quoteContext.formatStoredQuoteAge, buildInventoryTraderValuation: quoteContext.buildInventoryTraderValuation, Date }; vm.createContext(traderRenderContext); vm.runInContext(extractFunction('renderTornInventoryTrader'), traderRenderContext); const traderResult = { status: 'eligible', quote: { traderName: 'Trader A', price: '812000', lastSeenAt: quoteNow } }; const renderedTrader = traderRenderContext.renderTornInventoryTrader({ model: { itemId: 367, sourceRow: traderRow }, currentQuantity: 4 }, traderResult, quoteNow + 1000); assert.equal(traderRenderContext.renderTornInventoryTrader({ model: { itemId: 367, sourceRow: traderRow }, currentQuantity: 4 }, traderResult, quoteNow + 1000), renderedTrader); assert.match(renderedTrader.textContent, /\$812,000 · Trader A/); assert.match(renderedTrader.title, /Item-ID-bound/); assert.equal(renderedTrader.dataset.wahState, 'available'); assert.equal(renderedTrader.hidden, false); traderRenderContext.renderTornInventoryTrader({ model: { itemId: 367, sourceRow: traderRow }, currentQuantity: 4 }, { status: 'unverified', quote: traderResult.quote }, quoteNow + 1000); assert.equal(renderedTrader.dataset.wahState, 'stored'); assert.equal(renderedTrader.dataset.wahFreshness, 'stored'); assert.equal(renderedTrader.hidden, false); traderRenderContext.renderTornInventoryTrader({ model: { itemId: 367, sourceRow: traderRow }, currentQuantity: 4 }, { status: 'unavailable', quote: null }, quoteNow + 1000); assert.equal(renderedTrader.hidden, true);

  const deltaContext = { BigInt, makeBigIntRatio: basisContext.makeBigIntRatio, roundBigIntRatio: basisContext.roundBigIntRatio }; vm.createContext(deltaContext); vm.runInContext([extractFunction('resolveVerifiedMarketDelta'), extractFunction('formatInventoryAdvantagePercent')].join('\n'), deltaContext); const traderWins = { status: 'available', verification: 'verified', channel: 'trader', unitPrice: '830622' }; const traderDelta = deltaContext.resolveVerifiedMarketDelta(traderWins, { status: 'available', recommendedUnitPrice: '824998' }, { status: 'eligible', unitPrice: '830622' }); assert.equal(traderDelta.absoluteDelta, '5624'); assert.equal(traderDelta.alternative, 'bazaar'); assert.equal(deltaContext.formatInventoryAdvantagePercent(traderDelta.percentage), '+0.68%'); const bazaarDelta = deltaContext.resolveVerifiedMarketDelta({ status: 'available', verification: 'verified', channel: 'bazaar', unitPrice: '840000' }, { status: 'available', recommendedUnitPrice: '840000' }, { status: 'eligible', unitPrice: '830622' }); assert.equal(bazaarDelta.absoluteDelta, '9378'); assert.equal(deltaContext.formatInventoryAdvantagePercent(bazaarDelta.percentage), '+1.13%'); assert.equal(deltaContext.resolveVerifiedMarketDelta({ ...traderWins, unitPrice: '830622' }, { status: 'available', recommendedUnitPrice: '830622' }, { status: 'eligible', unitPrice: '830622' }), null); assert.equal(deltaContext.resolveVerifiedMarketDelta(traderWins, { status: 'sort-available', recommendedUnitPrice: '824998' }, { status: 'eligible', unitPrice: '830622' }), null); assert.equal(deltaContext.resolveVerifiedMarketDelta(traderWins, { status: 'available', recommendedUnitPrice: '824998' }, { status: 'unverified', unitPrice: '830622' }), null); assert.equal(deltaContext.resolveVerifiedMarketDelta(traderWins, { status: 'unavailable' }, { status: 'eligible', unitPrice: '830622' }), null);
  let bestSaleNode = null; const bestSaleHost = { appendChild(node) { bestSaleNode = node; } }; const bestSaleRow = { querySelector(selector) { if (selector.includes('[data-wah-item-best-sale]')) return bestSaleNode; if (selector === '.title-wrap') return bestSaleHost; return null; } }; const bestSaleRenderContext = { ensureTornInventoryWahRow() { return bestSaleHost; }, getTornInventoryWahArea() { return bestSaleHost; }, document: { createElement() { return { dataset: {} }; } }, Date, formatLedgerMoney: ledgerContext.formatLedgerMoney, formatStoredQuoteAge: quoteContext.formatStoredQuoteAge, formatInventoryAdvantagePercent: deltaContext.formatInventoryAdvantagePercent }; vm.createContext(bestSaleRenderContext); vm.runInContext(`${extractFunction('formatBestSalePrimary')}\n${extractFunction('renderTornInventoryBestSale')}`, bestSaleRenderContext); const bestSaleResult = bestSaleContext.resolveInventoryBestSale(safeBazaar(515), eligibleTrader(500), 3); const renderedBestSale = bestSaleRenderContext.renderTornInventoryBestSale({ model: { itemId: 367, sourceRow: bestSaleRow } }, bestSaleResult, null, quoteNow); assert.equal(bestSaleRenderContext.renderTornInventoryBestSale({ model: { itemId: 367, sourceRow: bestSaleRow } }, bestSaleResult, null, quoteNow), renderedBestSale); assert.equal(renderedBestSale.textContent, 'Best Sale: Bazaar · $515 each · $1,545 total'); assert.match(renderedBestSale.title, /gross value, not guaranteed proceeds/); assert.equal(renderedBestSale.dataset.wahState, 'available'); const singleBestSale = { ...bestSaleResult, unitPrice: '34849', totalValue: '34849', quantity: 1 }; bestSaleRenderContext.renderTornInventoryBestSale({ model: { itemId: 367, sourceRow: bestSaleRow } }, singleBestSale, null, quoteNow); assert.equal(renderedBestSale.textContent, 'Best Sale: Bazaar · $34,849'); assert.doesNotMatch(renderedBestSale.textContent, /each|total/); const deltaBestSale = { status: 'available', verification: 'verified', channel: 'trader', unitPrice: '830622', totalValue: '15781818', quantity: 19, trader: { traderName: 'MenacingG3ko' } }; bestSaleRenderContext.renderTornInventoryBestSale({ model: { itemId: 367, sourceRow: bestSaleRow } }, deltaBestSale, null, quoteNow, traderDelta); assert.match(renderedBestSale.textContent, /\+\$5,624 \(\+0\.68%\) vs Bazaar/); assert.equal(renderedBestSale.dataset.wahMarketDelta, 'verified'); bestSaleRenderContext.renderTornInventoryBestSale({ model: { itemId: 367, sourceRow: bestSaleRow } }, { status: 'unavailable', channel: 'unavailable' }, null, quoteNow); assert.equal(renderedBestSale.textContent, ''); assert.equal(renderedBestSale.hidden, true); assert.equal(renderedBestSale.dataset.wahState, 'unavailable');

  let profitNode = null; const profitHost = { appendChild(node) { profitNode = node; } }; const profitRow = { querySelector(selector) { if (selector.includes('[data-wah-item-profit]')) return profitNode; if (selector === '.title-wrap') return profitHost; return null; } }; const profitRenderContext = { ensureTornInventoryWahRow() { return profitHost; }, getTornInventoryWahArea() { return profitHost; }, document: { createElement() { return { dataset: {}, hidden: false }; } }, formatBigIntRatioMoney: basisContext.formatBigIntRatioMoney, formatInventoryRoi: basisContext.formatInventoryRoi, inventoryProfitUnavailableReason(reason) { return reason; } }; vm.createContext(profitRenderContext); vm.runInContext(extractFunction('renderTornInventoryProfit'), profitRenderContext); const renderedProfit = profitRenderContext.renderTornInventoryProfit({ model: { itemId: 1084, sourceRow: profitRow } }, partialProfit); assert.equal(profitRenderContext.renderTornInventoryProfit({ model: { itemId: 1084, sourceRow: profitRow } }, partialProfit), renderedProfit); assert.match(renderedProfit.textContent, /Profit \+\$150 · \+16\.67 % · 3\/4 covered/); assert.match(renderedProfit.title, /Potential gross profit/); assert.equal(renderedProfit.dataset.wahState, 'positive'); assert.equal(renderedProfit.hidden, false); profitRenderContext.renderTornInventoryProfit({ model: { itemId: 1084, sourceRow: profitRow } }, { ...partialProfit, potentialProfit: { numerator: '-1', denominator: '1' } }); assert.equal(renderedProfit.dataset.wahState, 'negative'); profitRenderContext.renderTornInventoryProfit({ model: { itemId: 1084, sourceRow: profitRow } }, { status: 'unavailable', reason: 'no-confirmed-basis' }); assert.equal(renderedProfit.textContent, 'Profit —'); assert.equal(renderedProfit.dataset.wahState, 'unavailable'); assert.equal(renderedProfit.hidden, true); profitRenderContext.renderTornInventoryProfit({ model: { itemId: 1084, sourceRow: profitRow } }, { status: 'unavailable', reason: 'no-verified-best-sale' }); assert.equal(renderedProfit.hidden, false);

  const sortContext = { BigInt, Math, INVENTORY_SORT_CRITERIA: ['original', 'name', 'quantity', 'average-acquisition', 'current-basis', 'bazaar-unit', 'bazaar-total', 'trader-unit', 'trader-total', 'best-sale-unit', 'best-sale-total', 'profit', 'roi'], normalizeInventoryIdentityText(value) { return String(value || '').toLowerCase(); }, normalizePositiveInt: offerContext.normalizePositiveInt, normalizeMoneyString: ledgerContext.normalizeMoneyString, makeBigIntRatio: basisContext.makeBigIntRatio, state: { inventorySortOriginals: new WeakMap(), inventorySortSettings: { criterion: 'original', direction: 'asc' }, inventorySortInternalRows: new WeakSet() }, setTimeout };
  vm.createContext(sortContext); vm.runInContext(['normalizeInventorySortSettings', 'inventorySortRatio', 'inventorySortMoney', 'resolveInventorySortBestSale', 'buildInventorySortKey', 'compareInventorySortRatios', 'compareInventorySortTieBreakers', 'compareInventorySortRecords', 'sortInventoryBlockRecords', 'inventoryBlockStableIdentity', 'getInventoryOriginalIndex', 'applyInventoryBlockOrder'].map(extractFunction).join('\n'), sortContext);
  const sortRecord = ({ name, itemId, quantity, originalIndex, average = null, basis = null, bazaarUnit = null, traderUnit = null, traderStatus = 'eligible', bestUnit = null, bestVerified = true, profit = null, roi = null, rowKey = `g${itemId}`, groupKey = null, rowKeys = [rowKey] }) => ({ block: { model: { itemName: name, itemId, rowKey, groupKey }, currentQuantity: quantity, rowKeys }, originalIndex, summary: { knownPurchaseQuantity: average ? average.denominator : 0, knownPurchaseCost: average ? average.numerator : '0', currentCostBasis: basis }, bazaarValuation: bazaarUnit == null ? { status: 'unavailable' } : { status: 'available', recommendedUnitPrice: String(bazaarUnit), recommendedTotal: (BigInt(bazaarUnit) * BigInt(quantity)).toString() }, bazaarSortValuation: bazaarUnit == null ? { status: 'unavailable' } : { status: 'available', recommendedUnitPrice: String(bazaarUnit), recommendedTotal: (BigInt(bazaarUnit) * BigInt(quantity)).toString() }, traderValuation: traderUnit == null ? { status: 'unavailable' } : { status: traderStatus, unitPrice: String(traderUnit), totalPrice: (BigInt(traderUnit) * BigInt(quantity)).toString() }, bestSaleResult: bestUnit == null ? { status: 'unavailable' } : bestVerified ? { status: 'available', verification: 'verified', unitPrice: String(bestUnit), totalValue: (BigInt(bestUnit) * BigInt(quantity)).toString() } : { status: 'fallback', channel: 'stored-trader', verification: 'unverified', unitPrice: String(bestUnit), totalValue: (BigInt(bestUnit) * BigInt(quantity)).toString() }, profitResult: profit == null ? { status: 'unavailable' } : { status: 'full', potentialProfit: profit, roi } });
  ...
  const unverifiedTraderSort = sortRecord({ name: 'Stored', itemId: 7, quantity: 1, originalIndex: 6, traderUnit: 999, traderStatus: 'unverified', bestUnit: 999, bestVerified: false }); assert.equal(sortContext.buildInventorySortKey(unverifiedTraderSort, 'trader-unit'), null); assert.equal(sortContext.buildInventorySortKey(unverifiedTraderSort, 'best-sale-unit').numerator, '999'); assert.deepEqual({ ...sortContext.normalizeInventorySortSettings({ criterion: 'invalid', direction: 'desc' }) }, { criterion: 'original', direction: 'asc' });
  const liveBestSaleRecords = [sortRecord({ name: 'Xanax', itemId: 206, quantity: 19, originalIndex: 0, bazaarUnit: 823984, bestUnit: 828000 }), sortRecord({ name: 'Angle Grinder', itemId: 615, quantity: 3, originalIndex: 1, bazaarUnit: 96998 }), sortRecord({ name: 'Bag of Bloody Eyeballs', itemId: 618, quantity: 1, originalIndex: 2, bazaarUnit: 84998, bestUnit: 85128, bestVerified: false }), sortRecord({ name: 'Bag of Candy Kisses', itemId: 617, quantity: 4, originalIndex: 3, bazaarUnit: 55999 }), sortRecord({ name: 'Bear Gall', itemId: 28, quantity: 1, originalIndex: 4, bazaarUnit: 34387 })]; assert.deepEqual(sortedNames('best-sale-total', 'desc', liveBestSaleRecords), ['Xanax', 'Angle Grinder', 'Bag of Candy Kisses', 'Bag of Bloody Eyeballs', 'Bear Gall']); assert.deepEqual(sortedNames('best-sale-unit', 'desc', liveBestSaleRecords), ['Xanax', 'Angle Grinder', 'Bag of Bloody Eyeballs', 'Bag of Candy Kisses', 'Bear Gall']); const verifiedPrecedence = sortContext.resolveInventorySortBestSale({ status: 'available', verification: 'verified', unitPrice: '100' }, { status: 'sort-available', recommendedUnitPrice: '999' }, 2); assert.equal(verifiedPrecedence.unitPrice, '100'); assert.equal(verifiedPrecedence.verification, 'verified'); assert.equal(sortContext.resolveInventorySortBestSale({ status: 'unavailable' }, { status: 'unavailable' }, 1).status, 'unavailable');
  ...
  assert.match(source, /@version\s+0\.5\.26/); const englishUi = ['Sort by', 'Direction', 'Bazaar sell only', 'Prices updated', 'Re-sort', 'No confirmed purchase basis', 'Stored', 'current', 'total', 'Sell all', 'Keep one', "Don't sell"]; englishUi.forEach((label) => assert.ok(source.includes(label), `missing English UI label: ${label}`)); assert.doesNotMatch(source, /Regel|Alles verkaufen|1 behalten|Nicht verkaufen|Sortieren|Richtung|Preise aktualisiert|Neu sortieren|Keine bestätigte Kaufbasis|Gespeichert|aktuell|gesamt|Stk\.|n\. v\./);
  ...
  assert.equal(sellDecisionContext.resolveBazaarInventorySellDecision(206, 19, { rules: { 206: 'sell-all' } }).sellableQuantity, 19); assert.equal(sellDecisionContext.resolveBazaarInventorySellDecision(206, 19, { rules: { 206: 'keep-one' } }).sellableQuantity, 18); assert.equal(sellDecisionContext.resolveBazaarInventorySellDecision(206, 19, { rules: { 206: 'dont-sell' } }).sellableQuantity, 0);
  assert.equal(sellDecisionContext.resolveBazaarInventorySellDecision(233, mixedOwnershipBlocks[0].currentQuantity, { rules: { 233: 'sell-all' } }).sellableQuantity, 1);
  let sellQuantityNode = null; const sellQuantityHost = { appendChild(node) { sellQuantityNode = node; } }; const sellQuantityRow = { querySelector(selector) { return selector.includes('[data-wah-bazaar-sell-quantity]') ? sellQuantityNode : null; } }; const sellQuantityContext = { getTornInventoryWahArea() { return sellQuantityHost; }, document: { createElement() { return { dataset: {} }; } } }; vm.createContext(sellQuantityContext); vm.runInContext(extractFunction('renderInventoryBazaarSellQuantity'), sellQuantityContext); const sellQuantityRecord = (quantity, rule) => ({ block: { model: { sourceRow: sellQuantityRow } }, sellDecision: sellDecisionContext.resolveBazaarInventorySellDecision(206, quantity, { rules: { 206: rule } }) }); sellQuantityContext.renderInventoryBazaarSellQuantity(sellQuantityRecord(3, 'sell-all')); assert.equal(sellQuantityNode.textContent, '· 3/3'); assert.equal(sellQuantityNode.dataset.wahState, 'available'); sellQuantityContext.renderInventoryBazaarSellQuantity(sellQuantityRecord(1, 'keep-one')); assert.equal(sellQuantityNode.textContent, '· 0/1'); assert.equal(sellQuantityNode.dataset.wahState, 'zero'); sellQuantityContext.renderInventoryBazaarSellQuantity(sellQuantityRecord(19, 'dont-sell')); assert.equal(sellQuantityNode.textContent, '· 0/19'); assert.equal(sellQuantityNode.dataset.wahState, 'zero'); assert.doesNotMatch(sellQuantityNode.textContent, /To Bazaar/); assert.match(extractFunction('renderInventoryBazaarSellRule'), /`Sell: <select/); assert.doesNotMatch(extractFunction('renderInventoryBazaarSellRule'), /Rule:/);
  let sharedRulesStore = { version: 1, rules: {} }; const sharedRuleContext = { BAZAAR_ADD_RULES: ['sell-all', 'keep-one', 'dont-sell'], BAZAAR_SELL_RULE_LABELS: { 'sell-all': 'Sell all', 'keep-one': 'Keep one', 'dont-sell': "Don't sell" }, BAZAAR_SELL_RULES_KEY: 'WEAV3R_ARBITRAGE_BAZAAR_SELL_RULES', tornState: { bazaarAdd: { rules: sharedRulesStore } }, normalizePositiveInt: offerContext.normalizePositiveInt, gmGet() { return structuredClone(sharedRulesStore); }, gmSet(key, value) { assert.equal(key, 'WEAV3R_ARBITRAGE_BAZAAR_SELL_RULES'); sharedRulesStore = structuredClone(value); } }; vm.createContext(sharedRuleContext); vm.runInContext([extractFunction('normalizeBazaarSellRules'), extractFunction('getBazaarSellRule'), extractFunction('setBazaarSellRule'), extractFunction('bazaarSellRuleOptionsHtml')].join('\n'), sharedRuleContext); assert.match(sharedRuleContext.bazaarSellRuleOptionsHtml(), />Sell all<.*>Keep one<.*>Don\'t sell</); assert.equal(sharedRuleContext.setBazaarSellRule(206, 'keep-one'), true); assert.equal(sharedRuleContext.getBazaarSellRule(206, sharedRulesStore), 'keep-one'); sharedRulesStore.rules['206'] = 'dont-sell'; assert.equal(sharedRuleContext.getBazaarSellRule(206, sharedRulesStore), 'dont-sell'); assert.equal((source.match(/WEAV3R_ARBITRAGE_BAZAAR_SELL_RULES/g) || []).length >= 1, true); assert.match(extractFunction('renderInventoryBazaarSellRule'), /data-wah-inventory-sell-rule/); assert.match(extractFunction('handleInventoryBazaarSellRuleChange'), /setBazaarSellRule/);
...
${extractFunction('snapshotWeav3rItemSourceTables')}`, sourceTableContext); const headerTable = (headers) => ({ querySelectorAll(selector) { return selector === 'thead th' ? headers.map((textContent) => ({ textContent })) : []; } }); assert.equal(sourceTableContext.normalizeHeaderText('  Trader \n '), 'trader'); assert.equal(sourceTableContext.normalizeHeaderText(null), ''); assert.equal(sourceTableContext.normalizeHeaderText(undefined), ''); assert.equal(sourceTableContext.isWeav3rItemSourceTable(headerTable(['Trader', 'Buy Price', 'Rating', 'Actions'])), true); assert.equal(sourceTableContext.isWeav3rItemSourceTable(headerTable(['Seller', 'Quantity', 'Price / Total Value', 'Last Checked'])), true); assert.equal(sourceTableContext.isWeav3rItemSourceTable(headerTable([])), false); assert.equal(sourceTableContext.isWeav3rItemSourceTable({}), false); assert.equal(sourceTableContext.isWeav3rItemSourceTable(headerTable(['Trader', 'Buy Price'])), false);
  ...
  function makeSmokeRow(rowKey, itemId) { const nodes = new Map(); const host = { appendChild(node) { node.parentElement = host; nodes.set(node.dataset?.wahWeav3rLinks !== undefined ? 'links' : node.dataset?.wahBazaarSellQuantity !== undefined ? 'sell' : `node-${nodes.size}`, node); } }; const existingWah = { removed: false, remove() { this.removed = true; } }; return { dataset: { rowkey: rowKey }, parentElement: null, classList: { values: new Set(), toggle(name, on) { if (on) this.values.add(name); else this.values.delete(name); } }, querySelector(selector) { if (selector === ':scope > [data-wah-item-row]') return existingWah.removed ? null : existingWah; if (selector === '.title-wrap') return host; if (selector.includes('data-wah-weav3r-links')) return nodes.get('links') || null; if (selector.includes('data-wah-bazaar-sell-quantity')) return nodes.get('sell') || null; return null; }, nodes, wahHost: host, existingWah, itemId }; }
  const integrationRows = [makeSmokeRow('g28', 28), makeSmokeRow('u19452406897:special', 233), makeSmokeRow('u11934147572', 233)]; const integrationContainer = { children: integrationRows, parentElement: {}, isConnected: true }; integrationRows.forEach((row) => { row.parentElement = integrationContainer; }); const integrationModels = integrationRows.slice(0, 2).map((row) => ({ rowKey: row.dataset.rowkey, itemId: row.itemId, itemName: `Item ${row.itemId}`, ownership: 'owned', ownedQuantity: 1, sourceRow: row })); const factionLoanIntegrationModel = { rowKey: 'u11934147572', itemId: 233, itemName: 'BT MP9', ownership: 'faction-loan', ownedQuantity: 0, sourceRow: integrationRows[2] }; const integrationBlocks = integrationModels.map((model) => ({ model, rowKeys: [model.rowKey], currentQuantity: model.itemId === 28 ? 5 : 1 })); const integrationFlags = { basis: 0, bazaar: 0, trader: 0, best: 0, profit: 0, control: 0, snapshot: 0 };
  ...
  const refreshContext = { ensureTornInventoryWahRow(blockOrModel) { return (blockOrModel.model || blockOrModel).sourceRow.wahHost; }, getTornInventoryWahArea(blockOrModel) { return (blockOrModel.model || blockOrModel).sourceRow.wahHost; }, ensureTornInventoryHistoryButton() { return {}; }, renderInventoryBazaarSellRule() { return {}; }, location: { pathname: '/item.php' }, document: integrationDocument, state: integrationState, Date, Set, Map, Array, normalizePositiveInt: offerContext.normalizePositiveInt, normalizeBazaarSellRules: bazaarAddContext.normalizeBazaarSellRules, normalizeBazaarAddSettings() { return { priceAdjustment: '-1' }; }, gmGet(key) { return key === 'rules' ? { version: 1, rules: { 28: 'keep-one', 233: 'dont-sell' } } : {}; }, BAZAAR_SELL_RULES_KEY: 'rules', BAZAAR_ADD_UI_SETTINGS_KEY: 'settings', getActiveTornInventoryContainer() { return integrationContainer; }, ensureFullTornInventoryLoaded() {}, isTornInventoryFullLoadPending() { return false; }, isInventorySortContainerActive() { return true; }, listTransactionEvents() { return []; }, aggregateItemBasisEvents() { return new Map(); }, indexLatestTraderQuotes() { return new Map(); }, readTraderQuotes() { return {}; }, indexBazaarQuotes() { return new Map(); }, readBazaarQuotes() { return {}; }, readTraderPersonalRatings() { return { records: {} }; }, scanTornInventoryList() { return { rows: integrationModels, allRows: [...integrationModels, factionLoanIntegrationModel], factionLoans: [factionLoanIntegrationModel] }; }, buildTornInventoryBasisBlocks() { return integrationBlocks; }, buildItemBasisSummary() { return {}; }, renderTornInventoryBasis() { integrationFlags.basis += 1; return {}; }, getBazaarAddCachedOffers() { return { fresh: true, offers: [{ price: 10 }] }; }, loadCachedItemData() { return { bazaar: { apiSource: 'test', capturedAt: 1 } }; }, resolveBazaarRecommendedPrice(itemId) { return { status: 'available', itemId, recommendedUnitPrice: '9', marketUnitPrice: '10', adjustment: '-1', offers: [{ price: 10 }] }; }, resolveInventoryTraderQuote() { return { status: 'eligible', quote: { price: '8' } }; }, buildInventoryBazaarValuation() { return { status: 'available', recommendedUnitPrice: '9', recommendedTotal: '45' }; }, resolveStoredBazaarSortValuation() { return { status: 'unavailable' }; }, resolveStoredBestSaleCandidate() { return null; }, buildInventoryTraderValuation() { return { status: 'eligible', unitPrice: '8', totalPrice: '40' }; }, resolveInventoryBestSale() { return { status: 'available', verification: 'verified', unitPrice: '9', totalValue: '45' }; }, resolveVerifiedMarketDelta() { return null; }, resolveInventoryPotentialProfit() { return { status: 'unavailable' }; }, renderTornInventoryBazaar() { integrationFlags.bazaar += 1; }, renderTornInventoryTrader() { integrationFlags.trader += 1; }, renderTornInventoryBestSale() { integrationFlags.best += 1; }, renderTornInventoryProfit() { integrationFlags.profit += 1; }, getInventoryOriginalIndex(_container, block) { return integrationBlocks.indexOf(block); }, ensureInventorySortControl() { integrationFlags.control += 1; return {}; }, registerInventoryBazaarRequests() { return []; }, prioritizeInventoryBazaarRequests() { return []; }, canApplyInventorySortSnapshot() { return { allowed: true }; }, updateInventorySortSnapshotState() { integrationFlags.snapshot += 1; }, logDebug() {}, loadBazaarRecommendedPrice() { throw new Error('unexpected request'); }, scheduleInventoryBazaarResultRefresh() {}, escapeAttribute(value) { return String(value); }, buildWeav3rItemViewUrl: directRouteContext.buildWeav3rItemViewUrl, getBazaarSellRule: sellDecisionContext.getBazaarSellRule, calculateBazaarSellable: bazaarAddContext.calculateBazaarSellable };
  vm.createContext(refreshContext); vm.runInContext([extractFunction('resolveBazaarInventorySellDecision'), extractFunction('renderTornInventoryWeav3rLinks'), extractFunction('renderInventoryBazaarSellQuantity'), extractFunction('applyInventoryBazaarSellFilter'), extractFunction('refreshTornInventoryBasis')].join('\n'), refreshContext); assert.doesNotThrow(() => refreshContext.refreshTornInventoryBasis({ querySelectorAll() { return integrationRows; } })); assert.deepEqual(integrationFlags, { basis: 2, bazaar: 2, trader: 2, best: 2, profit: 2, control: 1, snapshot: 1 }); assert.ok(integrationRows[0].nodes.get('links').innerHTML.includes('/item/28?mode=buy&amp;tab=all&amp;timeframe=7d') || integrationRows[0].nodes.get('links').innerHTML.includes('/item/28?mode=buy&tab=all&timeframe=7d')); assert.ok(integrationRows[0].nodes.get('sell')); assert.equal(integrationRows[1].classList.values.has('wah-inventory-sell-filter-hidden'), true); assert.equal(integrationRows[0].classList.values.has('wah-inventory-sell-filter-hidden'), false); assert.equal(integrationRows[2].classList.values.has('wah-inventory-sell-filter-hidden'), true); assert.equal(integrationRows[2].existingWah.removed, true); assert.equal(integrationRows[2].nodes.size, 0); integrationState.inventoryBazaarSellFilter = false; refreshContext.refreshTornInventoryBasis({ querySelectorAll() { return integrationRows; } }); assert.equal(integrationRows[2].classList.values.has('wah-inventory-sell-filter-hidden'), false); assert.equal(integrationRows[2].nodes.size, 0);
  ...
  const makeUiElement = (className = '') => ({ className, dataset: {}, children: [], parentElement: null, textContent: '', querySelector(selector) { const area = selector.match?.(/data-wah-area=\"([^\"]+)/)?.[1]; return area ? this.children.find((child) => child.dataset?.wahArea === area) || null : null; }, setAttribute(name, value) { this[name] = value; }, appendChild(child) { child.parentElement?.children?.splice(child.parentElement.children.indexOf(child), 1); child.parentElement = this; this.children.push(child); return child; }, remove() { if (this.parentElement) this.parentElement.children.splice(this.parentElement.children.indexOf(this), 1); this.parentElement = null; }, querySelectorAll(selector) { const descendants = (node) => (node.children || []).flatMap((child) => [child, ...descendants(child)]); const attrs = Array.from(String(selector).matchAll(/data-([a-z0-9-]+)/g), (match) => match[1].replace(/-([a-z])/g, (_all, letter) => letter.toUpperCase())); return attrs.length ? descendants(this).filter((child) => attrs.some((attr) => child.dataset?.[attr] !== undefined)) : []; } }); const TORN_INVENTORY_WAH_SELECTOR_TEST = '[data-wah-item-history],[data-wah-item-basis],[data-wah-weav3r-links],[data-wah-item-bazaar],[data-wah-item-trader],[data-wah-item-best-sale],[data-wah-item-profit],[data-wah-bazaar-sell-quantity]'; const titleFixture = makeUiElement('title-wrap'); const tornToolsPrice = makeUiElement('tt-item-price'); tornToolsPrice.textContent = '$123'; titleFixture.appendChild(tornToolsPrice); const legacyBasis = makeUiElement('wah-item-basis'); legacyBasis.dataset.wahItemBasis = '563'; titleFixture.appendChild(legacyBasis); const contFixture = makeUiElement('cont-wrap'); const rowFixture = makeUiElement(); rowFixture.dataset = { rowkey: 'g563', item: '563' }; rowFixture.children = [titleFixture, contFixture]; titleFixture.parentElement = rowFixture; contFixture.parentElement = rowFixture; rowFixture.insertBefore = function (child, anchor) { child.parentElement = this; const index = anchor ? this.children.indexOf(anchor) : -1; if (index < 0) this.children.push(child); else this.children.splice(index, 0, child); return child; }; rowFixture.querySelector = function (selector) { if (selector === ':scope > [data-wah-item-row]') return this.children.find((child) => child.dataset?.wahItemRow !== undefined) || null; if (selector === ':scope > .title-wrap' || selector === '.title-wrap') return titleFixture; if (selector === ':scope > .cont-wrap') return contFixture; if (selector.includes('data-wah-item-history')) return this.children.flatMap((child) => child.children || []).find((child) => child.dataset?.wahItemHistory !== undefined) || null; return null; }; rowFixture.querySelectorAll = function (selector) { return selector === ':scope > [data-wah-item-row]' ? this.children.filter((child) => child.dataset?.wahItemRow !== undefined) : []; }; const rowHostContext = { Array, TORN_INVENTORY_WAH_SELECTOR: TORN_INVENTORY_WAH_SELECTOR_TEST, document: { createElement() { return makeUiElement(); } } }; vm.createContext(rowHostContext); vm.runInContext(`${extractFunction('ensureTornInventoryWahAreas')}\n${extractFunction('ensureTornInventoryWahRow')}
${extractFunction('getTornInventoryWahArea')}
${extractFunction('ensureTornInventoryHistoryButton')}`, rowHostContext); const giftModel = { itemId: 563, itemName: 'Gift Card', sourceRow: rowFixture }; const giftHost = rowHostContext.ensureTornInventoryWahRow(giftModel); rowHostContext.ensureTornInventoryHistoryButton(giftModel); assert.equal(rowHostContext.ensureTornInventoryWahRow(giftModel), giftHost); assert.equal(rowFixture.children.filter((child) => child.dataset?.wahItemRow !== undefined).length, 1); assert.ok(rowFixture.children.indexOf(titleFixture) < rowFixture.children.indexOf(giftHost)); assert.ok(rowFixture.children.indexOf(giftHost) < rowFixture.children.indexOf(contFixture)); assert.equal(titleFixture.children.includes(legacyBasis), false); const giftDescendants = (node) => (node.children || []).flatMap((child) => [child, ...giftDescendants(child)]); assert.equal(giftDescendants(giftHost).includes(legacyBasis), true); assert.equal(titleFixture.children.includes(tornToolsPrice), true); assert.equal(tornToolsPrice.textContent, '$123'); assert.ok(giftDescendants(giftHost).some((child) => child.dataset?.wahItemHistory === '563')); assert.ok(giftDescendants(giftHost).some((child) => child.dataset?.wahArea === 'main')); assert.ok(giftDescendants(giftHost).some((child) => child.dataset?.wahArea === 'markets')); assert.ok(giftDescendants(giftHost).some((child) => child.dataset?.wahArea === 'meta')); assert.ok(giftDescendants(giftHost).some((child) => child.dataset?.wahArea === 'actions')); const semanticNodes = { best: makeUiElement('wah-item-best-sale'), bazaar: makeUiElement('wah-item-bazaar'), trader: makeUiElement('wah-item-trader'), profit: makeUiElement('wah-item-profit'), links: makeUiElement('wah-item-weav3r-links'), rule: makeUiElement('wah-item-sell-rule'), quantity: makeUiElement('wah-item-sell-quantity') }; semanticNodes.best.dataset.wahItemBestSale = '563'; semanticNodes.bazaar.dataset.wahItemBazaar = '563'; semanticNodes.trader.dataset.wahItemTrader = '563'; semanticNodes.profit.dataset.wahItemProfit = '563'; semanticNodes.links.dataset.wahWeav3rLinks = '563'; semanticNodes.rule.dataset.wahInventorySellRuleControl = '563'; semanticNodes.quantity.dataset.wahBazaarSellQuantity = ''; Object.values(semanticNodes).forEach((node) => giftHost.appendChild(node)); const giftAreas = rowHostContext.ensureTornInventoryWahAreas(giftHost); assert.equal(semanticNodes.best.parentElement, giftAreas.primary); assert.equal(semanticNodes.bazaar.parentElement, giftAreas.markets); assert.equal(semanticNodes.trader.parentElement, giftAreas.markets); assert.equal(legacyBasis.parentElement, giftAreas.meta); assert.equal(semanticNodes.profit.parentElement, giftAreas.meta); assert.equal(semanticNodes.rule.parentElement, giftAreas.sell); assert.equal(semanticNodes.quantity.parentElement, giftAreas.sell); assert.equal(semanticNodes.links.parentElement, giftAreas.actions); assert.equal(giftDescendants(giftHost).filter((node) => node.dataset?.wahArea === 'actions').length, 1); const inventoryRendererNames = ['renderTornInventoryTrader', 'renderTornInventoryBasis', 'renderTornInventoryBazaar', 'renderTornInventoryBestSale', 'renderTornInventoryProfit', 'renderTornInventoryWeav3rLinks', 'renderInventoryBazaarSellQuantity']; inventoryRendererNames.forEach((name) => { const source = extractFunction(name); assert.match(source, /getTornInventoryWahArea/); assert.doesNotMatch(source, /appendChild\([^)]*title|title\?\.appendChild/); }); assert.match(source, /\.wah-item-row\{flex:1 1 100%;width:100%;max-width:100%;box-sizing:border-box;display:grid/);
  // SPA visible-state regression: execute the real snapshot + route transition chain with Trader A in the DOM.
  const sourceTableA = { isConnected: true, rowText: 'Trader A $100', className: 'old', querySelectorAll(selector) { if (selector === 'thead th') return ['Trader', 'Buy Price', 'Rating', 'Actions'].map((textContent) => ({ textContent })); if (selector === 'tbody tr') return [{ textContent: this.rowText }]; return []; } }; const routeUiDocument = { querySelectorAll(selector) { return selector === 'table' ? [sourceTableA] : []; } }; const routeUiState = { lastUrl: 'https://weav3r.dev/item/1', itemId: '1', routeGeneration: 1, cachedData: { traders: { itemId: 1, traders: [{ traderName: 'Trader A' }] } }, lastResult: { status: 'BUY & SELL', bestTrader: { traderName: 'Trader A' } }, sourceProgress: {}, staleRouteTables: new Set(), routeDomReady: true, recheckTimer: 10, manualTraderCaptureTimer: 11, eligibilityTimer: 12 }; const renderedRouteResults = []; let routeCacheEnabled = false; const routeUiContext = { document: routeUiDocument, location: { href: 'https://weav3r.dev/item/2' }, state: routeUiState, Array, Set, normalizePositiveInt: offerContext.normalizePositiveInt, cancelCurrentCollectors() {}, clearTimeout() {}, emptySourceProgress() { return {}; }, emptyCachedData() { return { bazaar: null, itemMarket: null, traders: null }; }, highlightRows() {}, renderStatusCard(result) { renderedRouteResults.push(structuredClone(result)); }, loadCachedItemData(itemId) { return routeCacheEnabled ? { bazaar: null, itemMarket: null, traders: { itemId: Number(itemId), itemBindingVersion: 1, traders: [] } } : { bazaar: null, itemMarket: null, traders: null }; }, updateSourceProgressFromCache() {}, renderComputedResult(_cached, itemId, generation) { if (String(itemId) !== String(routeUiState.itemId) || generation !== routeUiState.routeGeneration) return null; routeUiState.lastResult = { status: 'DATA MISSING', itemId: Number(itemId), routeGeneration: generation }; renderedRouteResults.push(structuredClone(routeUiState.lastResult)); }, scheduleVisibleTraderQuoteCapture() {}, scheduleEvaluation() {}, logDebug() {} }; routeUiContext.getCurrentItemId = () => routeUiContext.location.href.match(/\/(\d+)(?:[?#]|$)/)?.[1] || null; vm.createContext(routeUiContext); vm.runInContext(['normalizeHeaderText', 'normalizeBoundedText', 'isWeav3rItemSourceTable', 'fingerprintWeav3rItemSourceTable', 'snapshotWeav3rItemSourceTables', 'beginWeav3rItemRouteTransition', 'updateWeav3rItemRouteDomReadiness', 'createRouteLoadingResult', 'invalidateCurrentItemUi', 'hasItemBoundCachedData', 'activateCurrentItemRoute', 'handleRouteChange'].map(extractFunction).join('\n'), routeUiContext); assert.doesNotThrow(() => routeUiContext.handleRouteChange()); assert.equal(routeUiState.itemId, '2'); assert.equal(routeUiState.lastResult.status, 'LOADING'); assert.equal(routeUiState.lastResult.bestTrader, undefined); assert.equal(renderedRouteResults.at(-1).itemId, 2); assert.equal(routeUiState.cachedData.traders, null); assert.equal(routeUiState.staleRouteTables.has(sourceTableA), true); const generationB = routeUiState.routeGeneration; routeUiState.cachedData.bazaar = { itemId: 2, offers: [] }; routeUiContext.renderComputedResult(true, '2', generationB); assert.equal(routeUiState.lastResult.status, 'DATA MISSING'); assert.equal(routeUiState.lastResult.itemId, 2); assert.equal(routeUiState.routeDomReady, false); routeUiContext.renderComputedResult(true, '1', generationB - 1); assert.equal(routeUiState.lastResult.itemId, 2); routeUiContext.location.href = 'https://weav3r.dev/item/3'; assert.doesNotThrow(() => routeUiContext.handleRouteChange()); assert.equal(routeUiState.itemId, '3'); assert.equal(routeUiState.lastResult.itemId, 3); assert.ok(routeUiState.routeGeneration > generationB); assert.equal(routeUiState.cachedData.traders, null); sourceTableA.className = 'only-style-changed'; assert.equal(routeUiContext.updateWeav3rItemRouteDomReadiness(), false); sourceTableA.rowText = 'Trader C $300'; assert.equal(routeUiContext.updateWeav3rItemRouteDomReadiness(), true); routeCacheEnabled = true; routeUiContext.location.href = 'https://weav3r.dev/item/4'; routeUiContext.handleRouteChange(); assert.equal(routeUiState.lastResult.status, 'DATA MISSING'); assert.equal(routeUiState.lastResult.itemId, 4);
  const brokenSnapshotContext = { ...routeUiContext, document: { querySelectorAll() { throw new Error('partial DOM'); } }, state: { ...routeUiState, staleRouteTables: new Set(), routeDomReady: true }, rendered: [] }; brokenSnapshotContext.renderStatusCard = (result) => brokenSnapshotContext.rendered.push(structuredClone(result)); vm.createContext(brokenSnapshotContext); vm.runInContext(['normalizeHeaderText', 'normalizeBoundedText', 'isWeav3rItemSourceTable', 'fingerprintWeav3rItemSourceTable', 'snapshotWeav3rItemSourceTables', 'beginWeav3rItemRouteTransition', 'createRouteLoadingResult', 'invalidateCurrentItemUi', 'hasItemBoundCachedData', 'activateCurrentItemRoute'].map(extractFunction).join('\n'), brokenSnapshotContext); assert.doesNotThrow(() => brokenSnapshotContext.activateCurrentItemRoute('9', 'Loading')); assert.equal(brokenSnapshotContext.state.lastResult.status, 'LOADING'); assert.equal(brokenSnapshotContext.state.routeDomReady, false); assert.equal(brokenSnapshotContext.state.routeSnapshotFailed, true);
...
${extractFunction('isCurrentRouteRender')}`, staleRenderContext); assert.equal(staleRenderContext.isCurrentRouteRender('1', 1), false); assert.equal(staleRenderContext.isCurrentRouteRender('2', 2), false); assert.equal(staleRenderContext.isCurrentRouteRender('3', 3), true);
  ...
  assert.equal((() => { const context = { location: {}, document: {}, normalizeHeaderText(value) { return String(value || '').trim().toLowerCase(); } }; vm.createContext(context); vm.runInContext(`${extractFunction('isTornBazaarManageRoute')}\n${extractFunction('findBazaarManagePanel')}`, context); assert.equal(context.isTornBazaarManageRoute('/bazaar.php', '#/manage'), true); assert.equal(context.isTornBazaarManageRoute('/bazaar.php', '#manage'), true); assert.equal(context.isTornBazaarManageRoute('/bazaar.php', '#/add'), false); const list = {}; const matching = { querySelector(selector) { if (selector.includes('panel-header')) return { textContent: 'Manage your Bazaar' }; if (selector.includes('virtualized-list')) return list; return null; } }; const unrelated = { querySelector(selector) { return selector.includes('panel-header') ? { textContent: 'Other panel' } : list; } }; assert.equal(context.findBazaarManagePanel({ querySelectorAll() { return [unrelated, matching]; } }), matching); return true; })(), true);
  assert.match(extractFunction('refreshOwnedInventoryBazaar'), /new Set/); assert.match(extractFunction('refreshOwnedInventoryBazaar'), /ownership !== 'faction-loan'/); assert.match(extractFunction('refreshOwnedInventoryBazaar'), /refreshBazaarValuesForItemIds/); assert.match(extractFunction('refreshBazaarValuesForItemIds'), /queueBazaarAddMarketplaceDetail/); assert.doesNotMatch(extractFunction('refreshOwnedInventoryBazaar'), /delete|submit|click/); assert.match(extractFunction('ensureInventorySortControl'), /Refresh Bazaar/);
  // Bazaar Manage semantic parsing and safe field-only preparation.
  const manageRemoveInput = { isConnected: true, disabled: false, readOnly: false, value: '', getAttribute(name) { return name === 'aria-label' ? 'Remove: Cluster Ring' : null; } }; const manageVisiblePrice = { value: '$3,896,227' }; const manageHiddenPrice = { value: '3896227' }; const manageImage = { getAttribute(name) { return name === 'src' ? '/images/items/1465/large.png' : name === 'alt' ? 'Cluster Ring' : null; } }; const manageHeading = { textContent: 'Cluster Ring x6', querySelector(selector) { return selector === 'b' ? { textContent: 'Cluster Ring' } : null; } }; const manageItem = { getAttribute(name) { return name === 'aria-label' ? 'Cluster Ring' : null; }, querySelector(selector) { if (selector === '[data-testid="img-container"] img') return manageImage; if (selector === '[role="heading"][aria-level="6"]') return manageHeading; if (selector.includes('data-input-money-visible')) return manageVisiblePrice; return null; }, querySelectorAll(selector) { if (selector.includes('aria-label^="Remove:"')) return [manageRemoveInput]; if (selector.includes('type="hidden"')) return [manageHiddenPrice]; return []; } }; const manageSortable = { querySelector(selector) { return selector === '[data-testid^="item-"]' ? manageItem : null; } }; const manageContext = { normalizePositiveInt: offerContext.normalizePositiveInt, normalizeMoneyString: ledgerContext.normalizeMoneyString, normalizeBoundedText(value, length) { return String(value || '').trim().slice(0, length); }, normalizeBazaarAddName(value) { return String(value || '').replace(/\s+/g, ' ').trim(); }, normalizedItemText(value) { return String(value || '').trim().toLowerCase(); }, extractTornItemIdFromImageSource(value) { return Number(String(value).match(/\/images\/items\/(\d+)\//)?.[1]) || null; }, normalizeBazaarAddPriceControlValue(value) { return String(value || '').replace(/[$,\s]/g, '') || null; }, writeBazaarManageField(input, value) { input.value = String(value); return true; } }; vm.createContext(manageContext); vm.runInContext([extractFunction('parseBazaarManageRow'), extractFunction('prepareBazaarManageRemoveAll')].join('\n'), manageContext); const parsedManage = manageContext.parseBazaarManageRow(manageSortable, 0); assert.deepEqual({ itemId: parsedManage.itemId, itemName: parsedManage.itemName, quantity: parsedManage.quantity, currentPrice: parsedManage.currentPrice }, { itemId: 1465, itemName: 'Cluster Ring', quantity: 6, currentPrice: '3896227' }); const removePrepared = manageContext.prepareBazaarManageRemoveAll([parsedManage, { quantity: null, removeInput: manageRemoveInput }, { quantity: 2, removeInput: { isConnected: false } }]); assert.deepEqual({ ...removePrepared }, { rows: 1, quantity: 6, skipped: 2 }); assert.equal(manageRemoveInput.value, '6'); assert.doesNotMatch(extractFunction('prepareBazaarManageRemoveAll'), /click|submit|requestSubmit/); assert.match(extractFunction('fillBazaarManagePrice'), /loadBazaarRecommendedPrice/); assert.doesNotMatch(extractFunction('fillBazaarManagePrice'), /SAVE|submit|click/); assert.match(extractFunction('renderBazaarManage'), /data-wah-bazaar-manage-row/); assert.match(extractFunction('initBazaarManage'), /MutationObserver/); assert.doesNotMatch(extractFunction('initBazaarManage'), /window\.confirm|confirm\(/); assert.match(extractFunction('initBazaarManage'), /prepared for removal/);
  const hiddenTradeRow = { className: 'weav3r-trade-filter-hidden', textContent: 'Unavailable', querySelectorAll() { return []; } }; const hiddenTradeContext = { normalizePositiveInt: offerContext.normalizePositiveInt, normalizedItemText(value) { return String(value || '').trim().toLowerCase(); }, getInventoryRowItemIds() { return [999]; }, getInventoryRowName() { return 'Zip Ties'; }, getInventoryRows() { return [hiddenTradeRow]; }, logDebug() {} }; vm.createContext(hiddenTradeContext); vm.runInContext(`${extractFunction('parseTradeRowIdentity')}\n${extractFunction('getVerifiedTargetRows')}`, hiddenTradeContext); assert.equal(hiddenTradeContext.getVerifiedTargetRows({ itemId: '999', itemName: 'Zip Ties' }).length, 1);
  assert.match(extractFunction('collectTradeInventoryRows'), /items-cont\.all-items/); assert.doesNotMatch(extractFunction('parseTradeRowIdentity'), /:visible|offsetParent|getClientRects|TRADE_FILTER_HIDDEN_CLASS/); assert.match(extractFunction('getVerifiedTargetRows'), /normalizePositiveInt\(handoff\.itemId\)/); assert.match(extractFunction('processAddStep'), /classList\.remove\(TRADE_FILTER_HIDDEN_CLASS, TRADE_FILTER_TARGET_CLASS\)/);
  assert.match(extractFunction('findTradeInventoryScrollRoot'), /const moved/); assert.match(extractFunction('getTradeDiscoverySnapshot'), /lastImage/); assert.match(source, /target-found-loading-rest/); assert.match(source, /targetFound: targetFound \|\| targets.length > 0/); assert.match(extractFunction('renderTradeFilterPanel'), /found — full inventory loading incomplete/);
  // Trade discovery is bounded, requires repeated stable-end evidence, and never treats an incomplete scan as not-found.
  const tradeDiscoverySource = source.slice(source.indexOf('async function findTradeTargetWithProgressiveScroll'), source.indexOf('function applyTradeItemFilter')); assert.match(tradeDiscoverySource, /unchangedAtEnd >= 3/); assert.match(tradeDiscoverySource, /status: 'complete'/); assert.match(tradeDiscoverySource, /status: signal\?\.aborted \? 'aborted' : 'incomplete'/); assert.doesNotMatch(tradeDiscoverySource, /if \(targets\.length\).*return targets/); assert.match(extractFunction('processAddStep'), /discovery\.status !== 'complete'/); assert.match(extractFunction('renderTradeFilterPanel'), /Loading full inventory/); assert.match(extractFunction('renderTradeFilterPanel'), /Inventory loading incomplete/);
  // The post-discovery resolver always queries the current attached All-items root and derives quantity from every fresh target row.
  const makeTradeRow = (itemId, name, quantity = null) => ({ isConnected: true, itemId, name, quantity });
  const rootA = { isConnected: false, rows: [makeTradeRow(1, 'Angle Grinder')], matches() { return true; }, querySelectorAll() { return this.rows; } };
  const rootB = { isConnected: true, rows: [makeTradeRow(1, 'Angle Grinder'), makeTradeRow(999, 'Zip Ties', 2), makeTradeRow(999, 'Zip Ties', 5)], matches() { return true; }, querySelectorAll() { return this.rows; } };
  let currentRoot = rootA;
  const finalTradeContext = { Array, setTimeout, Date, TRADE_FINAL_SETTLE_CHECK_MS: 5, TRADE_FINAL_SETTLE_TIMEOUT_MS: 100, normalizePositiveInt: offerContext.normalizePositiveInt, normalizedItemText(value) { return String(value || '').trim().toLowerCase(); }, parseTradeRowIdentity(row) { return { itemIds: [row.itemId], itemName: row.name }; }, resolveTradeRowActionability(row) { return row.quantity ? { status: 'available', control: { available: row.quantity } } : { status: 'unavailable', control: null }; }, getInventoryContainer() { return { querySelector() { return currentRoot; } }; } };
  vm.createContext(finalTradeContext); vm.runInContext([extractFunction('resolveCurrentTradeAllItemsRoot'), extractFunction('collectTradeInventoryRows'), extractFunction('resolveFinalTradeTargetState'), extractFunction('getTradeFinalSettleSignature'), `async ${extractFunction('waitForFinalTradeDomSettle')}`].join('\n'), finalTradeContext);
  const staleRows = rootA.rows.slice(); currentRoot = rootB;
  const replacedState = finalTradeContext.resolveFinalTradeTargetState({ root: finalTradeContext.resolveCurrentTradeAllItemsRoot(), targetItemId: 999, targetItemName: 'Zip Ties' });
  assert.equal(staleRows.some((row) => row.itemId === 999), false); assert.equal(replacedState.root, rootB); assert.equal(replacedState.targetFound, true); assert.equal(replacedState.targetRows.length, 2); assert.equal(replacedState.totalActionableQuantity, 7); assert.ok(replacedState.targetRows.every((row) => row.isConnected));
  rootB.rows = [makeTradeRow(1, 'Angle Grinder')]; const absentState = finalTradeContext.resolveFinalTradeTargetState({ root: rootB, targetItemId: 999, targetItemName: 'Zip Ties' }); assert.equal(absentState.targetFound, false);
  rootB.rows = [makeTradeRow(1, 'Angle Grinder'), makeTradeRow(2, 'Bear Gall')]; setTimeout(() => { rootB.rows.push(makeTradeRow(999, 'Zip Ties', 4)); }, 2); assert.equal(await finalTradeContext.waitForFinalTradeDomSettle({ isContextCurrent: () => true }), true); const settledState = finalTradeContext.resolveFinalTradeTargetState({ root: finalTradeContext.resolveCurrentTradeAllItemsRoot(), targetItemId: 999, targetItemName: 'Zip Ties' }); assert.equal(settledState.targetFound, true); assert.equal(settledState.totalActionableQuantity, 4);
  const addStepSource = extractFunction('processAddStep'); assert.match(addStepSource, /waitForFinalTradeDomSettle/); assert.match(addStepSource, /resolveCurrentTradeAllItemsRoot/); assert.match(addStepSource, /resolveFinalTradeTargetState/); assert.match(addStepSource, /finalState\.targetRows\.length/); assert.doesNotMatch(addStepSource, /renderTradeFilterPanel\(latest, discovery\.targets\.length, 'all'/);
  // Stored sale presentation is explicit, lower-confidence, and never broadens verified Profit semantics.
  const storedPresentationContext = { BigInt, normalizePositiveInt: offerContext.normalizePositiveInt, normalizeMoneyString: ledgerContext.normalizeMoneyString, formatLedgerMoney: ledgerContext.formatLedgerMoney, formatInventoryAdvantagePercent: deltaContext.formatInventoryAdvantagePercent }; vm.createContext(storedPresentationContext); vm.runInContext([extractFunction('resolveStoredBestSaleCandidate'), extractFunction('formatBestSalePrimary')].join('\n'), storedPresentationContext);
  const storedAngle = storedPresentationContext.resolveStoredBestSaleCandidate({ status: 'sort-available', recommendedUnitPrice: '96998', recommendedTotal: '290994', lastSeenAt: 1 }, { status: 'unavailable' }, 3); assert.equal(storedPresentationContext.formatBestSalePrimary({ status: 'unavailable' }, storedAngle), 'Stored best: Bazaar · $96,998 each · $290,994 total');
  const storedTraderWins = storedPresentationContext.resolveStoredBestSaleCandidate({ status: 'sort-available', recommendedUnitPrice: '84998', recommendedTotal: '84998' }, { status: 'fallback', channel: 'stored-trader', unitPrice: '85128', totalValue: '85128', freshness: 1 }, 1); assert.equal(storedPresentationContext.formatBestSalePrimary({ status: 'fallback' }, storedTraderWins), 'Stored best: Trader · $85,128'); assert.equal(storedPresentationContext.formatBestSalePrimary({ status: 'available', channel: 'trader', unitPrice: '828000', totalValue: '828000', quantity: 1 }, storedAngle), 'Best Sale: Trader · $828,000'); assert.equal(storedPresentationContext.formatBestSalePrimary({ status: 'unavailable' }, null), ''); assert.equal(profitContext.resolveInventoryPotentialProfit({ basisSummary: lossBasis, bestSaleResult: { status: 'fallback', verification: 'unverified', channel: 'stored-trader', unitPrice: '80000' }, quantity: lossBasis.currentQuantity }).reason, 'no-verified-best-sale');
  const managePricingContext = { BigInt, normalizeMoneyString: ledgerContext.normalizeMoneyString, formatLedgerMoney: ledgerContext.formatLedgerMoney }; vm.createContext(managePricingContext); vm.runInContext(`${extractFunction('resolveBazaarManagePricingStatus')}\n${extractFunction('summarizeBazaarManagePricing')}`, managePricingContext); assert.equal(managePricingContext.resolveBazaarManagePricingStatus('3896227', '3835997').text, 'Above market by $60,230'); assert.equal(managePricingContext.resolveBazaarManagePricingStatus('3835997', '3835997').text, 'At market'); assert.equal(managePricingContext.resolveBazaarManagePricingStatus('3800000', '3835997').text, 'Below market by $35,997'); assert.equal(managePricingContext.resolveBazaarManagePricingStatus('3800000', null).text, ''); assert.deepEqual({ ...managePricingContext.summarizeBazaarManagePricing([{ pricingStatus: { status: 'above' } }, { pricingStatus: { status: 'below' } }, { pricingStatus: { status: 'at-market' } }]) }, { total: 3, atMarket: 1, above: 1, below: 1, unavailable: 0 });
  const refreshRequests = []; const sharedRefreshContext = { Array, Set, Promise, tornState: { bazaarRefreshes: new Map() }, normalizePositiveInt: offerContext.normalizePositiveInt, normalizeBoundedText(value) { return String(value || ''); }, normalizeMoneyString: ledgerContext.normalizeMoneyString, normalizeBazaarAddSettings() { return { priceAdjustment: '-1' }; }, gmGet() { return null; }, BAZAAR_ADD_UI_SETTINGS_KEY: 'settings', requestBazaarAddMarketplaceDetail() {}, queueBazaarAddMarketplaceDetail(itemId) { refreshRequests.push(itemId); return itemId === 554 ? Promise.reject(new Error('failed')) : Promise.resolve({ offers: [{ price: String(itemId) }], apiSource: 'marketplaceDetail', capturedAt: 1 }); }, resolveBazaarRecommendedPrice(itemId) { return itemId === 206 ? { status: 'unsafe', itemId } : { status: 'available', itemId, recommendedUnitPrice: String(itemId) }; } }; vm.createContext(sharedRefreshContext); vm.runInContext(extractFunction('refreshBazaarValuesForItemIds'), sharedRefreshContext); const refreshResult = await sharedRefreshContext.refreshBazaarValuesForItemIds([1465, 206, 1465, 554], { surface: 'test' }); assert.deepEqual(refreshRequests, [1465, 206, 554]); assert.deepEqual({ checked: refreshResult.checked, updated: refreshResult.updated, kept: refreshResult.kept }, { checked: 3, updated: 1, kept: 2 }); assert.match(extractFunction('refreshOwnedInventoryBazaar'), /ownership !== 'faction-loan/); assert.doesNotMatch(extractFunction('refreshOwnedInventoryBazaar'), /applyLatestInventorySortSnapshot|queueInventoryStableSort/); assert.doesNotMatch(extractFunction('refreshBazaarValuesForItemIds'), /gmDelete|submit|click/);
  assert.match(source, /data-weav3r-bazaar-refresh/); assert.match(source, /data-wah-manage-refresh/); assert.match(extractFunction('renderBazaarManage'), /summarizeBazaarManageRegistry/); assert.match(extractFunction('renderBazaarManage'), /Recommended/); assert.doesNotMatch(extractFunction('renderBazaarManage'), /MutationObserver/); assert.doesNotMatch(extractFunction('refreshBazaarValuesForItemIds'), /writeBazaarManageField|writeBazaarAddPrice|applyBazaarSelectionPlan/);
  // Bazaar Manage keeps plain route-local observations across natural virtualization without claiming unknown completeness.
  const registryContext = { Array, Map, Object, Date, BigInt, normalizePositiveInt: offerContext.normalizePositiveInt, normalizeMoneyString: ledgerContext.normalizeMoneyString, normalizeBoundedText(value, length) { return String(value || '').trim().slice(0, length); }, formatLedgerMoney: ledgerContext.formatLedgerMoney }; vm.createContext(registryContext); vm.runInContext([extractFunction('resolveBazaarManagePricingStatus'), extractFunction('summarizeBazaarManagePricing'), extractFunction('reconcileBazaarManageRegistry'), extractFunction('summarizeBazaarManageRegistry'), extractFunction('formatBazaarManageCoverage'), extractFunction('formatBazaarManageRefreshSummary')].join('\n'), registryContext);
  const registry = new Map(); const listing = (key, itemId, price) => ({ listingKey: key, itemId, itemName: `Item ${itemId}`, quantity: 1, currentPrice: price }); const abc = [listing('item-a', 1, '100'), listing('item-b', 2, '200'), listing('item-c', 3, '300')]; let coverage = registryContext.reconcileBazaarManageRegistry(registry, abc, null, 10); assert.deepEqual({ ...coverage }, { status: 'unknown', observedCount: 3, mountedCount: 3, expectedCount: null }); assert.match(registryContext.formatBazaarManageCoverage(registryContext.summarizeBazaarManageRegistry(registry.values(), coverage)), /^Observed 3 listings · 3 currently visible/);
  const def = [listing('item-d', 4, '400'), listing('item-e', 5, '500'), listing('item-f', 6, '600')]; coverage = registryContext.reconcileBazaarManageRegistry(registry, def, 6, 20); assert.equal(registry.size, 6); assert.equal(coverage.status, 'complete'); assert.deepEqual([...registry.values()].filter((record) => record.isMounted).map((record) => record.itemId), [4, 5, 6]); coverage = registryContext.reconcileBazaarManageRegistry(registry, abc, 6, 30); assert.equal(registry.size, 6); assert.deepEqual([...registry.values()].filter((record) => record.isMounted).map((record) => record.itemId), [1, 2, 3]); assert.equal(coverage.status, 'complete');
  const partialRegistry = new Map(); const five = [...abc, ...def.slice(0, 2)]; const partialCoverage = registryContext.reconcileBazaarManageRegistry(partialRegistry, five, 6, 40); assert.equal(partialCoverage.status, 'partial'); assert.equal(partialCoverage.observedCount, 5); assert.equal(registryContext.reconcileBazaarManageRegistry(partialRegistry, def.slice(2), 6, 50).status, 'complete');
  const recycled = new Map(); registryContext.reconcileBazaarManageRegistry(recycled, [listing('item-1465', 1465, '3896227')], null, 60); registryContext.reconcileBazaarManageRegistry(recycled, [listing('item-206', 206, '3835997')], null, 70); assert.equal(recycled.size, 2); assert.equal(recycled.get('item-1465').isMounted, false); assert.equal(recycled.get('item-206').isMounted, true); assert.equal(recycled.get('item-1465').element, undefined); assert.equal(recycled.get('item-1465').priceInput, undefined);
  const duplicateItems = new Map(); registryContext.reconcileBazaarManageRegistry(duplicateItems, [listing('listing-one', 1465, '1'), listing('listing-two', 1465, '2'), listing('listing-three', 554, '3')], null, 80); assert.equal(duplicateItems.size, 3); assert.deepEqual([...new Set([...duplicateItems.values()].map((record) => record.itemId))], [1465, 554]);
  const offscreen = recycled.get('item-1465'); recycled.set('item-1465', Object.freeze({ ...offscreen, recommendedPrice: '3835997', pricingStatus: registryContext.resolveBazaarManagePricingStatus(offscreen.lastObservedCurrentPrice, '3835997') })); let analytics = registryContext.summarizeBazaarManageRegistry(recycled.values(), { status: 'unknown' }); assert.equal(analytics.aboveMarketCount, 1); recycled.set('item-1465', Object.freeze({ ...recycled.get('item-1465'), recommendedPrice: '3900000', pricingStatus: registryContext.resolveBazaarManagePricingStatus(offscreen.lastObservedCurrentPrice, '3900000') })); analytics = registryContext.summarizeBazaarManageRegistry(recycled.values(), { status: 'unknown' }); assert.equal(recycled.get('item-1465').pricingStatus.text, 'Below market by $3,773'); assert.equal(analytics.belowMarketCount, 1);
  const manageSource = `${extractFunction('renderBazaarManage')}\n${extractFunction('initBazaarManage')}\n${extractFunction('teardownBazaarManage')}`; assert.match(extractFunction('initBazaarManage'), /registry\.values/); assert.match(extractFunction('teardownBazaarManage'), /registry\.clear/); assert.match(extractFunction('initBazaarManage'), /mounted listings prepared/); assert.match(extractFunction('initBazaarManage'), /visible listings/); assert.doesNotMatch(manageSource, /scrollIntoView|scrollTop|scrollTo|scrollBy|wheel|GM_setValue|gmSet\(|__reactFiber|__reactProps|graphql/i); assert.equal((manageSource.match(/new MutationObserver/g) || []).length, 1); assert.doesNotMatch(extractFunction('reconcileBazaarManageRegistry'), /element|priceInput|removeInput|gmSet|GM_/);
