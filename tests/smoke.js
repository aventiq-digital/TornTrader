...
(async () => {
  ...
  vm.createContext(basisContext); vm.runInContext(['resolveItemLineAcquisitionCost', 'aggregateItemBasisEvents', 'formatItemBasisAverage', 'buildItemBasisSummary', 'buildTornInventoryBasisBlocks'].map(extractFunction).join('\n'), basisContext);
  ...
  let basisNode = null; const basisHost = { appendChild(node) { basisNode = node; } }; const basisRow = { querySelector(selector) { if (selector.includes('[data-wah-item-basis]')) return basisNode; if (selector === '.title-wrap') return basisHost; return null; } }; const basisRenderContext = { document: { createElement() { return { dataset: {} }; } }, formatLedgerMoney: ledgerContext.formatLedgerMoney }; vm.createContext(basisRenderContext); vm.runInContext(extractFunction('renderTornInventoryBasis'), basisRenderContext); const renderedBasis = basisRenderContext.renderTornInventoryBasis({ model: { itemId: 1084, sourceRow: basisRow } }, creditPartial); assert.equal(basisRenderContext.renderTornInventoryBasis({ model: { itemId: 1084, sourceRow: basisRow } }, creditPartial), renderedBasis); assert.match(renderedBasis.textContent, /3 \/ 4 getrackt/); assert.match(renderedBasis.title, /Item-ID-basierte/);
