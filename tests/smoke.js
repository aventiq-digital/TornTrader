...
${extractFunction('isCurrentRouteRender')}`, staleRenderContext); assert.equal(staleRenderContext.isCurrentRouteRender('1', 1), false); assert.equal(staleRenderContext.isCurrentRouteRender('2', 2), false); assert.equal(staleRenderContext.isCurrentRouteRender('3', 3), true);
  ...
  assert.match(extractFunction('refreshOwnedInventoryBazaar'), /new Set/); assert.match(extractFunction('refreshOwnedInventoryBazaar'), /ownership !== 'faction-loan'/); assert.match(extractFunction('refreshOwnedInventoryBazaar'), /queueBazaarAddMarketplaceDetail/); assert.doesNotMatch(extractFunction('refreshOwnedInventoryBazaar'), /delete|submit|click/); assert.match(extractFunction('ensureInventorySortControl'), /Refresh Bazaar/);
