...
(async () => {
  ...
  const inventoryHistoryContext = { ensureTornInventoryWahRow() { return historyHost; }, document: { createElement() { return { dataset: {}, setAttribute(name, value) { this[name] = value; } }; } } }; vm.createContext(inventoryHistoryContext); vm.runInContext(extractFunction('ensureTornInventoryHistoryButton'), inventoryHistoryContext);
  ...
  let traderNode = null; const traderHost = { appendChild(node) { traderNode = node; } }; const traderRow = { querySelector(selector) { if (selector.includes('[data-wah-item-trader]')) return traderNode; if (selector === '.title-wrap') return traderHost; return null; } }; const traderRenderContext = { ensureTornInventoryWahRow() { return traderHost; }, document: { createElement() { return { dataset: {} }; } }, normalizePositiveInt: offerContext.normalizePositiveInt, normalizeMoneyString: ledgerContext.normalizeMoneyString, formatLedgerMoney: ledgerContext.formatLedgerMoney, formatStoredQuoteAge: quoteContext.formatStoredQuoteAge, buildInventoryTraderValuation: quoteContext.buildInventoryTraderValuation, Date }; vm.createContext(traderRenderContext); vm.runInContext(extractFunction('renderTornInventoryTrader'), traderRenderContext); const traderResult = { status: 'eligible', quote: { traderName: 'Trader A', price: '812000', lastSeenAt: quoteNow } }; const renderedTrader = traderRenderContext.renderTornInventoryTrader({ model: { itemId: 367, sourceRow: traderRow }, currentQuantity: 4 }, traderResult, quoteNow + 1000); assert.equal(traderRenderContext.renderTornInventoryTrader({ model: { itemId: 367, sourceRow: traderRow }, currentQuantity: 4 }, traderResult, quoteNow + 1000), renderedTrader); assert.match(renderedTrader.textContent, /\$3,248,000 total/); assert.match(renderedTrader.title, /Item-ID-bound/);

  let bestSaleNode = null; const bestSaleHost = { appendChild(node) { bestSaleNode = node; } }; const bestSaleRow = { querySelector(selector) { if (selector.includes('[data-wah-item-best-sale]')) return bestSaleNode; if (selector === '.title-wrap') return bestSaleHost; return null; } }; const bestSaleRenderContext = { ensureTornInventoryWahRow() { return bestSaleHost; }, document: { createElement() { return { dataset: {} }; } }, Date, formatLedgerMoney: ledgerContext.formatLedgerMoney, formatStoredQuoteAge: quoteContext.formatStoredQuoteAge }; vm.createContext(bestSaleRenderContext); vm.runInContext(extractFunction('renderTornInventoryBestSale'), bestSaleRenderContext); const bestSaleResult = bestSaleContext.resolveInventoryBestSale(safeBazaar(515), eligibleTrader(500), 3); const renderedBestSale = bestSaleRenderContext.renderTornInventoryBestSale({ model: { itemId: 367, sourceRow: bestSaleRow } }, bestSaleResult, quoteNow); assert.equal(bestSaleRenderContext.renderTornInventoryBestSale({ model: { itemId: 367, sourceRow: bestSaleRow } }, bestSaleResult, quoteNow), renderedBestSale); assert.equal(renderedBestSale.textContent, 'Best Sale: Bazaar · $515 each · $1,545 total'); assert.match(renderedBestSale.title, /gross value, not guaranteed proceeds/);

  let profitNode = null; const profitHost = { appendChild(node) { profitNode = node; } }; const profitRow = { querySelector(selector) { if (selector.includes('[data-wah-item-profit]')) return profitNode; if (selector === '.title-wrap') return profitHost; return null; } }; const profitRenderContext = { ensureTornInventoryWahRow() { return profitHost; }, document: { createElement() { return { dataset: {} }; } }, formatBigIntRatioMoney: basisContext.formatBigIntRatioMoney, formatInventoryRoi: basisContext.formatInventoryRoi, inventoryProfitUnavailableReason(reason) { return reason; } }; vm.createContext(profitRenderContext); vm.runInContext(extractFunction('renderTornInventoryProfit'), profitRenderContext); const renderedProfit = profitRenderContext.renderTornInventoryProfit({ model: { itemId: 1084, sourceRow: profitRow } }, partialProfit); assert.equal(profitRenderContext.renderTornInventoryProfit({ model: { itemId: 1084, sourceRow: profitRow } }, partialProfit), renderedProfit); assert.match(renderedProfit.textContent, /Profit \+\$150 on 3\/4 · ROI \+16\.67 %/); assert.match(renderedProfit.title, /Potential gross profit/);
