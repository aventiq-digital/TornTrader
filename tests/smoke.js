...
(async () => {
  ...
  const makeFullLoadContainer = ({ all = '0', from = '100', queue = 'All', rows = 100 } = {}) => ({ attrs: { 'data-all': all, 'data-from': from, 'data-queue': queue }, rowCount: rows, getAttribute(name) { return this.attrs[name] ?? null; }, querySelectorAll() { return { length: this.rowCount }; } }); const fullLoadTimers = new Map(); let fullLoadTimerId = 1; let activeFullLoadContainer = makeFullLoadContainer(); let loadMoreAvailable = true; let loadTriggerCount = 0; const fullLoadButton = {}; let fullNow = 1000; const fullLoadContext = { Date: { now() { return fullNow; } }, Number, location: { pathname: '/item.php' }, INVENTORY_FULL_LOAD_RETRY_MS: 500, INVENTORY_FULL_LOAD_PROGRESS_TIMEOUT_MS: 5000, INVENTORY_FULL_LOAD_MAX_ATTEMPTS: 8, INVENTORY_FULL_LOAD_MAX_DURATION_MS: 25000, document: { querySelector(selector) { if (selector === '.items-cont[aria-hidden="false"]') return activeFullLoadContainer; if (selector === '#load-more-items [role="button"]') return loadMoreAvailable ? fullLoadButton : null; return null; } }, window: { jQuery(button) { assert.equal(button, fullLoadButton); return { trigger(type) { assert.equal(type, 'click'); loadTriggerCount += 1; } }; } }, logDebug() {}, scheduleTornInventoryBasisRefresh() {}, setTimeout(callback) { const id = fullLoadTimerId++; fullLoadTimers.set(id, callback); return id; }, clearTimeout(id) { fullLoadTimers.delete(id); }, state: { inventoryFullLoad: { container: null, generation: 0, status: 'idle', attempts: 0, startedAt: 0, awaitingProgress: false, snapshot: null, timer: 0 }, inventoryValuationGeneration: 1, inventoryBazaarPending: new Set() } }; vm.createContext(fullLoadContext); vm.runInContext(['getActiveTornInventoryContainer', 'getTornInventoryLoadState', 'abortTornInventoryFullLoad', 'scheduleTornInventoryFullLoad', 'ensureFullTornInventoryLoaded', 'isTornInventoryFullLoadPending'].map(extractFunction).join('\n'), fullLoadContext);
  const fullLoadStart = fullLoadContext.ensureFullTornInventoryLoaded(1000); assert.equal(fullLoadStart.status, 'triggered'); assert.equal(fullLoadStart.rowCount, 100); assert.equal(loadTriggerCount, 1); assert.equal(fullLoadContext.isTornInventoryFullLoadPending(activeFullLoadContainer), true); activeFullLoadContainer.rowCount = 154; activeFullLoadContainer.attrs['data-from'] = '154'; assert.equal(fullLoadContext.ensureFullTornInventoryLoaded(1100).status, 'progress'); const continueLoad = fullLoadTimers.get(fullLoadContext.state.inventoryFullLoad.timer); fullNow = 1150; continueLoad(); assert.equal(loadTriggerCount, 2); activeFullLoadContainer.attrs['data-all'] = '1'; const completedLoad = fullLoadContext.ensureFullTornInventoryLoaded(1200); assert.equal(completedLoad.status, 'complete'); assert.equal(completedLoad.rowCount, 154); fullLoadContext.ensureFullTornInventoryLoaded(1300); assert.equal(loadTriggerCount, 2); assert.equal(fullLoadContext.isTornInventoryFullLoadPending(activeFullLoadContainer), false);
  const alreadyComplete = makeFullLoadContainer({ all: '1', from: '20', rows: 20 }); activeFullLoadContainer = alreadyComplete; assert.equal(fullLoadContext.ensureFullTornInventoryLoaded(1400).status, 'complete'); assert.equal(loadTriggerCount, 2); const waitingControl = makeFullLoadContainer(); activeFullLoadContainer = waitingControl; loadMoreAvailable = false; assert.equal(fullLoadContext.ensureFullTornInventoryLoaded(1500).status, 'waiting-trigger'); assert.equal(loadTriggerCount, 2); loadMoreAvailable = true; const retryControl = fullLoadTimers.get(fullLoadContext.state.inventoryFullLoad.timer); fullNow = 1550; retryControl(); assert.equal(loadTriggerCount, 3); const replacementContainer = makeFullLoadContainer({ all: '1', from: '12', rows: 12 }); activeFullLoadContainer = replacementContainer; assert.equal(fullLoadContext.ensureFullTornInventoryLoaded(1600).status, 'complete'); assert.equal(fullLoadContext.state.inventoryFullLoad.container, replacementContainer);
  const stalledContainer = makeFullLoadContainer(); activeFullLoadContainer = stalledContainer; fullLoadContext.ensureFullTornInventoryLoaded(2000); for (let retry = 0; retry < 10 && fullLoadContext.state.inventoryFullLoad.status !== 'aborted'; retry += 1) { const timeout = fullLoadTimers.get(fullLoadContext.state.inventoryFullLoad.timer); fullNow += 5000; timeout(); } assert.equal(fullLoadContext.state.inventoryFullLoad.status, 'aborted'); const triggersAfterAbort = loadTriggerCount; fullLoadContext.ensureFullTornInventoryLoaded(30000); assert.equal(loadTriggerCount, triggersAfterAbort); activeFullLoadContainer = null; assert.notEqual(fullLoadContext.ensureFullTornInventoryLoaded(31000).status, 'triggered');
  const readinessContainer = {}; const readinessContext = { state: { inventorySortSettings: { criterion: 'bazaar-total' }, inventoryBazaarPending: new Set(['1:206']) }, isTornInventoryFullLoadPending() { return false; } }; vm.createContext(readinessContext); vm.runInContext(`${extractFunction('inventorySortDependsOnBazaar')}\n${extractFunction('canApplyInventorySortSnapshot')}`, readinessContext); assert.equal(readinessContext.canApplyInventorySortSnapshot(readinessContainer, 'name').allowed, true); assert.equal(readinessContext.canApplyInventorySortSnapshot(readinessContainer, 'trader-total').allowed, true); assert.deepEqual({ ...readinessContext.canApplyInventorySortSnapshot(readinessContainer, 'bazaar-total') }, { allowed: false, reason: 'bazaar-valuation-round' }); assert.equal(readinessContext.canApplyInventorySortSnapshot(readinessContainer, 'best-sale-total').allowed, false); assert.equal(readinessContext.canApplyInventorySortSnapshot(readinessContainer, 'profit').allowed, false); assert.equal(readinessContext.canApplyInventorySortSnapshot(readinessContainer, 'roi').allowed, false); readinessContext.state.inventoryBazaarPending.clear(); assert.equal(readinessContext.canApplyInventorySortSnapshot(readinessContainer, 'bazaar-total').allowed, true); readinessContext.isTornInventoryFullLoadPending = () => true; assert.deepEqual({ ...readinessContext.canApplyInventorySortSnapshot(readinessContainer, 'name') }, { allowed: false, reason: 'full-inventory-load' });
  ...
  assert.match(extractFunction('refreshTornInventoryBasis'), /queueInventoryStableSort\(container, sortRecords, scan/); assert.match(extractFunction('refreshTornInventoryBasis'), /sortRecords\.push\(\{ block, summary, bazaarValuation, traderValuation, bestSaleResult, profitResult/); assert.match(extractFunction('ensureInventorySortControl'), /getElementById\(INVENTORY_SORT_CONTROL_ID\)/); assert.match(extractFunction('ensureInventorySortControl'), /INVENTORY_SORT_SETTINGS_KEY/); assert.match(extractFunction('initTornItemInventoryHistory'), /normalizeInventorySortSettings\(gmGet\(INVENTORY_SORT_SETTINGS_KEY/); assert.doesNotMatch(extractFunction('buildInventorySortKey'), /tt-/i);
  ...
  const fullLoadSource = extractFunction('ensureFullTornInventoryLoaded'); assert.match(fullLoadSource, /#load-more-items \[role="button"\]/); assert.match(fullLoadSource, /jquery\(button\)\.trigger\('click'\)/); assert.doesNotMatch(fullLoadSource, /fetch|XMLHttpRequest|\.click\s*\(|dispatchEvent|scroll/); assert.equal((source.match(/\.trigger\('click'\)/g) || []).length, 1); assert.match(fullLoadSource, /INVENTORY_FULL_LOAD_MAX_ATTEMPTS/); assert.match(fullLoadSource, /INVENTORY_FULL_LOAD_MAX_DURATION_MS/); assert.match(extractFunction('refreshTornInventoryBasis'), /isTornInventoryFullLoadPending/); assert.match(extractFunction('refreshTornInventoryBasis'), /inventoryBazaarPending/); assert.match(extractFunction('refreshTornInventoryBasis'), /canApplyInventorySortSnapshot/); assert.match(extractFunction('initTornItemInventoryHistory'), /attributeFilter: \['data-all', 'data-from', 'aria-hidden'\]/); assert.match(extractFunction('initTornItemInventoryHistory'), /inventoryValuationGeneration \+= 1/);
