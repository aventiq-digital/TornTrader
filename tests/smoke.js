...
(async () => {
  ...
  let basisNode = null; const basisHost = { appendChild(node) { basisNode = node; } }; const basisRow = { querySelector(selector) { if (selector.includes('[data-wah-item-basis]')) return basisNode; if (selector === '.title-wrap') return basisHost; return null; } }; const basisRenderContext = { ensureTornInventoryWahRow() { return basisHost; }, getTornInventoryWahArea() { return basisHost; }, document: { createElement() { return { dataset: {} }; } }, formatLedgerMoney: ledgerContext.formatLedgerMoney, formatBigIntRatioMoney: basisContext.formatBigIntRatioMoney }; vm.createContext(basisRenderContext); vm.runInContext(extractFunction('renderTornInventoryBasis'), basisRenderContext); const renderedBasis = basisRenderContext.renderTornInventoryBasis({ model: { itemId: 1084, sourceRow: basisRow } }, creditPartial); assert.equal(basisRenderContext.renderTornInventoryBasis({ model: { itemId: 1084, sourceRow: basisRow } }, creditPartial), renderedBasis); assert.match(renderedBasis.textContent, /3\/4 tracked/); assert.match(renderedBasis.title, /Item-ID-based/); assert.equal(renderedBasis.dataset.wahState, 'available'); basisRenderContext.renderTornInventoryBasis({ model: { itemId: 1084, sourceRow: basisRow } }, { ...creditPartial, reconstructionStatus: 'untracked' }); assert.equal(renderedBasis.textContent, 'Basis —'); assert.equal(renderedBasis.dataset.wahState, 'unavailable'); assert.match(renderedBasis.title, /No confirmed purchase basis/);
