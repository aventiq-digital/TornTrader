...
${extractFunction('isCurrentRouteRender')}`, staleRenderContext); assert.equal(staleRenderContext.isCurrentRouteRender('1', 1), false); assert.equal(staleRenderContext.isCurrentRouteRender('2', 2), false); assert.equal(staleRenderContext.isCurrentRouteRender('3', 3), true);
  ...
  assert.match(source, /data-weav3r-bazaar-refresh/); assert.match(source, /data-wah-manage-refresh/); assert.match(extractFunction('renderBazaarManage'), /visible listings/); assert.match(extractFunction('renderBazaarManage'), /Recommended/); assert.doesNotMatch(extractFunction('renderBazaarManage'), /MutationObserver/); assert.doesNotMatch(extractFunction('refreshBazaarValuesForItemIds'), /writeBazaarManageField|writeBazaarAddPrice|applyBazaarSelectionPlan/);
