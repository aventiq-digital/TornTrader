...
(async () => {
  ...
  // SPA visible-state regression: route changes synchronously replace A with an item-bound loading result.
  const routeUiState = { lastUrl: 'https://weav3r.dev/item/1', itemId: '1', routeGeneration: 1, cachedData: { traders: { itemId: 1, traders: [{ traderName: 'Trader A' }] } }, lastResult: { status: 'BUY & SELL', bestTrader: { traderName: 'Trader A' } }, sourceProgress: {}, recheckTimer: 10, manualTraderCaptureTimer: 11, eligibilityTimer: 12 }; const renderedRouteResults = []; const routeUiContext = { location: { href: 'https://weav3r.dev/item/2' }, state: routeUiState, normalizePositiveInt: offerContext.normalizePositiveInt, cancelCurrentCollectors() {}, clearTimeout() {}, getCurrentItemId() { return this.location?.href?.endsWith('/3') ? '3' : '2'; }, emptySourceProgress() { return {}; }, emptyCachedData() { return { bazaar: null, itemMarket: null, traders: null }; }, beginWeav3rItemRouteTransition() {}, highlightRows() {}, renderStatusCard(result) { renderedRouteResults.push(structuredClone(result)); }, loadCachedItemData(itemId) { return { bazaar: null, itemMarket: null, traders: { itemId: Number(itemId), itemBindingVersion: 1, traders: [{ traderName: `Trader ${itemId}` }] } }; }, scheduleVisibleTraderQuoteCapture() {}, scheduleEvaluation() {} }; routeUiContext.getCurrentItemId = () => routeUiContext.location.href.endsWith('/3') ? '3' : '2'; vm.createContext(routeUiContext); vm.runInContext([extractFunction('createRouteLoadingResult'), extractFunction('invalidateCurrentItemUi'), extractFunction('handleRouteChange')].join('\n'), routeUiContext); routeUiContext.handleRouteChange(); assert.equal(routeUiState.itemId, '2'); assert.equal(routeUiState.lastResult.status, 'LOADING'); assert.equal(routeUiState.lastResult.bestTrader, undefined); assert.equal(renderedRouteResults.at(-1).itemId, 2); assert.equal(routeUiState.cachedData.traders.itemId, 2); const generationB = routeUiState.routeGeneration; routeUiContext.location.href = 'https://weav3r.dev/item/3'; routeUiContext.handleRouteChange(); assert.equal(routeUiState.itemId, '3'); assert.equal(routeUiState.lastResult.itemId, 3); assert.ok(routeUiState.routeGeneration > generationB); assert.equal(routeUiState.cachedData.traders.itemId, 3);
  assert.match(extractFunction('attachPageObservers'), /originalPushState[\s\S]*handleRouteChange\(\)/); assert.doesNotMatch(extractFunction('attachPageObservers'), /setTimeout\(handleRouteChange/); assert.match(extractFunction('renderComputedResult'), /isCurrentRouteRender\(expectedItemId, expectedGeneration\)/); assert.match(extractFunction('pumpCollectorQueue'), /isCurrentRoute\(job\.definition\.itemId, job\.generation\)/);
