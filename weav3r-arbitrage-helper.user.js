...
(function () {
  ...
  const state = {
    ...
    sharedHistoryUrl: '',
    nativeNavigationInitialized: false,
    ...
    historyTransportListenerId: null, inventoryHistoryObserver: null, inventoryHistoryInitialized: false, inventoryBasisRefreshTimer: null, inventoryBazaarResults: new Map(), inventoryBazaarSettingsListenerId: null, inventoryTraderQuoteListenerId: null, inventorySettingsListenerId: null,
  ...
  function parseTraderOffers(rootDocument = document, baseUrl = location.href) {
    const info = findTableByHeaders(['Trader', 'Buy Price', 'Rating', 'Actions'], rootDocument);
    if (!info) return [];
    const indexes = { trader: headerIndex(info.headers, 'Trader'), buyPrice: headerIndex(info.headers, 'Buy Price'), rating: headerIndex(info.headers, 'Rating'), actions: headerIndex(info.headers, 'Actions') };
    return getRows(info.table).map((row) => {
      ...
      const priceListLink = traderLinks.find((a) => /\/pricelist\/\d+(?:$|[?#/])/i.test(getUrlParts(a.getAttribute('href') || a.href, baseUrl)));
      const profileLink = traderLinks.find((a) => /profiles\.php\?XID=\d+/i.test(a.href));
      ...
      if (!buyPrice || buyPrice <= 0 || rating == null) return null;
      const links = Array.from(actionCell ? actionCell.querySelectorAll('a[href]') : []);
      const actionPriceListLink = links.find((a) => /price|list/i.test(a.textContent + a.href));
      const tradeNowLink = links.find((a) => /trade|now/i.test(a.textContent + a.href));
      const nameFallbackLink = traderLinks.find((a) => !/profiles\.php\?XID=\d+/i.test(a.href));
      const priceListIdMatch = priceListLink ? (priceListLink.getAttribute('href') || priceListLink.href).match(/\/pricelist\/(\d+)/i) : null;
      const profileIdMatch = profileLink ? profileLink.href.match(/[?&]XID=(\d+)/i) : null;
      const capturedAt = Date.now();
      return normalizeTraderActivityFields({ traderName: (priceListLink || nameFallbackLink) ? (priceListLink || nameFallbackLink).textContent.trim() : 'Unknown trader', traderId: priceListIdMatch ? priceListIdMatch[1] : profileIdMatch ? profileIdMatch[1] : null, buyPrice, buyPriceExact, rating, priceListUrl: absoluteUrl((priceListLink || actionPriceListLink || {}).getAttribute?.('href') || (priceListLink || actionPriceListLink || {}).href, baseUrl), tradeNowUrl: absoluteUrl((tradeNowLink || {}).getAttribute?.('href') || (tradeNowLink || {}).href, baseUrl), activityText: extractTraderActivityText(traderCell), capturedAt, row }, capturedAt);
    }).filter(Boolean);
  }
  ...
  function observeTraderQuote(raw, now = Date.now()) {
    const candidate = normalizeTraderQuoteRecord({ ...raw, version: 1, observedAt: Number(raw?.observedAt) || now, lastSeenAt: now, observationCount: 1, source: 'weav3r' }); if (!candidate) return null; const key = traderQuoteRecordKey(candidate); const collection = readTraderQuotes(); const existing = collection.records[key]; const record = existing ? normalizeTraderQuoteRecord({ ...existing, traderName: candidate.traderName, lastSeenAt: Math.max(existing.lastSeenAt, now), observationCount: existing.observationCount + 1, traderRatingAtObservation: candidate.traderRatingAtObservation, traderActivityTextAtObservation: candidate.traderActivityTextAtObservation, traderActivityLastSeenAt: candidate.traderActivityLastSeenAt }) : candidate; const result = mergeCriticalRecord(TRADER_QUOTES_KEY, { version: 1, records: {} }, key, record, normalizeTraderQuoteCollection); return result.ok ? result.collection.records[key] : null;
  }
  function persistObservedTraderQuotes(itemId, traders, sourceUrl, observedAt = Date.now()) {
    let parsedUrl; try { parsedUrl = new URL(sourceUrl, location.href); } catch (_) { return 0; } if (parsedUrl.protocol !== 'https:' || parsedUrl.hostname !== 'weav3r.dev') return 0; let saved = 0;
  ...
  function initWeav3rArbitrageHelper() { readSettings(); loadWatchlist(); state.itemId = getCurrentItemId(); state.routeGeneration += 1; state.sourceProgress = emptySourceProgress(); state.cachedData = loadCachedItemData(state.itemId); renderStatusCard({ status: 'LOADING', messages: ['Waiting for Weav3r tables...'] }); attachPageObservers(); evaluateCurrentPage(); }
  ...
  function resolveItemLineAcquisitionCost(event, line) {
    ...
    const totalPrice = line.totalPrice == null ? null : normalizeMoneyString(line.totalPrice); if (totalPrice != null) return totalPrice;
    const unitPrice = line.unitPrice == null ? null : normalizeMoneyString(line.unitPrice); if (unitPrice != null) return (BigInt(unitPrice) * BigInt(line.quantity)).toString();
    if (event?.itemLines?.length === 1 && event.cash?.direction === 'out') return normalizeMoneyString(event.cash.amount);
    return null;
  }
  ...
  function buildItemBasisSummary(itemId, currentQuantity, aggregates) {
    ...
    const knownPurchaseCost = aggregate.knownPurchaseCost.toString(); const weightedConfirmedPurchasePrice = formatItemBasisAverage(aggregate.knownPurchaseCost, aggregate.knownPurchaseQuantity);
    return { itemId: normalizedItemId, currentQuantity: current, confirmedInboundQuantity: aggregate.confirmedInboundQuantity, confirmedOutboundQuantity: aggregate.confirmedOutboundQuantity, trackedNetQuantity, coveredQuantity: ['full', 'partial'].includes(reconstructionStatus) ? trackedNetQuantity : null, unknownQuantity: reconstructionStatus === 'partial' || reconstructionStatus === 'untracked' ? current - Math.max(0, trackedNetQuantity) : 0, knownPurchaseQuantity: aggregate.knownPurchaseQuantity, knownPurchaseCost, weightedConfirmedPurchasePrice, reconstructionStatus };
  }
  ...
  function resolveInventoryTraderQuote(itemId, quoteIndex, minimumTraderRating, now = Date.now(), personalRatings = null) {
    const quotes = quoteIndex?.get?.(normalizePositiveInt(itemId)) || []; if (!quotes.length) return { status: 'unavailable', quote: null, reason: 'no-quotes' };
    const classified = quotes.map((quote) => ({ quote, eligibility: getStoredTraderQuoteEligibility(quote, minimumTraderRating, now, personalRatings) })); const eligible = classified.filter((entry) => entry.eligibility === 'eligible').sort(compareCurrentTraderQuotes); if (eligible.length) return { status: 'eligible', ...eligible[0], reason: null };
    const unknown = classified.filter((entry) => entry.eligibility === 'unknown').sort((left, right) => right.quote.lastSeenAt - left.quote.lastSeenAt || right.quote.observedAt - left.quote.observedAt || left.quote.traderId - right.quote.traderId); return unknown.length ? { status: 'unverified', ...unknown[0], reason: 'current-status-unverified' } : { status: 'unavailable', quote: null, reason: 'no-eligible-quotes' };
  }
  ...
  function renderTornInventoryBasis(block, summary) {
    ...
    else basis.textContent = `Basis · ${summary.trackedNetQuantity} / ${summary.currentQuantity} getrackt · Ø Kauf ${average} · Kosten ${summary.knownPurchaseQuantity ? formatLedgerMoney(summary.knownPurchaseCost) : '—'}`;
    basis.title = `Item-ID-basierte Auswertung aus bestätigten Weav3r-Arbitrage-Helper-Transaktionen. Andere Bestandsänderungen können nicht vollständig erfasst sein.${summary.knownPurchaseQuantity < summary.confirmedInboundQuantity ? ' Nicht alle bestätigten Eingänge besitzen einen eindeutig zuordenbaren Preis.' : ''}`;
    return basis;
  }
  ...
  function renderTornInventoryBazaar(block, result) {
    ...
    if (valuation.status === 'loading') node.textContent = 'Bazaar …'; else if (valuation.status !== 'available') node.textContent = 'Bazaar n. v.'; else node.textContent = valuation.quantity === 1 ? `Bazaar ${formatLedgerMoney(valuation.recommendedUnitPrice)}` : `Bazaar ${formatLedgerMoney(valuation.recommendedUnitPrice)} / Stk. · ${formatLedgerMoney(valuation.recommendedTotal)} gesamt`;
    node.title = valuation.status === 'available' ? `Möglicher Brutto-Verkaufswert auf Basis der aktuellen Bazaar-Preisempfehlung. Kein garantierter Verkaufserlös. Markt: ${formatLedgerMoney(valuation.marketUnitPrice)} · Anpassung: ${valuation.adjustment} · Empfehlung: ${formatLedgerMoney(valuation.recommendedUnitPrice)}.` : `Bazaar-Preis nicht verfügbar${valuation.reason ? ` (${valuation.reason})` : ''}. Keine Preis- oder Gewinnerwartung wird erfunden.`;
    return node;
  }
  function refreshTornInventoryBasis(root = document) {
    if (location.pathname !== '/item.php') return 0;
    const candidateRows = Array.from(root.querySelectorAll?.('li[data-item][data-rowkey]') || []); const containers = [...new Set(candidateRows.map((row) => row.parentElement).filter(Boolean))]; const events = listTransactionEvents(); const aggregates = aggregateItemBasisEvents(events); const quoteIndex = indexLatestTraderQuotes(readTraderQuotes()); const personalRatings = readTraderPersonalRatings().records; const adjustment = normalizeBazaarAddSettings(gmGet(BAZAAR_ADD_UI_SETTINGS_KEY, null)).priceAdjustment; const requestedItemIds = new Set(); const now = Date.now(); let rendered = 0;
    for (const container of containers) {
      const scan = scanTornInventoryList(container); const blocks = buildTornInventoryBasisBlocks(scan); const targetRows = new Set(blocks.map((block) => block.model.sourceRow));
      for (const model of scan.rows) if (!targetRows.has(model.sourceRow)) { model.sourceRow.querySelector?.(':scope [data-wah-item-basis]')?.remove(); model.sourceRow.querySelector?.(':scope [data-wah-item-bazaar]')?.remove(); model.sourceRow.querySelector?.(':scope [data-wah-item-trader]')?.remove(); }
      for (const block of blocks) {
        ...
        renderTornInventoryBazaar(block, priceResult);
        renderTornInventoryTrader(block, resolveInventoryTraderQuote(block.model.itemId, quoteIndex, state.settings.minimumTraderRating, now, personalRatings), now);
      }
    }
    for (const itemId of requestedItemIds) void loadBazaarRecommendedPrice(itemId, { adjustment }).then((result) => { if (location.pathname !== '/item.php') return; const currentAdjustment = normalizeBazaarAddSettings(gmGet(BAZAAR_ADD_UI_SETTINGS_KEY, null)).priceAdjustment; if (result.adjustment !== currentAdjustment) return; state.inventoryBazaarResults.set(itemId, result); scheduleTornInventoryBasisRefresh(); });
    logDebug('item.php basis refresh.', { eventCount: events.length, rendered }); return rendered;
  }
  function scheduleTornInventoryBasisRefresh() { if (location.pathname !== '/item.php' || state.inventoryBasisRefreshTimer) return; state.inventoryBasisRefreshTimer = setTimeout(() => { state.inventoryBasisRefreshTimer = null; refreshTornInventoryBasis(document); }, 40); }
  ...
  function initTornItemInventoryHistory() { if (location.pathname !== '/item.php') return; processTornInventoryRows(document); scheduleTornInventoryBasisRefresh(); if (state.inventoryHistoryInitialized) return; state.inventoryHistoryInitialized = true; state.inventoryHistoryObserver = new MutationObserver((mutations) => { let changed = false; for (const mutation of mutations) for (const node of mutation.addedNodes || []) if (node?.nodeType === 1) { processTornInventoryRows(node); changed = true; } if (changed) scheduleTornInventoryBasisRefresh(); }); state.inventoryHistoryObserver.observe(document.documentElement, { childList: true, subtree: true }); if (typeof GM_addValueChangeListener === 'function') { state.inventoryBazaarSettingsListenerId = GM_addValueChangeListener(BAZAAR_ADD_UI_SETTINGS_KEY, () => { state.inventoryBazaarResults.clear(); scheduleTornInventoryBasisRefresh(); }); state.inventoryTraderQuoteListenerId = GM_addValueChangeListener(TRADER_QUOTES_KEY, scheduleTornInventoryBasisRefresh); state.inventorySettingsListenerId = GM_addValueChangeListener(SETTINGS_KEY, () => { readSettings(); scheduleTornInventoryBasisRefresh(); }); } window.addEventListener('beforeunload', () => { state.inventoryHistoryObserver?.disconnect(); clearTimeout(state.inventoryBasisRefreshTimer); if (typeof GM_removeValueChangeListener === 'function') { if (state.inventoryBazaarSettingsListenerId) GM_removeValueChangeListener(state.inventoryBazaarSettingsListenerId); if (state.inventoryTraderQuoteListenerId) GM_removeValueChangeListener(state.inventoryTraderQuoteListenerId); if (state.inventorySettingsListenerId) GM_removeValueChangeListener(state.inventorySettingsListenerId); } state.inventoryHistoryObserver = null; state.inventoryBasisRefreshTimer = null; state.inventoryBazaarSettingsListenerId = null; state.inventoryTraderQuoteListenerId = null; state.inventorySettingsListenerId = null; state.inventoryBazaarResults.clear(); state.inventoryHistoryInitialized = false; }, { once: true }); logDebug('Torn item.php inventory history, basis, Bazaar, and Trader valuation parser initialized.'); }
