...
(function () {
  ...
  const state = {
    ...
    sharedHistoryUrl: '',
    nativeNavigationInitialized: false, manualTraderCaptureTimer: 0, routeDomReady: true, staleRouteTables: new Set(),
  ...
  function isWeav3rItemSourceTable(table) {
    const headers = Array.from(table?.querySelectorAll?.('thead th') || []).map((cell) => normalizeText(cell.textContent).toLowerCase());
    return headers.includes('trader') && headers.includes('buy price') || headers.some((header) => header.includes('price')) && headers.length >= 2;
  }
  ...
  function beginWeav3rItemRouteTransition() { state.staleRouteTables = snapshotWeav3rItemSourceTables(document); state.routeDomReady = state.staleRouteTables.size === 0; }
  function updateWeav3rItemRouteDomReadiness(mutations = []) {
    if (state.routeDomReady) return true;
    for (const table of Array.from(state.staleRouteTables)) {
      if (!table.isConnected) { state.staleRouteTables.delete(table); continue; }
    }
    if (!state.staleRouteTables.size) state.routeDomReady = true;
  ...
  function handleRouteChange() { if (location.href === state.lastUrl) return; state.lastUrl = location.href; cancelCurrentCollectors(); clearTimeout(state.recheckTimer); clearTimeout(state.manualTraderCaptureTimer); clearTimeout(state.eligibilityTimer); state.recheckTimer = 0; state.manualTraderCaptureTimer = 0; state.eligibilityTimer = 0; state.routeGeneration += 1; state.itemId = getCurrentItemId(); state.sourceProgress = emptySourceProgress(); state.cachedData = emptyCachedData(); beginWeav3rItemRouteTransition(); invalidateCurrentItemUi(state.itemId, state.routeGeneration, 'Route changed. Loading current item...'); state.cachedData = loadCachedItemData(state.itemId); scheduleVisibleTraderQuoteCapture(); scheduleEvaluation(); }
