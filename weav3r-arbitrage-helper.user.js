...
(function () {
  ...
  function ensureFullTornInventoryLoaded(now = Date.now()) {
    ...
    const snapshot = getTornInventoryLoadState(container); const validLoadState = Boolean(snapshot && ['0', '1'].includes(snapshot.dataAll) && snapshot.dataFrom != null && snapshot.dataQueue); logDebug('inventory-full-load-state', { ...snapshot, status: load.status, triggerAttempts: load.triggerAttempts, readinessRetries: load.readinessRetries, generation: load.generation });
    if (!validLoadState) { if (now - load.startedAt > INVENTORY_FULL_LOAD_MAX_DURATION_MS) return abortTornInventoryFullLoad('load-state-timeout'); load.status = 'waiting-for-load-state'; load.readinessRetries += 1; scheduleTornInventoryFullLoad(); return { status: 'waiting-for-load-state', reason: 'container-state-not-ready' }; }
    ...
    if (load.awaitingProgress) { const progressed = snapshot.dataAll === '1' || snapshot.dataFrom > load.snapshot.dataFrom || snapshot.rowCount > load.snapshot.rowCount; if (!progressed) return { status: 'awaiting-progress', ...snapshot }; clearTimeout(load.timer); load.timer = 0; load.awaitingProgress = false; load.status = 'ready-to-load'; logDebug('inventory-full-load-progress', { before: load.snapshot, after: snapshot, generation: load.generation }); scheduleTornInventoryFullLoad(100); return { status: 'ready-to-load', progressed: true, ...snapshot }; }
    if (load.triggerAttempts >= INVENTORY_FULL_LOAD_MAX_ATTEMPTS) return abortTornInventoryFullLoad('no-progress'); const button = document.querySelector?.('#load-more-items [role="button"]'); const jquery = window.jQuery || window.$;
    if (!button || typeof jquery !== 'function') { load.status = 'ready-to-load'; load.readinessRetries += 1; scheduleTornInventoryFullLoad(); return { status: 'ready-to-load', reason: !button ? 'load-more-control-missing' : 'jquery-unavailable' }; }
    load.triggerAttempts += 1; load.snapshot = snapshot; load.awaitingProgress = true; load.status = 'awaiting-progress'; logDebug('inventory-full-load-trigger', { attempt: load.triggerAttempts, ...snapshot, generation: load.generation }); jquery(button).trigger('click'); scheduleTornInventoryFullLoad(INVENTORY_FULL_LOAD_PROGRESS_TIMEOUT_MS, true); return { status: 'awaiting-progress', triggered: true, attempt: load.triggerAttempts, ...snapshot };
  }
  ...
  function canApplyInventorySortSnapshot(container, criterion = state.inventorySortSettings.criterion, now = Date.now()) { if (isTornInventoryFullLoadPending(container)) return { allowed: false, reason: 'full-inventory-load' }; if (!inventorySortDependsOnBazaar(criterion)) return { allowed: true, reason: null }; const round = state.inventoryBazaarRound; if (round.generation === state.inventoryValuationGeneration && state.inventoryBazaarPending.size && now - round.startedAt < INVENTORY_BAZAAR_INITIAL_SETTLE_MS) return { allowed: false, reason: 'bazaar-initial-settle' }; return { allowed: true, reason: null }; }
