...
(function () {
  ...
  const BAZAAR_MANAGE_TOOLS_ID = 'weav3r-bazaar-manage-tools';
  const SELLING_DECISION_STYLE_ID = 'weav3r-selling-decision-style';
  const BAZAAR_ADD_RULES = Object.freeze(['sell-all', 'keep-one', 'dont-sell']);
  const BAZAAR_SELL_RULE_LABELS = Object.freeze({ 'sell-all': 'Sell all', 'keep-one': 'Keep one', 'dont-sell': "Don't sell" });
  ...
  const state = {
    ...
    sharedHistoryUrl: '',
    nativeNavigationInitialized: false, manualTraderCaptureTimer: 0, routeDomReady: true, routeSnapshotFailed: false, staleRouteTables: new Map(), routeTransition: { startedAt: 0, itemId: null, generation: 0, visibleDomBlocked: false },
    ...
    historyTransportListenerId: null, inventoryHistoryObserver: null, inventoryHistoryInitialized: false, inventoryBasisRefreshTimer: null, inventoryBazaarRefreshTimer: null, inventoryBazaarResults: new Map(), inventoryBazaarManualRefresh: null, inventoryBazaarSettingsListenerId: null, inventoryBazaarQuoteListenerId: null, inventoryTraderQuoteListenerId: null, inventorySettingsListenerId: null, inventoryBazaarSellRulesListenerId: null, inventoryBazaarSellFilter: false, inventorySortSettings: { criterion: 'original', direction: 'asc' }, inventorySortOriginals: new WeakMap(), inventorySortLastSignatures: new WeakMap(), inventorySortSnapshots: new WeakMap(), inventorySortLatest: new WeakMap(), inventorySortDirty: new WeakSet(), inventorySortPending: new Map(), inventorySortQuietTimer: 0, inventorySortMaxTimer: 0, inventorySortImmediate: false, inventorySortInternalRows: new WeakSet(), inventoryFullLoad: { container: null, generation: 0, status: 'idle', triggerAttempts: 0, readinessRetries: 0, startedAt: 0, awaitingProgress: false, snapshot: null, terminalSnapshot: null, timer: 0 }, inventoryValuationGeneration: 1, inventoryBazaarPending: new Set(), inventoryBazaarRound: { generation: 0, startedAt: 0, total: 0, settled: 0, settledSinceRefresh: 0, initialSnapshotDone: false },
  ...
  function resolveStoredBestSaleCandidate(storedBazaar, bestSaleResult, quantity) {
    const normalizedQuantity = normalizePositiveInt(quantity); if (!normalizedQuantity || bestSaleResult?.status === 'available') return null;
    const candidates = [];
    if (storedBazaar?.status === 'sort-available' && normalizeMoneyString(storedBazaar.recommendedUnitPrice)) candidates.push({ source: 'bazaar', unitPrice: storedBazaar.recommendedUnitPrice, totalValue: storedBazaar.recommendedTotal, quantity: normalizedQuantity, freshness: storedBazaar.lastSeenAt || null, verification: 'unverified' });
    if (bestSaleResult?.channel === 'stored-trader' && normalizeMoneyString(bestSaleResult.unitPrice)) candidates.push({ source: 'trader', unitPrice: bestSaleResult.unitPrice, totalValue: bestSaleResult.totalValue, quantity: normalizedQuantity, freshness: bestSaleResult.freshness || null, trader: bestSaleResult.trader, verification: 'unverified' });
    candidates.sort((left, right) => { const difference = BigInt(right.unitPrice) - BigInt(left.unitPrice); return difference === 0n ? (left.source === 'bazaar' ? -1 : 1) : difference > 0n ? 1 : -1; }); return candidates[0] || null;
  }
  function formatBestSalePrimary(bestSaleResult, storedBest, marketDelta = null) {
    const candidate = bestSaleResult?.status === 'available' ? bestSaleResult : storedBest; if (!candidate) return '';
    const source = candidate.channel === 'bazaar' || candidate.source === 'bazaar' ? 'Bazaar' : 'Trader'; const value = candidate.quantity === 1 ? formatLedgerMoney(candidate.unitPrice) : `${formatLedgerMoney(candidate.unitPrice)} each · ${formatLedgerMoney(candidate.totalValue)} total`;
    if (bestSaleResult?.status !== 'available') return `Stored best: ${source} · ${value}`;
    const delta = marketDelta ? ` · +${formatLedgerMoney(marketDelta.absoluteDelta)} (${formatInventoryAdvantagePercent(marketDelta.percentage)}) vs ${marketDelta.alternative === 'bazaar' ? 'Bazaar' : 'Trader'}` : ''; return `Best Sale: ${source} · ${value}${delta}`;
  }
  ...
  function isWeav3rItemSourceTable(table) {
    const headers = Array.from(table?.querySelectorAll?.('thead th') || []).map((cell) => normalizeHeaderText(cell?.textContent));
    if (!headers.length) return false;
    return [['trader', 'buy price', 'rating', 'actions'], ['seller', 'quantity', 'price / total value', 'last checked'], ['#', 'price', 'quantity', 'total value', 'vs avg']].some((required) => required.every((header) => headers.includes(header)));
  }
  function fingerprintWeav3rItemSourceTable(table) {
    if (!isWeav3rItemSourceTable(table)) return null; const headers = Array.from(table.querySelectorAll?.('thead th') || []).map((cell) => normalizeHeaderText(cell?.textContent)); let rows = Array.from(table.querySelectorAll?.('tbody tr') || []); if (!rows.length) rows = Array.from(table.querySelectorAll?.('tr') || []).filter((row) => !row.querySelector?.('th')); const rowText = (row) => normalizeBoundedText(row?.textContent, 400).toLowerCase(); return JSON.stringify({ headers, rowCount: rows.length, first: rowText(rows[0]), last: rowText(rows[rows.length - 1]) });
  }
  function snapshotWeav3rItemSourceTables(root = document) { return new Map(Array.from(root?.querySelectorAll?.('table') || []).map((table) => [table, fingerprintWeav3rItemSourceTable(table)]).filter((entry) => entry[1])); }
  function beginWeav3rItemRouteTransition() {
    state.staleRouteTables = new Map(); state.routeDomReady = false; state.routeSnapshotFailed = false;
    try { state.staleRouteTables = snapshotWeav3rItemSourceTables(document); state.routeDomReady = state.staleRouteTables.size === 0; state.routeTransition.visibleDomBlocked = !state.routeDomReady; logDebug('route-transition-source-snapshot', { tableCount: state.staleRouteTables.size }); if (state.routeDomReady) logDebug('route-transition-ready'); else logDebug('route-transition-wait', { reason: 'stale-visible-dom' }); }
    catch (error) { state.routeSnapshotFailed = true; state.routeTransition.visibleDomBlocked = true; logDebug('route-transition-wait', { reason: 'source-snapshot-failed', error: String(error?.message || error) }); }
    return state.routeDomReady;
  }
  function updateWeav3rItemRouteDomReadiness() {
    if (state.routeDomReady) return true; if (state.routeSnapshotFailed) return beginWeav3rItemRouteTransition();
    for (const [table, fingerprint] of Array.from(state.staleRouteTables)) { const current = table.isConnected ? fingerprintWeav3rItemSourceTable(table) : null; if (!table.isConnected || current && current !== fingerprint) state.staleRouteTables.delete(table); }
    if (!state.staleRouteTables.size) { state.routeDomReady = true; state.routeTransition.visibleDomBlocked = false; logDebug('route-transition-ready'); }
    return state.routeDomReady;
  }
  function isCurrentItemView(itemId, generation) { return state.routeDomReady && normalizePositiveInt(itemId) === normalizePositiveInt(state.itemId) && generation === state.routeGeneration && normalizePositiveInt(resolveWeav3rItemRouteId(location.href)) === normalizePositiveInt(itemId); }
  function captureVisibleWeav3rTraderQuotes(now = Date.now(), expectedItemId = state.itemId, expectedGeneration = state.routeGeneration) {
    const itemId = resolveWeav3rItemRouteId(location.href); logDebug('visible-weav3r-route', { href: location.href, itemId, generation: expectedGeneration, domReady: state.routeDomReady }); if (!itemId) return { status: 'ignored', itemId: null, parsed: 0, saved: 0 }; if (!isCurrentItemView(expectedItemId, expectedGeneration) || itemId !== normalizePositiveInt(expectedItemId)) return { status: 'stale-route', itemId, parsed: 0, saved: 0 }; const traders = parseTraderOffers(document, location.href); if (!isCurrentItemView(expectedItemId, expectedGeneration)) return { status: 'stale-route', itemId, parsed: 0, saved: 0 }; if (!traders.length) return { status: 'waiting', itemId, parsed: 0, saved: 0 }; const saved = persistObservedTraderQuotes(itemId, traders, location.href, now); const stored = debugStoredTraderQuotes(itemId); return { status: saved ? 'persisted' : 'rejected', itemId, parsed: traders.length, saved, stored: stored.count };
  }
  function scheduleVisibleTraderQuoteCapture() { clearTimeout(state.manualTraderCaptureTimer); const itemId = state.itemId; const generation = state.routeGeneration; state.manualTraderCaptureTimer = setTimeout(() => { state.manualTraderCaptureTimer = 0; captureVisibleWeav3rTraderQuotes(Date.now(), itemId, generation); }, RECHECK_DELAY_MS); }
  ...
  function isCompatibleSourceEntry(entry, sourceType, itemId = null) { if (!entry || typeof entry !== 'object' || !Number.isFinite(Number(entry.capturedAt))) return false; if (itemId && normalizePositiveInt(entry.itemId) !== normalizePositiveInt(itemId)) return false; if (sourceType === 'traders') return entry.itemBindingVersion === 1 && Array.isArray(entry.traders); return Array.isArray(entry.offers); }
  ...
  function loadCachedSourceData(itemId, sourceType) { const entry = gmGet(storageKey(itemId, sourceType), null); return isCompatibleSourceEntry(entry, sourceType, itemId) ? entry : null; }
  ...
  function bazaarSellRuleOptionsHtml() { return BAZAAR_ADD_RULES.map((rule) => `<option value="${rule}">${BAZAAR_SELL_RULE_LABELS[rule]}</option>`).join(''); }
  ...
  function resolveBazaarInventorySellDecision(itemId, quantity, rules) {
    const sellRule = getBazaarSellRule(itemId, rules); const calculated = calculateBazaarSellable(quantity, null, sellRule, true);
    return { itemId: normalizePositiveInt(itemId), inventoryQuantity: normalizePositiveInt(quantity) || 0, sellRule, sellableQuantity: calculated.sellableQuantity, visible: calculated.sellableQuantity > 0 };
  }
  ...
  function getBazaarRowDisplayState(model, externalConflict = false) {
    if (!model?.isTradable) return { mode: 'unavailable', metadata: 'Not sellable on Torn', fillDisabled: true };
    if (model.sellRule === 'dont-sell') return { mode: 'protected', metadata: 'Protected', fillDisabled: true };
    if (model.sellRule === 'keep-one') return { mode: 'keep-one', metadata: `${model.sellableQuantity} sellable · ${formatMoney(model.sellableMarketValue)}`, fillDisabled: model.sellableQuantity <= 0 || externalConflict };
    return { mode: 'sell-all', metadata: '', fillDisabled: externalConflict };
  }
  function getBazaarFillMessage(result) { return result?.state === 'loading' ? 'Loading Bazaar prices…' : result?.reason === 'no-listings' ? 'No Bazaar listings found' : ['request-failed', 'malformed-response', 'cache-write-failed'].includes(result?.reason) ? 'Could not load Bazaar prices' : result?.reason === 'anomaly' ? 'Suspiciously low Bazaar price' : result?.reason === 'price-drop' ? 'New price is over 7% below the current price' : result?.reason === 'protected' ? 'Protected' : result?.reason === 'group-closed' ? 'Open the group first' : result?.reason === 'group-rule-exceeded' ? `Selection exceeds sell rule: maximum ${result.maximum} of ${result.total}` : result?.reason === 'partial-aggregate' ? 'Partial quantity cannot be selected safely here' : result?.reason === 'external-conflict' ? 'External price filler detected' : result?.ok ? `Price ${formatMoney(result.proposal)} prepared` : 'Could not set price'; }
  function renderBazaarPriceContext(model, result) { if (!model) return; const feedback = { state: result.state || (result.ok ? 'success' : ['anomaly', 'price-drop'].includes(result.reason) ? 'warning' : 'error'), message: getBazaarFillMessage(result), offers: (result.offers || []).slice(0, 8) }; tornState.bazaarAdd.rowFeedback.set(model.rowKey, feedback); renderBazaarAddRowTools(model); scheduleBazaarAddRender(); }
  ...
  function getBazaarBulkSelectionMessage(reason, result) { result = result || {}; return reason === 'group-rule-exceeded' ? `Selection exceeds sell rule: maximum ${result.maximum} of ${result.total}` : reason === 'rule-exceeded' ? 'Selection exceeds sell rule' : reason === 'protected' ? 'Protected by sell rule' : reason === 'group-closed' ? 'Open the group first' : reason === 'manual-keep-one' ? 'Keep one: select instances manually' : 'Invalid selection quantity'; }
  function setBazaarBulkRowResult(model, result) { if (!model) return; if (result.reason && ['rule-exceeded', 'group-rule-exceeded', 'protected', 'group-closed', 'manual-keep-one', 'invalid-selection', 'ineligible'].includes(result.reason)) { tornState.bazaarAdd.rowFeedback.set(model.rowKey, { state: 'error', message: getBazaarBulkSelectionMessage(result.reason, result), offers: [] }); renderBazaarAddRowTools(model); scheduleBazaarAddRender(); } else renderBazaarPriceContext(model, result); }
  ...
  function renderBazaarAddRowTools(model, decisionContext = null) {
    let tools = model.element.querySelector('[data-weav3r-bazaar-row-tools]'); if (!tools) { tools = document.createElement('div'); tools.dataset.weav3rBazaarRowTools = '1'; model.element.appendChild(tools); } const externalConflict = hasExternalBazaarPriceFiller(); const display = getBazaarRowDisplayState(model, externalConflict); const feedback = tornState.bazaarAdd.rowFeedback.get(model.rowKey); const statusId = `weav3r-bazaar-row-status-${domSafeBazaarRowKey(model.rowKey)}`; const group = getBazaarAddGroupForRow(model); const groupNote = group?.isUniqueGroup && model.sellRule === 'keep-one' ? (group.expanded ? `Keep one: select instances manually (max. ${group.sellableQuantity} of ${group.totalQuantity})` : 'Keep one: open the group first') : group?.isUniqueGroup && !group.expanded ? 'Open the group first' : ''; tools.className = `weav3r-bazaar-row-tools weav3r-bazaar-row-tools--${display.mode}`;
    if (!model.isTradable) tools.innerHTML = `<small class="weav3r-bazaar-row-tools__meta">${display.metadata}</small>`; else { const loading = feedback?.state === 'loading'; const disabled = display.fillDisabled || loading || (group?.isUniqueGroup && model.groupRole === 'parent'); const offers = (feedback?.offers || []).map((offer) => `<button type="button" class="weav3r-bazaar-row-tools__price" data-weav3r-bazaar-price-choice="${offer.price}" data-weav3r-bazaar-row-key="${escapeAttribute(model.rowKey)}">${offer.quantity ? `${offer.quantity} × ` : ''}${formatMoney(offer.price)}</button>`).join(''); tools.innerHTML = `<label class="weav3r-bazaar-row-tools__rule"><span>Sell</span><select aria-label="Sell rule for ${escapeAttribute(model.itemName)}" data-weav3r-bazaar-rule="${model.itemId}">${bazaarSellRuleOptionsHtml()}</select></label><button type="button" class="weav3r-bazaar-row-tools__fill" data-weav3r-bazaar-fill="1" data-weav3r-bazaar-row-key="${escapeAttribute(model.rowKey)}" aria-label="Fill quantity and price for ${escapeAttribute(model.itemName)}" ${feedback?.message ? `aria-describedby="${statusId}"` : ''} ${disabled ? 'disabled' : ''}>${loading ? 'Loading…' : 'Fill'}</button>${display.metadata ? `<small class="weav3r-bazaar-row-tools__meta">${display.metadata}</small>` : ''}<small id="${statusId}" class="weav3r-bazaar-row-tools__status${['warning', 'error'].includes(feedback?.state) ? ' weav3r-bazaar-row-tools__status--warning' : ''}" ${feedback?.message ? 'role="status"' : 'hidden'}>${escapeHtml(feedback?.message || '')}</small>${offers ? `<div class="weav3r-bazaar-row-tools__prices"><strong>Bazaar:</strong>${offers}</div>` : ''}<small data-weav3r-bazaar-selection-note>${escapeHtml(groupNote)}</small>`; tools.querySelector('select').value = model.sellRule; if (decisionContext && (!group || model === group.parent)) { const quantity = group?.totalQuantity || model.quantity; const decision = document.createElement('div'); decision.className = 'wah-selling-decision weav3r-bazaar-add-decision'; decision.dataset.wahBazaarAddDecision = String(model.itemId); decision.innerHTML = formatSellingDecisionHtml(resolveSellingDecisionSnapshot(model.itemId, quantity, decisionContext), quantity, model.itemId, model.itemName); tools.appendChild(decision); } } model.element.dataset.weav3rSellRule = model.sellRule; model.element.dataset.weav3rBazaarRowKey = model.rowKey; model.element.hidden = shouldHideBazaarAddRow(model, tornState.bazaarAdd.settings.hideProtected);
  }
  ...
  function ensureBazaarAddToolbar(panel) {
    ...
    toolbar = document.createElement('section'); toolbar.id = BAZAAR_ADD_MANAGER_ID; toolbar.className = 'weav3r-bazaar-manager'; toolbar.dataset.weav3rBazaarManager = '1'; toolbar.innerHTML = `<div class="weav3r-bazaar-manager__row weav3r-bazaar-manager__row--primary"><strong class="weav3r-bazaar-manager__title">Bazaar Sell Manager</strong><div class="weav3r-bazaar-manager__group"><span>Sort by:</span><div class="weav3r-bazaar-manager__sort"><label class="weav3r-visually-hidden" for="weav3r-bazaar-sort">Sort field</label><select id="weav3r-bazaar-sort" data-weav3r-bazaar-sort><option value="name">Name</option><option value="quantity">Quantity</option><option value="marketValueEach">Market value / item</option><option value="totalMarketValue">Total market value</option><option value="sellableMarketValue">Sellable market value</option></select><button type="button" data-weav3r-bazaar-direction></button></div></div><div class="weav3r-bazaar-manager__actions"><button type="button" data-weav3r-bazaar-refresh>Refresh Bazaar</button><button type="button" class="weav3r-bazaar-control--primary" data-weav3r-bazaar-select-all title="Select all items allowed by sell rules" aria-label="Select all items allowed by sell rules">Select sellable</button><button type="button" data-weav3r-bazaar-fill-selected title="Fill prices for selected sellable positions" aria-label="Fill prices for selected sellable positions">Fill prices</button><button type="button" data-weav3r-bazaar-reset title="Clear current quantity selection" aria-label="Clear current quantity selection">Clear selection</button></div></div><div class="weav3r-bazaar-manager__row weav3r-bazaar-manager__row--secondary"><label class="weav3r-bazaar-manager__check"><input type="checkbox" data-weav3r-bazaar-hide> Hide protected</label><label class="weav3r-bazaar-manager__adjustment">Price adjustment: <input type="text" inputmode="numeric" size="5" data-weav3r-bazaar-adjustment aria-label="Signed price adjustment in dollars"></label><span class="weav3r-bazaar-manager__summary" data-weav3r-bazaar-summary></span><span class="weav3r-bazaar-manager__warning" data-weav3r-bazaar-warning></span></div><div class="weav3r-bazaar-manager__status" data-weav3r-bazaar-status aria-live="polite" hidden></div><div class="weav3r-bazaar-manager__price-context" data-weav3r-bazaar-price-context hidden></div>`;
    panel.parentElement?.insertBefore(toolbar, panel); return toolbar;
  }
  function renderBazaarAddManager() {
    if (!isTornBazaarAddRoute()) return teardownBazaarAddManager(); ensureSellingDecisionStyles(); const panel = getVisibleBazaarAddPanel(); if (!panel) return; const toolbar = ensureBazaarAddToolbar(panel); const built = buildBazaarAddGroups(panel); tornState.bazaarAdd.rows = built.rows; tornState.bazaarAdd.groups = built.groups; const decisionContext = buildSellingDecisionContext(); const sortedGroups = [...built.groups].sort((a, b) => compareBazaarAddModels(a.sortModel, b.sortModel, tornState.bazaarAdd.settings)); let order = 0; sortedGroups.forEach((group) => group.rows.forEach((model) => { renderBazaarAddRowTools(model, decisionContext); model.element.style.order = String(order++); })); panel.style.display = 'flex'; panel.style.flexDirection = 'column'; logDebug('Bazaar Add groups sorted.', { groups: sortedGroups.map((group) => group.groupKey) });
    const summaryRows = built.groups.map((group) => ({ isTradable: group.parent.isTradable, sellableQuantity: group.sellableQuantity, sellableMarketValue: group.sellableMarketValue })); const summary = summarizeBazaarAddRows(summaryRows); const valuation = summary.hasUnknownValue ? (summary.knownValue === '0' ? 'Market value partially unknown' : `${formatMoney(summary.knownValue)}+ · market value partially unknown`) : formatMoney(summary.knownValue); toolbar.querySelector('[data-weav3r-bazaar-summary]').textContent = `Sellable: ${summary.positions} positions · ${summary.quantity} items · ${valuation}`; toolbar.querySelector('[data-weav3r-bazaar-sort]').value = tornState.bazaarAdd.settings.sortField; const direction = toolbar.querySelector('[data-weav3r-bazaar-direction]'); const descending = tornState.bazaarAdd.settings.sortDirection === 'desc'; direction.textContent = descending ? '↓' : '↑'; direction.setAttribute('aria-label', `Sort direction: ${descending ? 'descending' : 'ascending'}`); direction.title = direction.getAttribute('aria-label'); toolbar.querySelector('[data-weav3r-bazaar-hide]').checked = tornState.bazaarAdd.settings.hideProtected; toolbar.querySelector('[data-weav3r-bazaar-adjustment]').value = tornState.bazaarAdd.settings.priceAdjustment; const external = hasExternalBazaarPriceFiller(); toolbar.querySelector('[data-weav3r-bazaar-warning]').textContent = external ? 'External price filler detected – integrated filling disabled.' : ''; const bulkButton = toolbar.querySelector('[data-weav3r-bazaar-fill-selected]'); if (external) bulkButton.disabled = true; if (tornState.bazaarAdd.bulkPriceOperation) renderBazaarBulkPriceProgress(tornState.bazaarAdd.bulkPriceOperation);
  }
  ...
  function initBazaarAddManager() {
    ...
    if (!tornState.bazaarAdd.listenersAttached) { tornState.bazaarAdd.listenersAttached = true; document.addEventListener('change', (event) => { const rule = event.target.closest?.('[data-weav3r-bazaar-rule]'); if (rule) { setBazaarSellRule(rule.dataset.weav3rBazaarRule, rule.value); tornState.bazaarAdd.rows.filter((row) => row.itemId === normalizePositiveInt(rule.dataset.weav3rBazaarRule)).forEach((row) => tornState.bazaarAdd.rowFeedback.delete(row.rowKey)); scheduleBazaarAddRender(); return; } const toolbar = event.target.closest?.(`#${BAZAAR_ADD_MANAGER_ID}`); if (!toolbar) return; if (event.target.matches('[data-weav3r-bazaar-sort]')) tornState.bazaarAdd.settings.sortField = event.target.value; if (event.target.matches('[data-weav3r-bazaar-hide]')) tornState.bazaarAdd.settings.hideProtected = event.target.checked; if (event.target.matches('[data-weav3r-bazaar-adjustment]') && /^[+-]?\d+$/.test(event.target.value.trim())) tornState.bazaarAdd.settings.priceAdjustment = event.target.value.trim(); gmSet(BAZAAR_ADD_UI_SETTINGS_KEY, tornState.bazaarAdd.settings); scheduleBazaarAddRender(); }); document.addEventListener('click', (event) => { const target = event.target.closest?.('button'); if (!target) return; if (target.matches('[data-weav3r-bazaar-refresh]')) { if (tornState.bazaarRefreshes.has('bazaar-add')) return; target.disabled = true; target.textContent = 'Refreshing Bazaar…'; void refreshBazaarValuesForItemIds(tornState.bazaarAdd.rows.filter((row) => row.ownership !== 'faction-loan').map((row) => row.itemId), { surface: 'bazaar-add', onProgress: (operation) => showBazaarManagerStatus(`Refreshing Bazaar… ${operation.completed}/${operation.total}`) }).then((result) => { target.disabled = false; target.textContent = 'Refresh Bazaar'; showBazaarManagerStatus(formatBazaarRefreshSummary(result)); scheduleBazaarAddRender(); }); return; } if (target.matches('[data-weav3r-bazaar-direction]')) { tornState.bazaarAdd.settings.sortDirection = tornState.bazaarAdd.settings.sortDirection === 'desc' ? 'asc' : 'desc'; gmSet(BAZAAR_ADD_UI_SETTINGS_KEY, tornState.bazaarAdd.settings); scheduleBazaarAddRender(); return; } if (target.matches('[data-weav3r-bazaar-select-all]')) { const result = bulkPrepareBazaarAddSelection(); showBazaarManagerStatus(`${result.prepared} positions prepared · ${result.protected} protected · ${result.partial + result.unresolved} skipped`); } if (target.matches('[data-weav3r-bazaar-fill-selected]')) void handleBazaarAddBulkPriceFill(); if (target.matches('[data-weav3r-bazaar-reset]')) { resetBazaarAddSelection(); showBazaarManagerStatus('Selection cleared'); } if (target.matches('[data-weav3r-bazaar-fill]')) void handleBazaarAddFill(target.dataset.weav3rBazaarRowKey); if (target.matches('[data-weav3r-bazaar-price-choice]')) { const model = findBazaarAddModelByRowKey(target.dataset.weav3rBazaarRowKey); const price = calculateAdjustedBazaarPrice(target.dataset.weav3rBazaarPriceChoice, tornState.bazaarAdd.settings.priceAdjustment); if (model && price && writeBazaarAddPrice(model, price)) { tornState.bazaarAdd.rowFeedback.set(model.rowKey, { state: 'success', message: `Price ${formatMoney(price)} applied manually`, offers: [] }); renderBazaarAddRowTools(model); } } }); }
  ...
  function reconcileBazaarAddLifecycle() { if (isTornBazaarAddRoute()) initBazaarAddManager(); else teardownBazaarAddManager(); if (isTornBazaarManageRoute()) { logDebug('Manage route detected'); initBazaarManage(); } else { if (tornState.bazaarManage.root) logDebug('Manage route left'); teardownBazaarManage(); } }
  function initBazaarAddLifecycle() {
    ...
    if (!tornState.bazaarAdd.lifecycleObserver && document.documentElement) { tornState.bazaarAdd.lifecycleObserver = new MutationObserver(() => { const onAddRoute = isTornBazaarAddRoute(); const onManageRoute = isTornBazaarManageRoute(); const managerPresent = Boolean(document.getElementById(BAZAAR_ADD_MANAGER_ID)); const managePanel = findBazaarManagePanel(); if ((onAddRoute && (!managerPresent || !tornState.bazaarAdd.root)) || (!onAddRoute && (managerPresent || tornState.bazaarAdd.root)) || (onManageRoute && (!tornState.bazaarManage.root || managePanel !== tornState.bazaarManage.root)) || (!onManageRoute && tornState.bazaarManage.root)) reconcileBazaarAddLifecycle(); }); tornState.bazaarAdd.lifecycleObserver.observe(document.documentElement, { childList: true, subtree: true }); }
    reconcileBazaarAddLifecycle();
  }
  function isTornBazaarManageRoute(pathname = location.pathname, hash = location.hash) { return /\/bazaar\.php$/i.test(String(pathname || '')) && /^#\/?manage(?:[/?&]|$)/i.test(String(hash || '').trim()); }
  function findBazaarManagePanel(root = document) { return Array.from(root.querySelectorAll?.('[data-testid="panel"]') || []).find((panel) => normalizeHeaderText(panel.querySelector?.('[data-testid="panel-header"]')?.textContent) === 'manage your bazaar' && panel.querySelector?.('[data-testid="virtualized-list"]')) || null; }
  function ensureSellingDecisionStyles() { if (document.getElementById(SELLING_DECISION_STYLE_ID)) return; const style = GM_addStyle(`.wah-selling-decision,.wah-bazaar-manage-controls{display:flex;align-items:center;flex-wrap:wrap;gap:4px 10px;margin:4px 0;padding:5px 7px;border:1px solid rgba(100,116,139,.45);border-radius:6px;background:rgba(15,23,42,.35);color:#cbd5e1;font:600 10px system-ui,sans-serif}.wah-selling-decision__primary{color:#fde68a;font-weight:800}.wah-selling-decision__primary[data-wah-confidence="stored"]{color:#cbd5e1;font-weight:650}.wah-manage-pricing--at-market{color:#a7f3d0}.wah-manage-pricing--above{color:#fcd34d}.wah-manage-pricing--below{color:#93c5fd}.wah-selling-decision__markets,.wah-selling-decision__meta,.wah-selling-decision__actions{display:flex;align-items:center;flex-wrap:wrap;gap:4px 8px}.wah-selling-decision__markets{color:#99f6e4}.wah-selling-decision__meta{color:#94a3b8}.wah-selling-decision a,.wah-selling-decision button,.wah-bazaar-manage-controls button{border:1px solid #475569;border-radius:4px;background:#1e293b;color:#bfdbfe;padding:2px 6px;font:700 10px system-ui,sans-serif;text-decoration:none;cursor:pointer}.wah-selling-decision :focus-visible,.wah-bazaar-manage-controls :focus-visible{outline:2px solid #60a5fa;outline-offset:1px}`); if (style) style.id = SELLING_DECISION_STYLE_ID; }
  function buildSellingDecisionContext(now = Date.now()) { return { now, adjustment: normalizeBazaarAddSettings(gmGet(BAZAAR_ADD_UI_SETTINGS_KEY, null)).priceAdjustment, aggregates: aggregateItemBasisEvents(listTransactionEvents()), traderQuotes: indexLatestTraderQuotes(readTraderQuotes()), bazaarQuotes: indexBazaarQuotes(readBazaarQuotes()), personalRatings: readTraderPersonalRatings().records }; }
  function resolveSellingDecisionSnapshot(itemId, quantity, context) {
    const cached = getBazaarAddCachedOffers(itemId); const data = cached.fresh ? loadCachedItemData(itemId).bazaar : null; const bazaarResult = cached.fresh ? resolveBazaarRecommendedPrice(itemId, { offers: cached.offers, adjustment: context.adjustment, source: data?.apiSource, fetchedAt: data?.capturedAt }) : { status: 'unavailable', itemId, reason: 'not-current' }; const storedBazaar = resolveStoredBazaarSortValuation(context.bazaarQuotes.get(itemId), quantity, context.adjustment, context.now); const traderResult = resolveInventoryTraderQuote(itemId, context.traderQuotes, state.settings.minimumTraderRating, context.now, context.personalRatings); const bazaarValuation = buildInventoryBazaarValuation(bazaarResult, quantity); const traderValuation = buildInventoryTraderValuation(traderResult, quantity); const bestSale = resolveInventoryBestSale(bazaarResult, traderResult, quantity); const storedBest = resolveStoredBestSaleCandidate(storedBazaar, bestSale, quantity); const basis = buildItemBasisSummary(itemId, quantity, context.aggregates); const profit = resolveInventoryPotentialProfit({ basisSummary: basis, bestSaleResult: bestSale, quantity }); return { bazaarResult, storedBazaar, storedBest, traderResult, bazaarValuation, traderValuation, bestSale, basis, profit, delta: resolveVerifiedMarketDelta(bestSale, bazaarValuation, traderValuation) };
  }
  function formatSellingDecisionHtml(snapshot, quantity, itemId, itemName, currentPrice = null) {
    const best = formatBestSalePrimary(snapshot.bestSale, snapshot.storedBest, snapshot.delta); const bazaar = snapshot.bazaarValuation.status === 'available' ? `Bazaar ${formatLedgerMoney(snapshot.bazaarValuation.recommendedUnitPrice)} · current` : snapshot.storedBazaar.status === 'sort-available' && snapshot.storedBest?.source !== 'bazaar' ? `Stored Bazaar ${formatLedgerMoney(snapshot.storedBazaar.recommendedUnitPrice)} · ${formatStoredQuoteAge(snapshot.storedBazaar.lastSeenAt, Date.now())}` : ''; const trader = snapshot.traderValuation.unitPrice && snapshot.storedBest?.source !== 'trader' ? `${snapshot.traderValuation.status === 'eligible' ? 'Trader' : 'Stored Trader'} ${formatLedgerMoney(snapshot.traderValuation.unitPrice)} · ${escapeHtml(snapshot.traderValuation.quote.traderName)}` : ''; const basis = snapshot.basis?.reconstructionStatus === 'divergent' ? 'Basis —' : snapshot.basis && snapshot.basis.reconstructionStatus !== 'untracked' ? `Basis ${snapshot.basis.weightedConfirmedPurchasePrice?.display || '—'} each` : ''; const profit = ['full', 'partial'].includes(snapshot.profit.status) ? `Profit ${formatBigIntRatioMoney(snapshot.profit.potentialProfit, true)} · ${formatInventoryRoi(snapshot.profit.roi)}` : snapshot.profit.reason !== 'no-confirmed-basis' && basis ? 'Profit —' : ''; const links = `<a href="${escapeAttribute(buildWeav3rItemViewUrl(itemId, 'bazaar'))}" target="_blank" rel="noopener noreferrer">Bazaar</a><a href="${escapeAttribute(buildWeav3rItemViewUrl(itemId, 'trade'))}" target="_blank" rel="noopener noreferrer">Trade</a><button type="button" data-action="open-item-history" data-wah-item-history="${itemId}" data-wah-item-name="${escapeAttribute(itemName)}">History</button>`; return `${best ? `<div class="wah-selling-decision__primary" data-wah-confidence="${snapshot.bestSale.status === 'available' ? 'verified' : 'stored'}">${escapeHtml(best)}</div>` : ''}<div class="wah-selling-decision__markets">${currentPrice ? `<span>Current ${formatLedgerMoney(currentPrice)}</span>` : ''}${bazaar ? `<span>${escapeHtml(bazaar)}</span>` : ''}${trader ? `<span>${trader}</span>` : ''}</div><div class="wah-selling-decision__meta">${basis ? `<span>${escapeHtml(basis)}</span>` : ''}${profit ? `<span>${escapeHtml(profit)}</span>` : ''}</div><div class="wah-selling-decision__actions">${links}</div>`;
  }
  function parseBazaarManageRow(sortable, originalIndex = 0) {
    const item = sortable?.querySelector?.('[data-testid^="item-"]'); const image = item?.querySelector?.('[data-testid="img-container"] img'); const itemId = extractTornItemIdFromImageSource(image?.getAttribute?.('src')); const imageName = normalizeBoundedText(image?.getAttribute?.('alt'), 150); const ariaName = normalizeBoundedText(item?.getAttribute?.('aria-label'), 150); const heading = item?.querySelector?.('[role="heading"][aria-level="6"]'); const boldName = normalizeBoundedText(heading?.querySelector?.('b')?.textContent, 150); const itemName = boldName || ariaName || imageName; if (!itemId || !itemName || [imageName, ariaName, boldName].filter(Boolean).some((name) => normalizedItemText(name) !== normalizedItemText(itemName))) return null; const headingText = normalizeBazaarAddName(heading?.textContent); const quantityMatch = headingText.match(/\bx\s*([\d,]+)\s*$/i); const quantity = quantityMatch ? normalizePositiveInt(quantityMatch[1].replace(/,/g, '')) : null; const removeInput = Array.from(item.querySelectorAll?.('input[aria-label^="Remove:"][data-testid="number-input"]') || []).find((input) => normalizedItemText(input.getAttribute('aria-label')?.replace(/^Remove:\s*/i, '')) === normalizedItemText(itemName)); const visiblePriceInput = item.querySelector?.('input[data-testid="legacy-money-input"][data-input-money-visible="true"]'); const hiddenPriceInput = Array.from(item.querySelectorAll?.('input[data-testid="legacy-money-input"][type="hidden"]') || []).find((input) => normalizeMoneyString(input.value)); const currentPrice = normalizeBazaarAddPriceControlValue(visiblePriceInput?.value || hiddenPriceInput?.value); const semanticItemKey = normalizeBoundedText(item.getAttribute?.('data-testid'), 100); const listingKey = semanticItemKey && /^item-/i.test(semanticItemKey) ? semanticItemKey : `item-${itemId}:${normalizedItemText(itemName)}`; return { sortable, element: item, listingKey, itemId, itemName, quantity, currentPrice, removeInput, priceInput: visiblePriceInput, originalIndex };
  }
  function collectBazaarManageRows(root = document) { return Array.from(root.querySelectorAll?.('[data-testid="virtualized-list"] [data-testid="sortable-item"]') || []).map(parseBazaarManageRow).filter(Boolean); }
  function getBazaarManageExpectedCount(panel) { const list = panel?.querySelector?.('[data-testid="virtualized-list"]'); const value = list?.getAttribute?.('aria-rowcount'); return /^\d+$/.test(String(value || '')) && Number(value) > 0 ? Number(value) : null; }
  function reconcileBazaarManageRegistry(registry, mountedModels, expectedCount = null, now = Date.now()) {
    for (const [key, record] of registry) registry.set(key, Object.freeze({ ...record, isMounted: false }));
    for (const model of mountedModels || []) { const key = normalizeBoundedText(model.listingKey, 160); if (!key || !normalizePositiveInt(model.itemId) || !normalizePositiveInt(model.quantity) || !normalizeMoneyString(model.currentPrice)) continue; const previous = registry.get(key); registry.set(key, Object.freeze({ listingKey: key, itemId: model.itemId, itemName: model.itemName, listedQuantity: model.quantity, lastObservedCurrentPrice: model.currentPrice, observedAt: previous?.observedAt || now, lastSeenAt: now, isMounted: true, parseConfidence: 'semantic-dom', pricingStatus: previous?.pricingStatus || { status: 'unavailable', delta: null, text: '' }, recommendedPrice: previous?.recommendedPrice || null, recommendationFreshness: previous?.recommendationFreshness || 'unavailable' })); }
    const observedCount = registry.size; const mountedCount = Array.from(registry.values()).filter((record) => record.isMounted).length; const expected = normalizePositiveInt(expectedCount); const status = expected ? observedCount === expected ? 'complete' : observedCount < expected ? 'partial' : 'unknown' : 'unknown'; return { status, observedCount, mountedCount, expectedCount: expected || null };
  }
  function writeBazaarManageField(input, value) { return Boolean(input?.isConnected !== false && !input.disabled && !input.readOnly && setControlledInputValue(input, String(value), { change: true })); }
  async function fillBazaarManagePrice(model) { if (!model?.priceInput?.isConnected) return { ok: false, reason: 'stale-row' }; const result = await loadBazaarRecommendedPrice(model.itemId, { adjustment: normalizeBazaarAddSettings(gmGet(BAZAAR_ADD_UI_SETTINGS_KEY, null)).priceAdjustment }); if (result.status !== 'available' || !result.recommendedUnitPrice) return { ok: false, reason: result.reason || 'unavailable' }; return { ok: writeBazaarManageField(model.priceInput, result.recommendedUnitPrice), price: result.recommendedUnitPrice, reason: null }; }
  function prepareBazaarManageRemoveAll(models) { const result = { rows: 0, quantity: 0, skipped: 0 }; for (const model of models || []) { if (!model.quantity || !model.removeInput?.isConnected || !writeBazaarManageField(model.removeInput, model.quantity)) { result.skipped += 1; continue; } result.rows += 1; result.quantity += model.quantity; } return result; }
  function resolveBazaarManagePricingStatus(currentPrice, recommendedPrice) { const current = normalizeMoneyString(currentPrice); const recommended = normalizeMoneyString(recommendedPrice); if (!current || !recommended) return { status: 'unavailable', delta: null, text: '' }; const delta = BigInt(current) - BigInt(recommended); return delta === 0n ? { status: 'at-market', delta: '0', text: 'At market' } : delta > 0n ? { status: 'above', delta: delta.toString(), text: `Above market by ${formatLedgerMoney(delta)}` } : { status: 'below', delta: delta.toString(), text: `Below market by ${formatLedgerMoney(-delta)}` }; }
  function summarizeBazaarManagePricing(models) { const summary = { total: 0, atMarket: 0, above: 0, below: 0, unavailable: 0 }; for (const model of models || []) { summary.total += 1; const status = model.pricingStatus?.status || 'unavailable'; if (status === 'at-market') summary.atMarket += 1; else if (status === 'above') summary.above += 1; else if (status === 'below') summary.below += 1; else summary.unavailable += 1; } return summary; }
  function updateBazaarManageRegistryPricing(registry, context) { for (const [key, record] of registry) { const snapshot = resolveSellingDecisionSnapshot(record.itemId, record.listedQuantity, context); const recommendation = snapshot.bazaarValuation.status === 'available' ? snapshot.bazaarValuation.recommendedUnitPrice : snapshot.storedBazaar.status === 'sort-available' ? snapshot.storedBazaar.recommendedUnitPrice : null; registry.set(key, Object.freeze({ ...record, recommendedPrice: recommendation, recommendationFreshness: recommendation ? snapshot.bazaarValuation.status === 'available' ? 'current' : 'stored' : 'unavailable', pricingStatus: resolveBazaarManagePricingStatus(record.lastObservedCurrentPrice, recommendation) })); } return registry; }
  function summarizeBazaarManageRegistry(records, coverage) { const values = Array.from(records || []); const pricing = summarizeBazaarManagePricing(values); return { observedCount: values.length, mountedCount: values.filter((record) => record.isMounted).length, expectedCount: coverage?.expectedCount || null, coverageStatus: coverage?.status || 'unknown', atMarketCount: pricing.atMarket, aboveMarketCount: pricing.above, belowMarketCount: pricing.below, unavailableCount: pricing.unavailable }; }
  function formatBazaarManageCoverage(summary) { const scope = summary.coverageStatus === 'complete' && summary.expectedCount ? `All ${summary.expectedCount} listings observed` : summary.expectedCount ? `Observed ${summary.observedCount} / ${summary.expectedCount} listings` : `Observed ${summary.observedCount} listings · ${summary.mountedCount} currently visible`; return `${scope} · ${summary.aboveMarketCount} above · ${summary.belowMarketCount} below · ${summary.atMarketCount} at market`; }
  function formatBazaarManageRefreshSummary(result, coverage) { const scope = coverage.status === 'complete' && coverage.expectedCount ? `${coverage.expectedCount} listings` : coverage.expectedCount ? `${coverage.observedCount} / ${coverage.expectedCount} observed listings` : `${coverage.observedCount} observed listings`; return `Bazaar refreshed · ${scope} · ${result.updated} updated${result.kept ? ` · ${result.kept} stored fallbacks` : ''}`; }
  function ensureBazaarManageControls(panel) { let controls = document.getElementById(BAZAAR_MANAGE_TOOLS_ID); if (controls) return controls; controls = document.createElement('section'); controls.id = BAZAAR_MANAGE_TOOLS_ID; controls.className = 'wah-bazaar-manage-controls'; controls.innerHTML = `<strong>Weav3r Bazaar Manager</strong><button type="button" data-wah-manage-refresh>Refresh Bazaar</button><button type="button" data-wah-manage-fill-prices>Fill prices</button><button type="button" data-wah-manage-remove-all>Remove all</button><span data-wah-manage-summary></span><span data-wah-manage-status aria-live="polite"></span>`; const anchor = panel.querySelector?.('[aria-label="Confirmation section"]') || panel.querySelector?.('[data-testid="panel-header"]'); anchor?.appendChild?.(controls); return controls; }
  function renderBazaarManage() {
    if (!isTornBazaarManageRoute()) return teardownBazaarManage(); const panel = findBazaarManagePanel(); if (!panel) { logDebug('Manage panel waiting'); return; }
    ensureSellingDecisionStyles(); const controls = ensureBazaarManageControls(panel); const context = buildSellingDecisionContext(); const models = collectBazaarManageRows(panel); const expectedCount = getBazaarManageExpectedCount(panel); tornState.bazaarManage.coverage = reconcileBazaarManageRegistry(tornState.bazaarManage.registry, models, expectedCount); updateBazaarManageRegistryPricing(tornState.bazaarManage.registry, context);
    for (const model of models) { const record = tornState.bazaarManage.registry.get(model.listingKey); if (!record) continue; let host = model.element.querySelector('[data-wah-bazaar-manage-row]'); if (!host) { host = document.createElement('div'); host.className = 'wah-selling-decision wah-bazaar-manage-row'; model.element.appendChild(host); } if (host.dataset.wahBazaarManageListing !== model.listingKey) { host.dataset.wahBazaarManageListing = model.listingKey; host.dataset.wahBazaarManageRow = String(model.itemId); host.textContent = ''; } const snapshot = resolveSellingDecisionSnapshot(model.itemId, model.quantity, context); const recommendationLabel = record.recommendedPrice ? `<span>Recommended ${formatLedgerMoney(record.recommendedPrice)} · ${record.recommendationFreshness}</span>` : ''; const pricingLabel = record.pricingStatus.text ? `<span class="wah-manage-pricing wah-manage-pricing--${record.pricingStatus.status}">${record.pricingStatus.text}</span>` : ''; host.innerHTML = formatSellingDecisionHtml(snapshot, model.quantity, model.itemId, model.itemName, model.currentPrice) + recommendationLabel + pricingLabel + `<button type="button" data-wah-manage-fill="${escapeAttribute(model.listingKey)}">Fill</button>`; }
    const summary = summarizeBazaarManageRegistry(tornState.bazaarManage.registry.values(), tornState.bazaarManage.coverage); const summaryNode = controls?.querySelector?.('[data-wah-manage-summary]'); if (summaryNode) summaryNode.textContent = formatBazaarManageCoverage(summary); tornState.bazaarManage.models = models; return { controls, models, summary, coverage: tornState.bazaarManage.coverage };
  }

  function scheduleBazaarManageRender() { clearTimeout(tornState.bazaarManage.timer); tornState.bazaarManage.timer = setTimeout(renderBazaarManage, 120); }
  function teardownBazaarManage() { clearTimeout(tornState.bazaarManage.timer); tornState.bazaarManage.observer?.disconnect(); tornState.bazaarManage.observer = null; tornState.bazaarManage.root = null; tornState.bazaarManage.routeKey = ''; tornState.bazaarManage.models = []; tornState.bazaarManage.registry.clear(); tornState.bazaarManage.coverage = { status: 'unknown', observedCount: 0, mountedCount: 0, expectedCount: null }; document.getElementById(BAZAAR_MANAGE_TOOLS_ID)?.remove(); document.querySelectorAll('[data-wah-bazaar-manage-row]').forEach((node) => node.remove()); }
  function initBazaarManage() { if (!isTornBazaarManageRoute()) return teardownBazaarManage(); const root = findBazaarManagePanel(); if (!root) { logDebug('Manage panel waiting'); return; } const routeKey = `${location.pathname}${location.hash.split('?')[0]}`; if (tornState.bazaarManage.routeKey && tornState.bazaarManage.routeKey !== routeKey) { tornState.bazaarManage.registry.clear(); tornState.bazaarManage.coverage = { status: 'unknown', observedCount: 0, mountedCount: 0, expectedCount: null }; } tornState.bazaarManage.routeKey = routeKey; if (tornState.bazaarManage.root !== root) { const replaced = Boolean(tornState.bazaarManage.root); tornState.bazaarManage.observer?.disconnect(); tornState.bazaarManage.root = root; logDebug(replaced ? 'Manage panel replaced' : 'Manage panel mounted'); tornState.bazaarManage.observer = new MutationObserver((mutations) => { if (mutations.some((mutation) => !mutation.target.closest?.('[data-wah-bazaar-manage-row],#weav3r-bazaar-manage-tools'))) scheduleBazaarManageRender(); }); tornState.bazaarManage.observer.observe(root, { childList: true, subtree: true }); } if (!tornState.bazaarManage.listenersAttached) { tornState.bazaarManage.listenersAttached = true; document.addEventListener('input', (event) => { if (event.target?.matches?.('input[data-testid="legacy-money-input"][data-input-money-visible="true"]') && event.target.closest?.('[data-testid^="item-"]')) scheduleBazaarManageRender(); }); document.addEventListener('click', async (event) => { const fill = event.target.closest?.('[data-wah-manage-fill]'); const bulk = event.target.closest?.('[data-wah-manage-fill-prices]'); const remove = event.target.closest?.('[data-wah-manage-remove-all]'); const refresh = event.target.closest?.('[data-wah-manage-refresh]'); if (!fill && !bulk && !remove && !refresh) return; const status = document.querySelector('#weav3r-bazaar-manage-tools [data-wah-manage-status]'); if (refresh) { if (tornState.bazaarRefreshes.has('manage')) return; refresh.disabled = true; refresh.textContent = 'Refreshing Bazaar…'; const result = await refreshBazaarValuesForItemIds(Array.from(tornState.bazaarManage.registry.values(), (entry) => entry.itemId), { surface: 'manage', onProgress: (operation) => { if (status) status.textContent = `Refreshing Bazaar… ${operation.completed}/${operation.total}`; } }); refresh.disabled = false; refresh.textContent = 'Refresh Bazaar'; if (status) status.textContent = formatBazaarManageRefreshSummary(result, tornState.bazaarManage.coverage); scheduleBazaarManageRender(); } else if (fill) { const model = tornState.bazaarManage.models.find((entry) => entry.listingKey === fill.dataset.wahManageFill); const result = await fillBazaarManagePrice(model); if (status) status.textContent = result.ok ? `Price ${formatLedgerMoney(result.price)} prepared` : 'Safe Bazaar price unavailable'; } else if (bulk) { const results = await Promise.all(tornState.bazaarManage.models.map(fillBazaarManagePrice)); if (status) status.textContent = `${results.filter((result) => result.ok).length} mounted listings prepared · Review and click SAVE CHANGES manually`; } else if (remove) { const result = prepareBazaarManageRemoveAll(tornState.bazaarManage.models); if (status) status.textContent = `${result.rows} visible listings · ${result.quantity} items prepared for removal${result.skipped ? ` · ${result.skipped} skipped` : ''}`; } }); } logDebug('Manage renderer initialized'); scheduleBazaarManageRender(); }
  ...
  function parseTradeRowIdentity(row) { const itemIds = getInventoryRowItemIds(row); return { itemIds, itemName: getInventoryRowName(row) }; }
  function resolveTradeRowActionability(row) { const control = getTradeRowQuantityControl(row); if (control) return { status: 'available', control }; const text = normalizedItemText(row?.textContent); return { status: text.includes('untradable') ? 'untradable' : text.includes('unavailable') ? 'unavailable' : 'unknown', control: null }; }
  function rowHasTradeItemId(row, itemId) { return parseTradeRowIdentity(row).itemIds.includes(normalizePositiveInt(itemId)); }
  function getVerifiedTargetRows(handoff) {
    const expectedName = normalizedItemText(handoff.itemName); const targetItemId = normalizePositiveInt(handoff.itemId);
    return getInventoryRows().filter((row) => {
      const identity = parseTradeRowIdentity(row); if (!identity.itemIds.includes(targetItemId)) return false;
      const rowName = identity.itemName;
  ...
  function renderTradeFilterPanel(handoff, count, mode, notFound = false, searching = false, incomplete = false) {
    ...
    const quantityControls = !searching && !notFound && !incomplete ? getVerifiedTargetRows(handoff).map(getTradeRowQuantityControl).filter(Boolean) : [];
    const available = quantityControls.reduce((sum, entry) => sum + entry.available, 0);
    text.textContent = searching ? (count ? `${handoff.itemName} found · loading remaining inventory…` : `Loading full inventory… ${handoff.itemName}`) : incomplete ? (count ? `${handoff.itemName} found — full inventory loading incomplete.` : `Inventory loading incomplete — ${handoff.itemName} could not be verified.`) : notFound ? `${handoff.itemName} was not found in the fully loaded trade inventory.` : `${formatTargetFilterLabel(handoff.itemName, count)}${available ? ` · Available: ${available}` : ''}`;
    ...
    if (!searching && !incomplete) controls.append(button);
  ...
  function findTradeInventoryScrollRoot(inventory) {
    for (let element = inventory; element && element !== document.body; element = element.parentElement) {
      ...
      if (!/(auto|scroll)/.test(style.overflowY) || element.scrollHeight <= element.clientHeight + 8) continue;
      const original = element.scrollTop; element.scrollTop = Math.min(original + 1, element.scrollHeight - element.clientHeight); const moved = element.scrollTop !== original; element.scrollTop = original;
      if (moved) return element;
    }
    return null;
  }
  function getTradeDiscoverySnapshot(inventory, scrollRoot) { const rows = collectTradeInventoryRows(inventory); const lastImage = rows.at(-1)?.querySelector?.('img[src*="/images/items/"]')?.getAttribute?.('src') || ''; const listRect = inventory.getBoundingClientRect?.() || { bottom: 0 }; const height = scrollRoot ? scrollRoot.scrollHeight : document.documentElement.scrollHeight; const position = scrollRoot ? scrollRoot.scrollTop : window.scrollY; const viewport = scrollRoot ? scrollRoot.clientHeight : window.innerHeight; const end = scrollRoot ? position + viewport >= height - 2 : Math.max(position + viewport, viewport) >= Math.max(height, position + listRect.bottom) - 2; return { rows, signature: `${rows.length}:${lastImage}:${height}:${Math.round(listRect.bottom)}`, end, height, position, viewport }; }
  ...
  async function findTradeTargetWithProgressiveScroll({ inventory, targetItemId, signal, isContextCurrent = () => true, onProgress = null }) {
    if (!inventory || !normalizePositiveInt(targetItemId)) return { status: 'incomplete', targets: [], reason: 'invalid-input' };
    ...
    let unchangedAtEnd = 0; let previousSignature = ''; let targetFound = false; let lastProgrammaticPosition = initialScrollTop;
    try {
      for (let step = 0; step < TRADE_TARGET_DISCOVERY_MAX_STEPS && Date.now() < deadline && !signal?.aborted; step += 1) {
        if (!isContextCurrent() || inventory.isConnected === false) return { status: 'aborted', targets: [], targetFound, reason: 'stale-context' };
        const snapshot = getTradeDiscoverySnapshot(inventory, scrollRoot); const targets = snapshot.rows.filter((row) => rowHasTradeItemId(row, targetItemId));
        if (targets.length && !targetFound) { targetFound = true; onProgress?.({ status: 'target-found-loading-rest', targets }); }
        unchangedAtEnd = snapshot.end && snapshot.signature === previousSignature ? unchangedAtEnd + 1 : 0;
        if (unchangedAtEnd >= 3) return { status: 'complete', targets, targetFound, rowCount: snapshot.rows.length };
        previousSignature = snapshot.signature;
        const next = Math.min(snapshot.height, snapshot.position + Math.max(240, Math.floor(snapshot.viewport * 0.8))); lastProgrammaticPosition = next;
        if (scrollRoot) scrollRoot.scrollTo({ top: next, behavior: 'auto' }); else window.scrollTo({ top: next, behavior: 'auto' });
        const grew = await waitForInventoryGrowth(inventory, signal); if (grew) { unchangedAtEnd = 0; previousSignature = ''; }
      }
      const targets = collectTradeInventoryRows(inventory).filter((row) => rowHasTradeItemId(row, targetItemId));
      return { status: signal?.aborted ? 'aborted' : 'incomplete', targets, targetFound: targetFound || targets.length > 0, reason: signal?.aborted ? 'aborted' : 'limit-reached' };
    } finally {
      if (isContextCurrent()) {
        const current = scrollRoot ? scrollRoot.scrollTop : window.scrollY; if (Math.abs(current - lastProgrammaticPosition) <= 8) { if (scrollRoot) scrollRoot.scrollTo({ top: initialScrollTop, behavior: 'auto' }); else window.scrollTo({ top: initialScrollTop, behavior: 'auto' }); logDebug('inventory scroll restored after full inventory discovery.'); }
      }
    }
  }
  ...
  function processAddStep(handoff, route) {
    waitForTornElement(isSupportedAddInventoryReady, async (inventoryReady) => {
      ...
      const discoveryRoot = getInventoryContainer(); collectTradeInventoryRows(discoveryRoot).forEach((row) => row.classList.remove(TRADE_FILTER_HIDDEN_CLASS, TRADE_FILTER_TARGET_CLASS));
  ...
  function buildWeav3rItemViewUrl(itemId, view, origin = 'https://weav3r.dev') {
    const id = normalizePositiveInt(itemId); const route = view === 'trade' ? ['sell', 'itemmarket'] : view === 'item-market' ? ['buy', 'itemmarket'] : view === 'bazaar' ? ['buy', 'all'] : null;
    if (!id || !route) return ''; const url = new URL(`/item/${id}`, origin); url.searchParams.set('mode', route[0]); url.searchParams.set('tab', route[1]); url.searchParams.set('timeframe', '7d'); return url.href;
  }
  function getSourceDefinitions(itemId) {
    return [
      { itemId: String(itemId), sourceType: 'bazaar', parsedKey: 'offers', url: buildWeav3rItemViewUrl(itemId, 'bazaar'), method: 'api' },
      { itemId: String(itemId), sourceType: 'itemMarket', parsedKey: 'offers', url: buildWeav3rItemViewUrl(itemId, 'item-market'), method: 'iframe' },
      { itemId: String(itemId), sourceType: 'traders', parsedKey: 'traders', url: buildWeav3rItemViewUrl(itemId, 'trade'), method: 'iframe' },
    ];
  }
  function getItemUrl(itemId) { return buildWeav3rItemViewUrl(itemId, 'bazaar'); }
  ...
  function isCurrentRoute(itemId, generation) { return state.itemId === String(itemId) && state.routeGeneration === generation && getCurrentItemId() === String(itemId); }
  ...
  function formatBazaarRefreshSummary(result, scope = '') { const prefix = scope ? `Bazaar refreshed · ${scope}` : 'Bazaar refreshed'; return `${prefix} · ${result.checked} checked · ${result.updated} updated${result.kept ? ` · ${result.kept} stored fallbacks` : ''}`; }
  function refreshBazaarValuesForItemIds(itemIds, options) {
    options = options || {};
    const surface = normalizeBoundedText(options.surface, 40) || 'shared'; const existing = tornState.bazaarRefreshes.get(surface); if (existing) return existing.promise;
    const ids = [...new Set(Array.from(itemIds || [], normalizePositiveInt).filter(Boolean))]; const operation = { surface, total: ids.length, completed: 0, checked: 0, updated: 0, kept: 0, promise: null }; const adjustment = options.adjustment ?? normalizeBazaarAddSettings(gmGet(BAZAAR_ADD_UI_SETTINGS_KEY, null)).priceAdjustment;
    operation.promise = Promise.all(ids.map(async (itemId) => { try { const entry = await queueBazaarAddMarketplaceDetail(itemId, options.transport || requestBazaarAddMarketplaceDetail); const result = resolveBazaarRecommendedPrice(itemId, { adjustment, offers: entry.offers, source: entry.apiSource, fetchedAt: entry.capturedAt }); if (result.status === 'available' && normalizeMoneyString(result.recommendedUnitPrice)) { operation.updated += 1; options.onResult?.(itemId, result); } else operation.kept += 1; } catch (_) { operation.kept += 1; } finally { operation.checked += 1; operation.completed += 1; options.onProgress?.(operation); } })).then(() => ({ checked: operation.checked, updated: operation.updated, kept: operation.kept, itemIds: ids })).finally(() => tornState.bazaarRefreshes.delete(surface)); tornState.bazaarRefreshes.set(surface, operation); options.onProgress?.(operation); return operation.promise;
  }
  ...
  function collectMissingSources(itemId, options = {}) {
    ...
    collectBazaarWithApi(itemId, generation, Boolean(options.forceBazaar)).finally(() => { if (isCurrentRoute(itemId, generation)) { fallbackBazaarIfNeeded(itemId, generation); renderComputedResult(true, itemId, generation); } });
  ...
  function pumpCollectorQueue() {
    ...
    const promise = collectSourceWithIframe(job.definition, job.generation, job).then((entry) => handleCollectorSuccess(job, entry)).catch((error) => handleCollectorFailure(job, error)).finally(() => { state.activeCollectors.delete(job.key); pumpCollectorQueue(); if (job.context === 'current' && isCurrentRoute(job.definition.itemId, job.generation)) renderComputedResult(true, job.definition.itemId, job.generation); updateScanProgress(); renderWatchlistPanel(); });
  ...
  function collectSourceWithIframe(sourceDefinition, generation, job) {
    ...
    return new Promise((resolve, reject) => {
      ...
      iframe.addEventListener('load', () => { waitForIframeDocument(iframe).then((iframeDocument) => waitForSourceTable(iframeDocument, sourceDefinition)).then((iframeDocument) => { if (!isCollectorJobStillValid(job)) throw new Error('Collector context changed before parsing.'); const parsed = parseSourceDocument(iframeDocument, sourceDefinition.sourceType, sourceDefinition.itemId, sourceDefinition.url); if (!parsed.data.length) throw new Error('Expected table was present but no valid rows were parsed.'); resolve(saveCachedSourceData(sourceDefinition.itemId, sourceDefinition.sourceType, parsed.parsedKey, parsed.data, sourceDefinition.url, sourceDefinition.sourceType === 'traders' ? { captureContext: 'isolated-iframe' } : {})); }).catch(reject); }, { once: true });
  ...
  function createRouteLoadingResult(itemId, generation, message) { return { status: 'LOADING', messages: [message], itemId: normalizePositiveInt(itemId), routeGeneration: generation, routeBinding: 'item' }; }
  function invalidateCurrentItemUi(itemId, generation, message = 'Loading current item data...') {
    const loading = createRouteLoadingResult(itemId, generation, message); state.lastResult = loading; highlightRows({}); renderStatusCard(loading); return loading;
  }
  function isCurrentRouteRender(itemId, generation) { return isCurrentRoute(itemId, generation); }
  function hasItemBoundCachedData(itemId, data = state.cachedData) { const id = normalizePositiveInt(itemId); return Boolean(id && Object.values(data || {}).some((entry) => normalizePositiveInt(entry?.itemId) === id)); }
  function activateCurrentItemRoute(itemId, message) {
    const fromItemId = state.itemId; state.routeGeneration += 1; state.itemId = itemId; state.routeTransition = { startedAt: Date.now(), itemId: normalizePositiveInt(itemId), generation: state.routeGeneration, visibleDomBlocked: true }; state.sourceProgress = emptySourceProgress(); state.cachedData = emptyCachedData(); logDebug('route-transition-start', { fromItemId, toItemId: itemId, generation: state.routeGeneration }); invalidateCurrentItemUi(itemId, state.routeGeneration, message); logDebug('route-transition-ui-invalidated', { itemId, generation: state.routeGeneration }); beginWeav3rItemRouteTransition(); state.cachedData = loadCachedItemData(itemId); if (hasItemBoundCachedData(itemId)) { updateSourceProgressFromCache(); renderComputedResult(true, itemId, state.routeGeneration); } return state.routeGeneration;
  }
  function parseCurrentVisiblePage(itemId, generation = state.routeGeneration) { if (!isCurrentItemView(itemId, generation)) return { ...state.cachedData }; const bazaarOffers = parseBazaarOffers(itemId, document, location.href); const itemMarketOffers = parseItemMarketOffers(document, itemId, location.href); const traders = parseTraderOffers(document, location.href); if (bazaarOffers.length) state.cachedData.bazaar = saveCachedSourceData(itemId, 'bazaar', 'offers', bazaarOffers, location.href, { fallbackSource: 'visibleDom' }); if (itemMarketOffers.length) state.cachedData.itemMarket = saveCachedSourceData(itemId, 'itemMarket', 'offers', itemMarketOffers, location.href); if (traders.length && isCurrentItemView(itemId, generation)) state.cachedData.traders = saveCachedSourceData(itemId, 'traders', 'traders', traders, location.href, { captureContext: 'route-bound-visible' }); filterTraderRows(traders, state.settings.minimumTraderRating); const liveAwareData = { ...state.cachedData }; if (bazaarOffers.length) liveAwareData.bazaar = { ...state.cachedData.bazaar, offers: bazaarOffers }; if (itemMarketOffers.length) liveAwareData.itemMarket = { ...state.cachedData.itemMarket, offers: itemMarketOffers }; if (traders.length) liveAwareData.traders = { ...state.cachedData.traders, traders }; return liveAwareData; }
  function renderComputedResult(useCachedOnly = false, expectedItemId = state.itemId, expectedGeneration = state.routeGeneration) { if (!isCurrentRouteRender(expectedItemId, expectedGeneration)) return null; scheduleEligibilityExpiryTimer(); const failedSources = Object.entries(state.sourceProgress).filter(([, value]) => value === 'failed').map(([source]) => source); const result = calculateArbitrage(state.cachedData, state.settings, { checking: hasActiveCollectorsForCurrentItem() || state.sourceProgress.bazaar === 'checking', progressMessages: sourceProgressMessages(), failedSources }); if (!isCurrentRouteRender(expectedItemId, expectedGeneration)) return null; result.itemId = normalizePositiveInt(expectedItemId); result.routeGeneration = expectedGeneration; result.routeBinding = 'item'; state.lastResult = result; if (!useCachedOnly) highlightRows(result); syncCurrentItemToWatchlist(result); renderStatusCard(result); return result; }
  function evaluateCurrentPage(options = {}) { try { const itemId = getCurrentItemId(); if (!itemId) return; if (itemId !== state.itemId) { cancelCurrentCollectors(); clearTimeout(state.eligibilityTimer); state.eligibilityTimer = 0; activateCurrentItemRoute(itemId, 'Loading item data...'); } if (options.forceCollect) { cancelCurrentCollectors(); state.routeGeneration += 1; state.sourceProgress = emptySourceProgress(); } const generation = state.routeGeneration; const liveAwareData = parseCurrentVisiblePage(itemId, generation); if (!isCurrentItemView(itemId, generation)) { updateSourceProgressFromCache(); collectMissingSources(itemId, { forceBazaar: Boolean(options.forceCollect), forceCollectors: Boolean(options.forceCollect) }); if (hasItemBoundCachedData(itemId)) renderComputedResult(true, itemId, generation); else renderStatusCard(createRouteLoadingResult(itemId, generation, 'Waiting for securely item-bound data...')); return; } updateSourceProgressFromCache(); const result = calculateArbitrage(liveAwareData, state.settings); if (!isCurrentRouteRender(itemId, generation)) return; result.itemId = normalizePositiveInt(itemId); result.routeGeneration = generation; result.routeBinding = 'item'; state.lastResult = result; highlightRows(result); syncCurrentItemToWatchlist(result); renderStatusCard(result); collectMissingSources(itemId, { forceBazaar: Boolean(options.forceCollect), forceCollectors: Boolean(options.forceCollect) }); renderComputedResult(true, itemId, generation); logDebug('Evaluation complete.', result); } catch (error) { logError('Evaluation failed.', error); renderStatusCard({ status: 'DATA MISSING', messages: ['Could not evaluate the current page safely.'] }); } }
  ...
  function handleRouteChange() { if (location.href === state.lastUrl) return; state.lastUrl = location.href; cancelCurrentCollectors(); clearTimeout(state.recheckTimer); clearTimeout(state.manualTraderCaptureTimer); clearTimeout(state.eligibilityTimer); state.recheckTimer = 0; state.manualTraderCaptureTimer = 0; state.eligibilityTimer = 0; activateCurrentItemRoute(getCurrentItemId(), 'Route changed. Loading current item...'); scheduleVisibleTraderQuoteCapture(); scheduleEvaluation(); }
  ...
  function parseTornInventoryRow(row) {
    ...
    const isGroupParent = group === 'parent'; const isGroupChild = group === 'item' && Boolean(instanceKey); const rawQuantity = normalizePositiveInt(row.dataset?.qty); const quantity = instanceKey ? 1 : rawQuantity; if (!quantity) return null; const ownership = row.querySelector?.('[data-action="return"][data-type="armoury"]') ? 'faction-loan' : 'owned';
    return { itemId, itemName, category: normalizeBoundedText(row.dataset?.category, 80) || '', quantity, ownedQuantity: ownership === 'owned' ? quantity : 0, ownership, isFactionLoaned: ownership === 'faction-loan', rowKey, instanceKey, armoryId: instanceKey, groupKey: isGroupParent || isGroupChild ? `g${itemId}` : null, groupState: isGroupParent ? 'closed-parent' : isGroupChild ? 'open-child' : instanceKey ? 'instance' : 'stack', isGroupParent, isGroupChild, sourceRow: row };
  }
  function scanTornInventoryList(container) {
    const sourceRows = Array.from(container?.querySelectorAll?.(':scope > li[data-item][data-rowkey]') || []); const parsedRows = sourceRows.map(parseTornInventoryRow); const allRows = parsedRows.filter(Boolean); const rows = allRows.filter((row) => row.ownership === 'owned'); const factionLoans = allRows.filter((row) => row.ownership === 'faction-loan'); const groups = [];
    for (let index = 0; index < parsedRows.length;) { const row = parsedRows[index]; if (!row) { index += 1; continue; } if (row.isGroupParent) { if (row.ownership === 'owned') groups.push({ groupKey: row.groupKey, itemId: row.itemId, state: 'closed', rowKeys: [row.rowKey], totalQuantity: row.ownedQuantity }); index += 1; continue; } if (row.isGroupChild) { const physicalBlock = []; let cursor = index; while (cursor < parsedRows.length && parsedRows[cursor]?.isGroupChild && parsedRows[cursor].itemId === row.itemId && parsedRows[cursor].groupKey === row.groupKey) physicalBlock.push(parsedRows[cursor++]); const ownedBlock = physicalBlock.filter((entry) => entry.ownership === 'owned'); if (ownedBlock.length) groups.push({ groupKey: row.groupKey, itemId: row.itemId, state: 'open', rowKeys: ownedBlock.map((entry) => entry.rowKey), totalQuantity: ownedBlock.reduce((total, entry) => total + entry.ownedQuantity, 0) }); index = cursor; continue; } index += 1; }
    return { rows, allRows, factionLoans, groups, totalQuantity: rows.reduce((total, row) => total + row.ownedQuantity, 0) };
  }
  ...
  function formatInventoryRoi(ratio) { if (!ratio) return 'N/A'; const hundredths = roundBigIntRatio(BigInt(ratio.numerator) * 100n, BigInt(ratio.denominator)); const negative = hundredths < 0n; const absolute = negative ? -hundredths : hundredths; return `${negative ? '-' : hundredths > 0n ? '+' : ''}${absolute / 100n}.${(absolute % 100n).toString().padStart(2, '0')} %`; }
  ...
  function buildTornInventoryBasisBlocks(scan) {
    ...
    for (const model of scan?.rows || []) { if (model.ownership === 'faction-loan') continue; const group = groupByFirstRow.get(model.rowKey); if (group) blocks.push({ model, currentQuantity: group.totalQuantity, rowKeys: group.rowKeys.slice() }); else if (!groupedRows.has(model.rowKey)) blocks.push({ model, currentQuantity: model.ownedQuantity ?? model.quantity, rowKeys: [model.rowKey] }); }
    return blocks;
  }
  ...
  function formatStoredQuoteAge(timestamp, now = Date.now()) { const age = Math.max(0, now - Number(timestamp)); const units = age < 60 * 60 * 1000 ? [60 * 1000, 'min', 'min'] : age < 24 * 60 * 60 * 1000 ? [60 * 60 * 1000, 'h', 'h'] : age < 60 * 24 * 60 * 60 * 1000 ? [24 * 60 * 60 * 1000, 'd', 'd'] : [30 * 24 * 60 * 60 * 1000, 'mo', 'mo']; const count = Math.max(1, Math.floor(age / units[0])); return `${count}${units[1]} ago`; }
  const TORN_INVENTORY_WAH_SELECTOR = '[data-wah-item-history],[data-wah-item-basis],[data-wah-weav3r-links],[data-wah-item-bazaar],[data-wah-item-trader],[data-wah-item-best-sale],[data-wah-item-profit],[data-wah-bazaar-sell-quantity],[data-wah-inventory-sell-rule-control]';
  function ensureTornInventoryWahAreas(host) {
    if (!host) return null; const ensure = (parent, name, className) => { let node = parent.querySelector?.(`:scope > [data-wah-area="${name}"]`); if (!node) { node = document.createElement('div'); node.className = className; node.dataset.wahArea = name; parent.appendChild(node); } return node; };
    const main = ensure(host, 'main', 'wah-item-main'); const primary = ensure(main, 'primary', 'wah-item-primary'); const markets = ensure(main, 'markets', 'wah-item-market-values'); const meta = ensure(host, 'meta', 'wah-item-meta'); const sell = ensure(meta, 'sell', 'wah-item-sell-control'); const actions = ensure(host, 'actions', 'wah-item-actions');
    const move = (selector, destination) => Array.from(host.querySelectorAll?.(selector) || []).forEach((node) => { if (node !== destination && node.parentElement !== destination && !destination.contains?.(node)) destination.appendChild(node); });
    move('[data-wah-item-best-sale]', primary); move('[data-wah-item-bazaar],[data-wah-item-trader]', markets); move('[data-wah-item-basis],[data-wah-item-profit]', meta); move('[data-wah-inventory-sell-rule-control],[data-wah-bazaar-sell-quantity]', sell); move('[data-wah-weav3r-links],[data-wah-item-history]', actions); return { main, primary, markets, meta, sell, actions };
  }
  function ensureTornInventoryWahRow(blockOrModel) {
    const block = blockOrModel?.model ? blockOrModel : null; const model = block?.model || blockOrModel; const row = model?.sourceRow; if (!row) return null; if (block?.wahHost?.parentElement === row) { ensureTornInventoryWahAreas(block.wahHost); return block.wahHost; } let host = row.querySelector?.(':scope > [data-wah-item-row]'); const title = row.querySelector?.(':scope > .title-wrap') || row.querySelector?.('.title-wrap');
    if (!host) { host = document.createElement('div'); host.className = 'wah-item-row'; host.dataset.wahItemRow = String(model.itemId); const anchor = title?.nextSibling || row.querySelector?.(':scope > .cont-wrap') || null; if (row.insertBefore) row.insertBefore(host, anchor); else if (title?.insertAdjacentElement) title.insertAdjacentElement('afterend', host); else row.appendChild?.(host); }
    for (const duplicate of Array.from(row.querySelectorAll?.(':scope > [data-wah-item-row]') || [])) if (duplicate !== host) { for (const child of Array.from(duplicate.children || [])) host.appendChild(child); duplicate.remove(); }
    for (const legacy of Array.from(title?.querySelectorAll?.(TORN_INVENTORY_WAH_SELECTOR) || [])) host.appendChild(legacy); ensureTornInventoryWahAreas(host); if (block) block.wahHost = host; return host;
  }
  function getTornInventoryWahArea(blockOrModel, area) { const host = ensureTornInventoryWahRow(blockOrModel); return ensureTornInventoryWahAreas(host)?.[area] || host; }
  function renderTornInventoryTrader(block, result, now = Date.now()) {
    const row = block?.model?.sourceRow; if (!row) return null; let node = row.querySelector?.(':scope [data-wah-item-trader]'); const host = getTornInventoryWahArea(block, 'markets'); if (!host) return null; if (!node) { node = document.createElement('span'); node.className = 'wah-item-trader'; node.dataset.wahItemTrader = String(block.model.itemId); host.appendChild(node); }
    const valuation = buildInventoryTraderValuation(result, block.currentQuantity); node.dataset.wahState = valuation.unitPrice ? valuation.status === 'eligible' ? 'available' : 'stored' : 'unavailable'; node.dataset.wahFreshness = valuation.status === 'eligible' ? 'current' : valuation.unitPrice ? 'stored' : 'unavailable'; node.hidden = !valuation.unitPrice; if (!valuation.unitPrice) node.textContent = 'Trader —'; else { const prefix = valuation.status === 'eligible' ? 'Trader' : 'Stored Trader'; node.textContent = `${prefix} ${formatLedgerMoney(valuation.unitPrice)} · ${valuation.quote.traderName} · ${formatStoredQuoteAge(valuation.quote.lastSeenAt, now)}`; }
    node.title = valuation.status === 'eligible' ? `Eligible according to the latest safely observed Weav3r data. Item-ID-bound Trader price, not a valuation of a specific instance. Quote seen ${formatStoredQuoteAge(valuation.quote.lastSeenAt, now)}.` : valuation.status === 'unverified' ? `Safely stored Weav3r Trader price. Current eligibility is unverified; this is not claimed to be the current best Trader. Item-ID-bound quote seen ${formatStoredQuoteAge(valuation.quote.lastSeenAt, now)}.` : 'No currently eligible or safely displayable fallback Trader quote.'; return node;
  }
  function renderTornInventoryBasis(block, summary) {
    const row = block?.model?.sourceRow; if (!row || !summary) return null; let basis = row.querySelector?.(':scope [data-wah-item-basis]'); const host = getTornInventoryWahArea(block, 'meta'); if (!host) return null;
    ...
    basis.dataset.wahState = ['divergent', 'untracked'].includes(summary.reconstructionStatus) ? 'unavailable' : 'available'; basis.dataset.wahBasisStatus = summary.reconstructionStatus; basis.hidden = summary.reconstructionStatus === 'untracked';
    if (summary.reconstructionStatus === 'divergent') basis.textContent = 'Basis —';
    else if (summary.reconstructionStatus === 'untracked') basis.textContent = 'Basis —';
    else basis.textContent = `Basis ${average} each${summary.trackedNetQuantity < summary.currentQuantity ? ` · ${summary.trackedNetQuantity}/${summary.currentQuantity} tracked` : ''}`;
    basis.title = `${summary.reconstructionStatus === 'untracked' ? 'No confirmed purchase basis. ' : summary.reconstructionStatus === 'divergent' ? 'Inventory cannot be fully reconstructed. ' : ''}Item-ID-based evaluation from confirmed Weav3r Arbitrage Helper transactions. Remaining basis uses only safely attributable costs; after outbounds, fully known inbound costs use exact rational WAC accounting. Other inventory changes may not be fully captured.${summary.knownPurchaseQuantity < summary.confirmedInboundQuantity ? ' Not all confirmed inbounds have an unambiguously attributable price.' : ''}`;
    return basis;
  }
  ...
  function resolveVerifiedMarketDelta(bestSaleResult, bazaarValuation, traderValuation) {
    if (bestSaleResult?.status !== 'available' || bestSaleResult.verification !== 'verified' || !['bazaar', 'trader'].includes(bestSaleResult.channel)) return null;
    if (bazaarValuation?.status !== 'available' || !bazaarValuation.recommendedUnitPrice || traderValuation?.status !== 'eligible' || !traderValuation.unitPrice) return null;
    const bazaarUnit = BigInt(bazaarValuation.recommendedUnitPrice); const traderUnit = BigInt(traderValuation.unitPrice); const winnerUnit = bestSaleResult.channel === 'bazaar' ? bazaarUnit : traderUnit; const alternativeUnit = bestSaleResult.channel === 'bazaar' ? traderUnit : bazaarUnit;
    if (winnerUnit !== BigInt(bestSaleResult.unitPrice) || winnerUnit <= alternativeUnit || alternativeUnit <= 0n) return null; const absoluteDelta = winnerUnit - alternativeUnit;
    return { winner: bestSaleResult.channel, alternative: bestSaleResult.channel === 'bazaar' ? 'trader' : 'bazaar', absoluteDelta: absoluteDelta.toString(), percentage: makeBigIntRatio(absoluteDelta * 100n, alternativeUnit) };
  }
  function formatInventoryAdvantagePercent(ratio) { if (!ratio) return null; const hundredths = roundBigIntRatio(BigInt(ratio.numerator) * 100n, BigInt(ratio.denominator)); const absolute = hundredths < 0n ? -hundredths : hundredths; return `${hundredths > 0n ? '+' : hundredths < 0n ? '-' : ''}${absolute / 100n}.${(absolute % 100n).toString().padStart(2, '0')}%`; }
  function inventoryProfitUnavailableReason(reason) { return ({ 'invalid-input': 'Invalid inventory data.', 'divergent-inventory': 'Inventory cannot be fully reconstructed.', 'no-confirmed-basis': 'No confirmed purchase basis.', 'acquisition-cost-not-assignable': 'Acquisition costs cannot be safely attributed to remaining inventory.', 'unsafe-covered-quantity': 'Quantity covered by known costs cannot be determined safely.', 'no-verified-best-sale': 'No currently verified sale value.' })[reason] || 'Potential gross profit cannot be determined safely.'; }
  function renderTornInventoryProfit(block, result) {
    const row = block?.model?.sourceRow; if (!row) return null; let node = row.querySelector?.(':scope [data-wah-item-profit]'); const host = getTornInventoryWahArea(block, 'meta'); if (!host) return null;
    ...
    const available = ['full', 'partial'].includes(result.status); node.dataset.wahState = available ? BigInt(result.potentialProfit.numerator) < 0n ? 'negative' : 'positive' : 'unavailable'; node.hidden = !available && result.reason === 'no-confirmed-basis'; if (!available) node.textContent = 'Profit —'; else { const coverage = result.status === 'partial' ? ` · ${result.coveredQuantity}/${result.currentQuantity} covered` : ''; node.textContent = `Profit ${formatBigIntRatioMoney(result.potentialProfit, true)} · ${formatInventoryRoi(result.roi)}${coverage}`; }
    node.title = ['full', 'partial'].includes(result.status) ? `Potential gross profit based on confirmed acquisition costs and the currently verified Best Sale value. Unknown inventory portions are excluded. Item-ID-based WAC remaining basis; not a statement about a specific instance. Sale fees are excluded.${result.roi ? '' : ' ROI is unavailable for a confirmed $0 cost basis.'}` : `${inventoryProfitUnavailableReason(result.reason)} No realized or guaranteed profit.`;
    return node;
  }
  function renderTornInventoryBestSale(block, result, storedBest = null, now = Date.now(), marketDelta = null) {
    const row = block?.model?.sourceRow; if (!row) return null; let node = row.querySelector?.(':scope [data-wah-item-best-sale]'); const host = getTornInventoryWahArea(block, 'primary'); if (!host) return null;
    ...
    node.dataset.wahState = result.status === 'available' ? 'available' : storedBest ? 'stored' : 'unavailable'; node.dataset.wahChannel = result.channel || storedBest?.source || 'unavailable'; node.dataset.wahMarketDelta = marketDelta ? 'verified' : 'none'; node.textContent = formatBestSalePrimary(result, storedBest, marketDelta); node.hidden = !node.textContent;
    node.title = result.status === 'available' ? `Currently verified sale channel (${result.channel === 'bazaar' ? 'Bazaar recommendation' : 'eligible Trader'}). Possible gross value, not guaranteed proceeds.` : storedBest ? `Safely stored ${storedBest.source === 'bazaar' ? 'Bazaar' : 'Trader'} candidate (${formatStoredQuoteAge(storedBest.freshness, now)}). Unverified and excluded from Profit, ROI, and verified market comparisons.` : '';
    return node;
  }
  function renderTornInventoryBazaar(block, result, storedValuation = null, now = Date.now()) {
    const row = block?.model?.sourceRow; if (!row) return null; let node = row.querySelector?.(':scope [data-wah-item-bazaar]'); const host = getTornInventoryWahArea(block, 'markets'); if (!host) return null;
    ...
    const stored = valuation.status !== 'available' && storedValuation?.status === 'sort-available'; node.dataset.wahState = valuation.status === 'available' ? 'available' : stored ? 'stored' : valuation.status === 'loading' ? 'loading' : 'unavailable'; node.dataset.wahFreshness = valuation.status === 'available' ? 'current' : stored ? 'stored' : 'unavailable'; if (stored) node.textContent = `Stored Bazaar ${formatLedgerMoney(storedValuation.recommendedUnitPrice)} · ${formatStoredQuoteAge(storedValuation.lastSeenAt, now)}`; else if (valuation.status === 'loading') node.textContent = 'Bazaar …'; else if (valuation.status !== 'available') node.textContent = 'Bazaar —'; else node.textContent = `Bazaar ${formatLedgerMoney(valuation.recommendedUnitPrice)} · current`;
    node.title = valuation.status === 'available' ? `Currently verified possible gross sale value based on the safe Bazaar recommendation. Not guaranteed proceeds. Market: ${formatLedgerMoney(valuation.marketUnitPrice)} · Adjustment: ${valuation.adjustment} · Recommendation: ${formatLedgerMoney(valuation.recommendedUnitPrice)}.` : storedValuation?.status === 'sort-available' ? `Safely stored Bazaar observation, seen ${formatStoredQuoteAge(storedValuation.lastSeenAt, now)}. The current price adjustment was reapplied. Available for sorting, but not a currently verified Best Sale or Profit value.` : `Bazaar price unavailable${valuation.reason ? ` (${valuation.reason})` : ''}. No price or profit expectation is invented.`;
    return node;
  }
  ...
  function resolveInventorySortBestSale(bestSaleResult, bazaarSortValuation, currentQuantity) { const quantity = normalizePositiveInt(currentQuantity); const missing = { status: 'unavailable', unitPrice: null, totalValue: null, quantity, origin: 'missing', verification: 'none' }; if (!quantity) return missing; if (bestSaleResult?.status === 'available' && bestSaleResult.verification === 'verified' && normalizeMoneyString(bestSaleResult.unitPrice)) return { status: 'sort-available', unitPrice: normalizeMoneyString(bestSaleResult.unitPrice), totalValue: (BigInt(bestSaleResult.unitPrice) * BigInt(quantity)).toString(), quantity, origin: 'verified', verification: 'verified' }; const candidates = []; if (['available', 'sort-available'].includes(bazaarSortValuation?.status) && normalizeMoneyString(bazaarSortValuation.recommendedUnitPrice)) candidates.push({ unitPrice: normalizeMoneyString(bazaarSortValuation.recommendedUnitPrice), origin: 'stored-bazaar' }); if (bestSaleResult?.status === 'fallback' && bestSaleResult.verification === 'unverified' && bestSaleResult.channel === 'stored-trader' && normalizeMoneyString(bestSaleResult.unitPrice)) candidates.push({ unitPrice: normalizeMoneyString(bestSaleResult.unitPrice), origin: 'stored-trader' }); if (!candidates.length) return missing; candidates.sort((left, right) => BigInt(left.unitPrice) === BigInt(right.unitPrice) ? 0 : BigInt(left.unitPrice) > BigInt(right.unitPrice) ? -1 : 1); const winner = candidates[0]; return { status: 'sort-available', unitPrice: winner.unitPrice, totalValue: (BigInt(winner.unitPrice) * BigInt(quantity)).toString(), quantity, origin: winner.origin, verification: 'sort-only' }; }
  function buildInventorySortKey(record, criterion) {
    if (!record) return null; if (criterion === 'original') return inventorySortMoney(record.originalIndex); if (criterion === 'name') return { text: normalizeInventoryIdentityText(record.block?.model?.itemName) }; if (criterion === 'quantity') return inventorySortMoney(record.displayQuantity ?? record.block?.currentQuantity);
    ...
    if (criterion === 'best-sale-unit' || criterion === 'best-sale-total') { const sortBestSale = resolveInventorySortBestSale(record.bestSaleResult, record.bazaarSortValuation, record.block?.currentQuantity); return sortBestSale.status === 'sort-available' ? inventorySortMoney(criterion === 'best-sale-unit' ? sortBestSale.unitPrice : sortBestSale.totalValue) : null; }
  ...
  function buildInventorySortSnapshot(records, settings = state.inventorySortSettings) { const normalized = normalizeInventorySortSettings(settings); const entries = records.map((record) => { const model = record.block.model; const sortKey = buildInventorySortKey(record, normalized.criterion); const frozenKey = sortKey == null ? null : Object.freeze({ ...sortKey }); return Object.freeze({ identity: inventoryBlockStableIdentity(record.block), sortKey: frozenKey, originalIndex: record.originalIndex, rowKeys: Object.freeze(record.block.rowKeys.slice()), tieBreaker: Object.freeze({ name: normalizeInventoryIdentityText(model.itemName), itemId: model.itemId, originalIndex: record.originalIndex, rowKey: String(model.rowKey || model.instanceKey || '') }) }); }); const signatureEntries = entries.map((entry) => [entry.identity, inventorySortKeySignature(entry.sortKey)]).sort((left, right) => String(left[0]).localeCompare(String(right[0]))); const signature = JSON.stringify([normalized.criterion, normalized.direction, ...signatureEntries]); return Object.freeze({ criterion: normalized.criterion, direction: normalized.direction, signature, entries: Object.freeze(entries) }); }
  function compareInventorySnapshotTieBreakers(left, right) { const name = left.tieBreaker.name.localeCompare(right.tieBreaker.name); if (name) return name; if (left.tieBreaker.itemId !== right.tieBreaker.itemId) return left.tieBreaker.itemId - right.tieBreaker.itemId; if (left.tieBreaker.originalIndex !== right.tieBreaker.originalIndex) return left.tieBreaker.originalIndex - right.tieBreaker.originalIndex; return left.tieBreaker.rowKey.localeCompare(right.tieBreaker.rowKey); }
  function compareInventorySnapshotEntries(left, right, direction = 'asc') { if (left.sortKey == null || right.sortKey == null) { if (left.sortKey == null && right.sortKey == null) return compareInventorySnapshotTieBreakers(left, right); return left.sortKey == null ? 1 : -1; } let comparison = left.sortKey.text != null || right.sortKey.text != null ? String(left.sortKey.text || '').localeCompare(String(right.sortKey.text || '')) : compareInventorySortRatios(left.sortKey, right.sortKey); if (comparison && direction === 'desc') comparison = -comparison; return comparison || compareInventorySnapshotTieBreakers(left, right); }
  function applyInventorySortSnapshot(container, snapshot, scan) { const byRowKey = new Map(scan.rows.map((model) => [model.rowKey, model.sourceRow])); const sorted = snapshot.entries.slice().sort((left, right) => compareInventorySnapshotEntries(left, right, snapshot.direction)); const desiredRows = sorted.flatMap((entry) => entry.rowKeys.map((rowKey) => byRowKey.get(rowKey)).filter((row) => row?.parentElement === container)); const currentRows = Array.from(container.children || []).filter((row) => byRowKey.has(row.dataset?.rowkey)); if (desiredRows.length === currentRows.length && desiredRows.some((row, index) => row !== currentRows[index])) for (const row of desiredRows) { state.inventorySortInternalRows.add(row); container.appendChild(row); } if (desiredRows.length) setTimeout(() => desiredRows.forEach((row) => state.inventorySortInternalRows.delete(row)), 0); return sorted; }
  function setInventorySortDirty(container, dirty) { if (!container) return false; if (dirty) state.inventorySortDirty.add(container); else state.inventorySortDirty.delete(container); const control = document.getElementById(INVENTORY_SORT_CONTROL_ID); if (control && control.nextElementSibling === container) { const status = control.querySelector('[data-wah-inventory-sort-status]'); const refresh = control.querySelector('[data-wah-inventory-resort]'); if (status) { status.hidden = !dirty; status.textContent = dirty ? 'Prices updated' : ''; } if (refresh) refresh.hidden = !dirty; } return dirty; }
  function applyLatestInventorySortSnapshot(container, settings = state.inventorySortSettings) { const latest = state.inventorySortLatest.get(container); if (!latest) return { applied: false, reason: 'no-current-records' }; const snapshot = buildInventorySortSnapshot(latest.records, settings); applyInventorySortSnapshot(container, snapshot, latest.scan); state.inventorySortSnapshots.set(container, snapshot); state.inventorySortLastSignatures.set(container, snapshot.signature); setInventorySortDirty(container, false); logDebug('inventory-sort-snapshot-applied', { criterion: snapshot.criterion, direction: snapshot.direction, blockCount: snapshot.entries.length }); return { applied: true, snapshot }; }
  function updateInventorySortSnapshotState(container, records, scan, settings = state.inventorySortSettings) { state.inventorySortLatest.set(container, { records, scan }); const current = state.inventorySortSnapshots.get(container); if (!current) return applyLatestInventorySortSnapshot(container, settings); const candidateSignature = buildInventorySortSnapshot(records, settings).signature; if (candidateSignature !== current.signature) setInventorySortDirty(container, true); return { applied: false, dirty: state.inventorySortDirty.has(container), snapshot: current }; }
  ...
  function renderTornInventoryWeav3rLinks(block) {
    const row = block?.model?.sourceRow; const itemId = normalizePositiveInt(block?.model?.itemId); const host = getTornInventoryWahArea(block, 'actions'); if (!row || !itemId || !host) return null;
    let node = row.querySelector?.(':scope [data-wah-weav3r-links]'); if (!node) { node = document.createElement('span'); node.className = 'wah-item-weav3r-links'; node.dataset.wahWeav3rLinks = String(itemId); host.appendChild(node); }
    const bazaarUrl = buildWeav3rItemViewUrl(itemId, 'bazaar'); const tradeUrl = buildWeav3rItemViewUrl(itemId, 'trade'); node.innerHTML = `<a data-wah-weav3r-link="bazaar" href="${escapeAttribute(bazaarUrl)}" target="_blank" rel="noopener noreferrer">Bazaar</a><a data-wah-weav3r-link="trade" href="${escapeAttribute(tradeUrl)}" target="_blank" rel="noopener noreferrer">Trade</a>`; return node;
  }
  function renderInventoryBazaarSellQuantity(record) {
    const row = record?.block?.model?.sourceRow; const host = getTornInventoryWahArea(record?.block, 'sell'); if (!host) return null; let node = row.querySelector?.(':scope [data-wah-bazaar-sell-quantity]');
    if (!node) { node = document.createElement('span'); node.className = 'wah-item-sell-quantity'; node.dataset.wahBazaarSellQuantity = ''; host.appendChild(node); }
    node.dataset.wahState = record.sellDecision.sellableQuantity > 0 ? 'available' : 'zero'; node.textContent = `· ${record.sellDecision.sellableQuantity}/${record.sellDecision.inventoryQuantity}`; return node;
  }
  function renderInventoryBazaarSellRule(record) {
    const block = record?.block; const row = block?.model?.sourceRow; const host = getTornInventoryWahArea(block, 'sell'); if (!row || !host) return null; let label = row.querySelector?.(':scope [data-wah-inventory-sell-rule-control]');
    if (!label) { label = document.createElement('label'); label.className = 'wah-item-sell-rule'; label.dataset.wahInventorySellRuleControl = String(block.model.itemId); label.innerHTML = `Sell: <select data-wah-inventory-sell-rule="${block.model.itemId}" aria-label="Sell rule for ${escapeAttribute(block.model.itemName)}">${bazaarSellRuleOptionsHtml()}</select>`; host.appendChild(label); }
    label.dataset.wahState = record.sellDecision.sellRule === 'dont-sell' ? 'unavailable' : 'available'; const select = label.querySelector?.('[data-wah-inventory-sell-rule]'); if (select) select.value = record.sellDecision.sellRule; return label;
  }
  function handleInventoryBazaarSellRuleChange(select) {
    const itemId = normalizePositiveInt(select?.dataset?.wahInventorySellRule); if (!itemId || !setBazaarSellRule(itemId, select.value)) return false; const container = getActiveTornInventoryContainer(document); const latest = state.inventorySortLatest.get(container); if (!latest) { scheduleTornInventoryBasisRefresh(); return true; }
    for (const record of latest.records) if (record.block.model.itemId === itemId) { record.sellDecision = resolveBazaarInventorySellDecision(itemId, record.block.currentQuantity, tornState.bazaarAdd.rules); record.displayQuantity = state.inventoryBazaarSellFilter ? record.sellDecision.sellableQuantity : record.block.currentQuantity; renderInventoryBazaarSellRule(record); renderInventoryBazaarSellQuantity(record); }
    applyInventoryBazaarSellFilter(container, latest.records); if (state.inventorySortSettings.criterion === 'quantity') applyLatestInventorySortSnapshot(container, state.inventorySortSettings); return true;
  }
  function applyInventoryBazaarSellFilter(container, records) {
    if (!container) return 0; let visible = 0;
    const rowsByKey = new Map(Array.from(container.children || []).filter((row) => row?.dataset?.rowkey).map((row) => [row.dataset.rowkey, row]));
    for (const record of records || []) { record.displayQuantity = state.inventoryBazaarSellFilter ? record.sellDecision?.sellableQuantity : record.block?.currentQuantity; const hidden = state.inventoryBazaarSellFilter && !record.sellDecision?.visible; for (const rowKey of record.block?.rowKeys || []) rowsByKey.get(String(rowKey))?.classList.toggle('wah-inventory-sell-filter-hidden', hidden); if (!hidden) visible += 1; renderInventoryBazaarSellRule(record); renderInventoryBazaarSellQuantity(record); }
    return visible;
  }
  function renderInventoryBazaarRefreshState(control = document.getElementById(INVENTORY_SORT_CONTROL_ID)) { const button = control?.querySelector?.('[data-wah-inventory-refresh-bazaar]'); const status = control?.querySelector?.('[data-wah-inventory-refresh-status]'); const operation = state.inventoryBazaarManualRefresh; if (button) button.disabled = Boolean(operation); if (status) { status.hidden = !operation; status.textContent = operation ? `Refreshing Bazaar… ${operation.completed}/${operation.total}` : ''; } }
  async function refreshOwnedInventoryBazaar() { if (state.inventoryBazaarManualRefresh) return state.inventoryBazaarManualRefresh.promise; const container = getActiveTornInventoryContainer(document); const latest = state.inventorySortLatest.get(container); const itemIds = (latest?.records || []).filter((record) => record.block?.model?.ownership !== 'faction-loan').map((record) => record.block?.model?.itemId); const operation = { total: [...new Set(itemIds.map(normalizePositiveInt).filter(Boolean))].length, completed: 0, updated: 0, kept: 0, promise: null }; state.inventoryBazaarManualRefresh = operation; renderInventoryBazaarRefreshState(); operation.promise = refreshBazaarValuesForItemIds(itemIds, { surface: 'item-inventory', onResult: (itemId, result) => state.inventoryBazaarResults.set(itemId, result), onProgress: (progress) => { operation.completed = progress.completed; operation.updated = progress.updated; operation.kept = progress.kept; renderInventoryBazaarRefreshState(); } }).then((result) => { if (container) setInventorySortDirty(container, result.updated > 0 || state.inventorySortDirty.has(container)); scheduleTornInventoryBasisRefresh(); return result; }).finally(() => { const control = document.getElementById(INVENTORY_SORT_CONTROL_ID); state.inventoryBazaarManualRefresh = null; renderInventoryBazaarRefreshState(control); const status = control?.querySelector?.('[data-wah-inventory-refresh-status]'); if (status) { status.hidden = false; status.textContent = formatBazaarRefreshSummary({ checked: operation.completed, updated: operation.updated, kept: operation.kept }); } }); return operation.promise; }

  function ensureInventorySortControl(container) {
    if (!container?.parentElement) return null; let control = document.getElementById(INVENTORY_SORT_CONTROL_ID); if (!control) { control = document.createElement('div'); control.id = INVENTORY_SORT_CONTROL_ID; control.className = 'wah-inventory-sort'; control.innerHTML = `<label>Sort by: <select data-wah-inventory-sort="criterion">${[['original','Original'],['name','Name'],['quantity','Quantity'],['average-acquisition','Avg. purchase'],['current-basis','Remaining basis'],['bazaar-unit','Bazaar / item'],['bazaar-total','Bazaar total'],['trader-unit','Trader / item'],['trader-total','Trader total'],['best-sale-unit','Best Sale / item'],['best-sale-total','Best Sale total'],['profit','Profit'],['roi','ROI']].map(([value,label]) => `<option value="${value}">${label}</option>`).join('')}</select></label><label>Direction: <select data-wah-inventory-sort="direction"><option value="asc">Ascending</option><option value="desc">Descending</option></select></label><label><input type="checkbox" data-wah-inventory-sell-filter> Bazaar sell only</label><button type="button" data-wah-inventory-refresh-bazaar>Refresh Bazaar</button><span data-wah-inventory-refresh-status hidden></span><span data-wah-inventory-sort-status hidden></span><button type="button" data-wah-inventory-resort hidden>Re-sort</button>`; control.addEventListener('change', (event) => { if (event.target?.matches?.('[data-wah-inventory-sell-filter]')) { state.inventoryBazaarSellFilter = event.target.checked === true; const active = getActiveTornInventoryContainer(document); const latest = state.inventorySortLatest.get(active); if (latest) { applyInventoryBazaarSellFilter(active, latest.records); applyLatestInventorySortSnapshot(active, state.inventorySortSettings); } else scheduleTornInventoryBasisRefresh(); return; } const field = event.target?.dataset?.wahInventorySort; if (!['criterion', 'direction'].includes(field)) return; state.inventorySortSettings = normalizeInventorySortSettings({ ...state.inventorySortSettings, [field]: event.target.value }); gmSet(INVENTORY_SORT_SETTINGS_KEY, state.inventorySortSettings); const active = getActiveTornInventoryContainer(document); if (!applyLatestInventorySortSnapshot(active, state.inventorySortSettings).applied) scheduleTornInventoryBasisRefresh(); ensureInventorySortControl(active); }); control.addEventListener('click', (event) => { if (event.target?.matches?.('[data-wah-inventory-refresh-bazaar]')) { void refreshOwnedInventoryBazaar(); return; } if (!event.target?.matches?.('[data-wah-inventory-resort]')) return; const active = getActiveTornInventoryContainer(document); applyLatestInventorySortSnapshot(active, state.inventorySortSettings); }); }
    if (control.parentElement !== container.parentElement || control.nextElementSibling !== container) container.parentElement.insertBefore(control, container); renderInventoryBazaarRefreshState(control); const criterion = control.querySelector('[data-wah-inventory-sort="criterion"]'); const direction = control.querySelector('[data-wah-inventory-sort="direction"]'); criterion.value = state.inventorySortSettings.criterion; direction.value = state.inventorySortSettings.direction; direction.disabled = state.inventorySortSettings.criterion === 'original'; const sellFilter = control.querySelector('[data-wah-inventory-sell-filter]'); if (sellFilter) sellFilter.checked = state.inventoryBazaarSellFilter; setInventorySortDirty(container, state.inventorySortDirty.has(container)); return control;
  }
  ...
  function refreshTornInventoryBasis(root = document) {
    ...
    for (const container of containers) {
      ...
      for (const model of scan.allRows || scan.rows) if (!targetRows.has(model.sourceRow)) { model.sourceRow.querySelector?.(':scope > [data-wah-item-row]')?.remove(); model.sourceRow.querySelector?.(':scope [data-wah-item-history]')?.remove(); model.sourceRow.querySelector?.(':scope [data-wah-item-basis]')?.remove(); model.sourceRow.querySelector?.(':scope [data-wah-item-bazaar]')?.remove(); model.sourceRow.querySelector?.(':scope [data-wah-item-trader]')?.remove(); model.sourceRow.querySelector?.(':scope [data-wah-item-best-sale]')?.remove(); model.sourceRow.querySelector?.(':scope [data-wah-item-profit]')?.remove(); model.sourceRow.querySelector?.(':scope [data-wah-weav3r-links]')?.remove(); if (model.ownership === 'faction-loan') model.sourceRow.classList?.toggle('wah-inventory-sell-filter-hidden', state.inventoryBazaarSellFilter); }
      for (const block of blocks) {
        ensureTornInventoryWahRow(block); ensureTornInventoryHistoryButton(block.model);
        const sellDecision = resolveBazaarInventorySellDecision(block.model.itemId, block.currentQuantity, bazaarSellRules);
        const summary = buildItemBasisSummary(block.model.itemId, block.currentQuantity, aggregates); if (renderTornInventoryBasis(block, summary)) rendered += 1; renderTornInventoryWeav3rLinks(block);
  ...
  function ensureTornInventoryHistoryButton(model) { const row = model?.sourceRow; if (!row?.isConnected && row?.isConnected !== undefined) return null; let button = row?.querySelector?.(':scope [data-wah-item-history]'); if (button) return button; const host = getTornInventoryWahArea(model, 'actions'); if (!host) return null; button = document.createElement('button'); button.type = 'button'; button.className = 'wah-item-history-button'; button.dataset.wahItemHistory = String(model.itemId); button.dataset.wahItemName = model.itemName; button.dataset.action = 'open-item-history'; button.setAttribute('aria-label', `History: ${model.itemName}`); button.textContent = 'History'; host.appendChild(button); return button; }
  function processTornInventoryRows(root = document) { const rows = []; if (root?.matches?.('li[data-item][data-rowkey]')) rows.push(root); rows.push(...Array.from(root?.querySelectorAll?.('li[data-item][data-rowkey]') || [])); return rows.filter((row) => parseTornInventoryRow(row)).length; }
  ...
  function initTornItemInventoryHistory() { if (location.pathname !== '/item.php') return; processTornInventoryRows(document); ensureFullTornInventoryLoaded(); scheduleTornInventoryBasisRefresh(); if (state.inventoryHistoryInitialized) return; state.inventorySortSettings = normalizeInventorySortSettings(gmGet(INVENTORY_SORT_SETTINGS_KEY, null)); state.inventoryHistoryInitialized = true; document.addEventListener('change', (event) => { const select = event.target?.closest?.('[data-wah-inventory-sell-rule]'); if (select) handleInventoryBazaarSellRuleChange(select); }); state.inventoryHistoryObserver = new MutationObserver((mutations) => { let changed = false; for (const mutation of mutations) { if (isInternalInventorySortMutation(mutation)) continue; if (mutation.type === 'attributes') { changed = true; continue; } for (const node of mutation.addedNodes || []) if (nodeContainsTornInventoryRow(node)) { processTornInventoryRows(node); changed = true; } } if (changed) { state.inventoryValuationGeneration += 1; state.inventoryBazaarPending.clear(); resetInventoryBazaarRound(); state.inventorySortSnapshots = new WeakMap(); state.inventorySortLatest = new WeakMap(); state.inventorySortDirty = new WeakSet(); state.inventorySortPending.clear(); clearTimeout(state.inventorySortQuietTimer); clearTimeout(state.inventorySortMaxTimer); state.inventorySortQuietTimer = 0; state.inventorySortMaxTimer = 0; scheduleTornInventoryFullLoad(80); scheduleTornInventoryBasisRefresh(); } }); state.inventoryHistoryObserver.observe(document.documentElement, { childList: true, subtree: true, attributes: true, attributeFilter: ['data-all', 'data-from', 'data-queue', 'aria-hidden'] }); if (typeof GM_addValueChangeListener === 'function') { state.inventoryBazaarSettingsListenerId = GM_addValueChangeListener(BAZAAR_ADD_UI_SETTINGS_KEY, () => { state.inventoryBazaarResults.clear(); state.inventoryValuationGeneration += 1; state.inventoryBazaarPending.clear(); resetInventoryBazaarRound(); scheduleTornInventoryBasisRefresh(); }); state.inventoryBazaarQuoteListenerId = GM_addValueChangeListener(BAZAAR_QUOTES_KEY, (_key, _oldValue, _newValue, remote) => { const refreshScheduled = Boolean(remote); logDebug('bazaar-quote-store-change', { remote: Boolean(remote), inventoryRefreshScheduled: refreshScheduled }); if (refreshScheduled) scheduleTornInventoryBasisRefresh(); }); state.inventoryTraderQuoteListenerId = GM_addValueChangeListener(TRADER_QUOTES_KEY, (_key, _oldValue, _newValue, remote) => { logDebug('trader-quote-store-change', { remote: Boolean(remote), inventoryRefreshScheduled: true }); scheduleTornInventoryBasisRefresh(); }); state.inventorySettingsListenerId = GM_addValueChangeListener(SETTINGS_KEY, () => { readSettings(); scheduleTornInventoryBasisRefresh(); }); state.inventoryBazaarSellRulesListenerId = GM_addValueChangeListener(BAZAAR_SELL_RULES_KEY, (_key, _oldValue, _newValue, remote) => { if (remote) scheduleTornInventoryBasisRefresh(); }); } window.addEventListener('beforeunload', () => { state.inventoryHistoryObserver?.disconnect(); clearTimeout(state.inventoryBasisRefreshTimer); clearTimeout(state.inventoryBazaarRefreshTimer); clearTimeout(state.inventorySortQuietTimer); clearTimeout(state.inventorySortMaxTimer); clearTimeout(state.inventoryFullLoad.timer); if (typeof GM_removeValueChangeListener === 'function') { if (state.inventoryBazaarSettingsListenerId) GM_removeValueChangeListener(state.inventoryBazaarSettingsListenerId); if (state.inventoryBazaarQuoteListenerId) GM_removeValueChangeListener(state.inventoryBazaarQuoteListenerId); if (state.inventoryTraderQuoteListenerId) GM_removeValueChangeListener(state.inventoryTraderQuoteListenerId); if (state.inventorySettingsListenerId) GM_removeValueChangeListener(state.inventorySettingsListenerId); if (state.inventoryBazaarSellRulesListenerId) GM_removeValueChangeListener(state.inventoryBazaarSellRulesListenerId); } state.inventoryHistoryObserver = null; state.inventoryBasisRefreshTimer = null; state.inventoryBazaarSettingsListenerId = null; state.inventoryBazaarQuoteListenerId = null; state.inventoryTraderQuoteListenerId = null; state.inventorySettingsListenerId = null; state.inventoryBazaarSellRulesListenerId = null; state.inventoryBazaarResults.clear(); state.inventorySortOriginals = new WeakMap(); state.inventorySortLastSignatures = new WeakMap(); state.inventorySortSnapshots = new WeakMap(); state.inventorySortLatest = new WeakMap(); state.inventorySortDirty = new WeakSet(); state.inventorySortPending.clear(); state.inventoryBazaarPending.clear(); state.inventoryHistoryInitialized = false; }, { once: true }); logDebug('Torn item.php inventory history, full loading, valuation, and stable sorting initialized.'); }
  ...
  function ensureSharedHistoryStyles() {
    ...
    GM_addStyle(`.wah-history-backdrop{position:fixed;inset:0;z-index:2147482500;background:rgba(0,0,0,.58);display:flex;align-items:center;justify-content:center;padding:16px}.wah-history-dialog{width:min(920px,calc(100vw - 32px));max-height:88vh;display:flex;flex-direction:column;overflow:hidden;border:1px solid #475569;border-radius:14px;background:#111827;color:#f8fafc;box-shadow:0 18px 60px rgba(0,0,0,.5);font:13px system-ui,sans-serif}.wah-history-header,.wah-history-tabs,.wah-history-filters,.wah-history-summary{display:flex;gap:8px;flex-wrap:wrap;align-items:center}.wah-history-header{justify-content:space-between;padding:14px 16px;border-bottom:1px solid #334155}.wah-history-header h2{margin:0}.wah-history-tabs,.wah-history-filters{padding:10px 16px;border-bottom:1px solid #334155}.wah-history-content{overflow:auto;padding:14px 16px;display:grid;gap:10px}.wah-history-card{display:grid;gap:9px;padding:12px;border:1px solid #334155;border-radius:10px}.wah-history-card-top{display:flex;justify-content:space-between}.wah-history-grid,.wah-history-details{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px}.wah-history-cell span,.wah-note{display:block;color:#94a3b8;font-size:11px}.wah-history-warning{padding:9px;border:1px solid #f59e0b;border-radius:8px}.wah-history-filters label{display:grid;gap:3px}.wah-history-filters input,.wah-history-filters select,.wah-history-dialog button,.wah-history-dialog .wah-button{border:1px solid #475569;border-radius:7px;background:#1e293b;color:#fff;padding:7px;text-decoration:none}.wah-history-empty{padding:24px;text-align:center}.wah-item-basis{display:inline-block;margin-left:6px;padding:2px 6px;border:1px solid #64748b;border-radius:5px;background:#0f172a;color:#e2e8f0;font:600 10px system-ui,sans-serif;vertical-align:middle}.wah-item-bazaar{display:inline-block;margin-left:6px;padding:2px 6px;border:1px solid #0f766e;border-radius:5px;background:#042f2e;color:#ccfbf1;font:600 10px system-ui,sans-serif;vertical-align:middle}.wah-item-trader{display:inline-block;margin-left:6px;padding:2px 6px;border:1px solid #7c3aed;border-radius:5px;background:#2e1065;color:#ede9fe;font:600 10px system-ui,sans-serif;vertical-align:middle}.wah-item-best-sale{display:inline-block;margin-left:6px;padding:2px 6px;border:1px solid #ca8a04;border-radius:5px;background:#422006;color:#fef9c3;font:700 10px system-ui,sans-serif;vertical-align:middle}.wah-item-profit{display:inline-block;margin-left:6px;padding:2px 6px;border:1px solid #2563eb;border-radius:5px;background:#172554;color:#dbeafe;font:700 10px system-ui,sans-serif;vertical-align:middle}.wah-inventory-sort{display:flex;gap:8px;align-items:center;flex-wrap:wrap;margin:6px 0;padding:6px 8px;border:1px solid #475569;border-radius:7px;background:#111827;color:#e2e8f0;font:600 11px system-ui,sans-serif}.wah-inventory-sort label{display:flex;gap:4px;align-items:center}.wah-inventory-sort select{max-width:150px;border:1px solid #64748b;border-radius:5px;background:#1e293b;color:#fff;padding:3px 5px}.wah-inventory-sort [data-wah-inventory-sort-status]{color:#facc15}.wah-inventory-sort [data-wah-inventory-resort],.wah-inventory-sort [data-wah-inventory-refresh-bazaar]{border:1px solid #a16207;border-radius:5px;background:#422006;color:#fef9c3;padding:3px 6px;cursor:pointer}.wah-inventory-sort [hidden]{display:none!important}.wah-item-row{display:flex;flex-wrap:wrap;align-items:center;gap:4px;margin:3px 0;min-width:0}.wah-item-row>.wah-item-basis,.wah-item-row>.wah-item-bazaar,.wah-item-row>.wah-item-trader,.wah-item-row>.wah-item-best-sale,.wah-item-row>.wah-item-profit,.wah-item-row>.wah-item-sell-quantity,.wah-item-row>.wah-item-weav3r-links,.wah-item-row>.wah-item-history-button,.wah-item-row>.wah-item-sell-rule{margin-left:0}.wah-item-sell-rule{display:inline-flex;align-items:center;gap:3px;font:600 10px system-ui,sans-serif}.wah-item-sell-rule select{border:1px solid #64748b;border-radius:5px;background:#1e293b;color:#fff;padding:2px 4px}.wah-item-weav3r-links{display:inline-flex;gap:4px;margin-left:6px;vertical-align:middle}.wah-item-weav3r-links a{border:1px solid #475569;border-radius:5px;background:#111827;color:#bfdbfe;padding:2px 5px;font:700 10px system-ui,sans-serif;text-decoration:none}.wah-item-sell-quantity{display:inline-block;margin-left:6px;padding:2px 6px;border:1px solid #16a34a;border-radius:5px;background:#052e16;color:#dcfce7;font:700 10px system-ui,sans-serif;vertical-align:middle}.wah-inventory-sell-filter-hidden{display:none!important}.wah-item-history-button{margin-left:6px;border:1px solid #64748b;border-radius:999px;background:#1e293b;color:#fff;padding:2px 7px;font:600 10px system-ui,sans-serif;cursor:pointer}.wah-item-row{flex:1 1 100%;width:100%;max-width:100%;box-sizing:border-box;display:grid;grid-template-columns:minmax(260px,1.35fr) minmax(220px,1fr) auto;align-items:center;gap:5px 12px;margin:2px 0;padding:4px 6px;border-top:1px solid rgba(148,163,184,.18);background:rgba(15,23,42,.22);font:600 10px system-ui,sans-serif}.wah-item-main,.wah-item-meta,.wah-item-market-values,.wah-item-actions,.wah-item-sell-control{display:flex;align-items:center;flex-wrap:wrap;min-width:0;gap:3px 9px}.wah-item-main{gap:4px 12px}.wah-item-market-values{color:#cbd5e1}.wah-item-meta{color:#94a3b8}.wah-item-actions{justify-content:flex-end;gap:4px}.wah-item-sell-control{gap:4px}.wah-item-row .wah-item-best-sale,.wah-item-row .wah-item-bazaar,.wah-item-row .wah-item-trader,.wah-item-row .wah-item-basis,.wah-item-row .wah-item-profit,.wah-item-row .wah-item-sell-quantity{display:inline-flex;margin:0;padding:0;border:0;border-radius:0;background:transparent;line-height:1.35}.wah-item-row .wah-item-best-sale{color:#fde68a;font-weight:800}.wah-item-row .wah-item-bazaar{color:#99f6e4}.wah-item-row .wah-item-trader{color:#ddd6fe}.wah-item-row [data-wah-state="unavailable"]{color:#64748b;font-weight:500}.wah-item-row [data-wah-state="stored"]{color:#94a3b8;font-weight:600}.wah-item-row [data-wah-state="loading"]{color:#94a3b8;font-weight:500}.wah-item-row [hidden]{display:none!important}.wah-item-row .wah-item-sell-quantity[data-wah-state="zero"]{color:#64748b;font-weight:500}.wah-item-row .wah-item-profit[data-wah-state="positive"]{color:#86efac}.wah-item-row .wah-item-profit[data-wah-state="negative"]{color:#fca5a5}.wah-item-row .wah-item-primary{display:flex;align-items:center;min-width:0}.wah-item-row .wah-item-weav3r-links{display:contents}.wah-item-row .wah-item-weav3r-links a{order:1}.wah-item-row .wah-item-history-button{order:2}.wah-item-row .wah-item-weav3r-links a,.wah-item-row .wah-item-history-button{margin:0;border:1px solid #475569;border-radius:4px;background:rgba(30,41,59,.72);color:#bfdbfe;padding:2px 6px;font:700 10px system-ui,sans-serif;text-decoration:none;line-height:1.35}.wah-item-row .wah-item-history-button{color:#e2e8f0}.wah-item-row .wah-item-weav3r-links a:hover,.wah-item-row .wah-item-history-button:hover{background:#334155}.wah-item-row .wah-item-weav3r-links a:focus-visible,.wah-item-row .wah-item-history-button:focus-visible,.wah-item-row select:focus-visible{outline:2px solid #60a5fa;outline-offset:1px}.wah-item-row .wah-item-sell-rule{color:#94a3b8}.wah-item-row .wah-item-sell-rule select{padding:1px 3px;font-size:10px}.wah-item-row .wah-item-sell-quantity{color:#a7f3d0;font-weight:700}.wah-inventory-sort{gap:5px 9px;margin:4px 0;padding:4px 6px;border-color:rgba(100,116,139,.55);background:rgba(15,23,42,.72);font-size:10px}.wah-inventory-sort select{max-width:140px;padding:2px 4px}.wah-inventory-sort [data-wah-inventory-sort-status]{font-weight:500;color:#cbd5e1}.wah-inventory-sort [data-wah-inventory-resort],.wah-inventory-sort [data-wah-inventory-refresh-bazaar]{padding:2px 5px;border-color:#64748b;background:#1e293b;color:#e2e8f0}@media(max-width:900px){.wah-item-row{grid-template-columns:minmax(240px,1fr) minmax(210px,1fr)}.wah-item-actions{grid-column:1/-1;justify-content:flex-start}}@media(max-width:620px){.wah-item-row{grid-template-columns:1fr;padding:4px}.wah-item-actions{grid-column:auto}}#${HISTORY_LAUNCHER_ID}{position:fixed;right:16px;top:72px;z-index:2147482200;border:1px solid #64748b;border-radius:999px;background:#111827;color:#fff;padding:7px 11px;font:600 12px system-ui,sans-serif;box-shadow:0 5px 18px rgba(0,0,0,.3);cursor:pointer}@media(max-width:620px){#${HISTORY_LAUNCHER_ID}{right:8px;top:56px}.wah-history-grid,.wah-history-details{grid-template-columns:1fr}.wah-history-backdrop{padding:8px}}`);
