...
(function () {
  ...
  function ensureTornInventoryWahRow(blockOrModel) {
    ...
    for (const legacy of Array.from(title?.querySelectorAll?.(TORN_INVENTORY_WAH_SELECTOR) || [])) host.appendChild(legacy); if (block) block.wahHost = host; return host;
  }
  function renderTornInventoryTrader(block, result, now = Date.now()) {
    const row = block?.model?.sourceRow; if (!row) return null; let node = row.querySelector?.(':scope [data-wah-item-trader]'); const host = ensureTornInventoryWahRow(block); if (!host) return null; if (!node) { node = document.createElement('span'); node.className = 'wah-item-trader'; node.dataset.wahItemTrader = String(block.model.itemId); host.appendChild(node); }
    const valuation = buildInventoryTraderValuation(result, block.currentQuantity); if (!valuation.unitPrice) node.textContent = 'Trader N/A'; else { const prefix = valuation.status === 'eligible' ? 'Trader' : 'Stored Trader'; const total = valuation.quantity === 1 ? '' : ` · ${formatLedgerMoney(valuation.totalPrice)} total`; node.textContent = `${prefix} ${valuation.quote.traderName} · ${formatLedgerMoney(valuation.unitPrice)} each${total} · ${formatStoredQuoteAge(valuation.quote.lastSeenAt, now)}`; }
  ...
  function renderTornInventoryBasis(block, summary) {
    const row = block?.model?.sourceRow; if (!row || !summary) return null; let basis = row.querySelector?.(':scope [data-wah-item-basis]'); const host = ensureTornInventoryWahRow(block); if (!host) return null;
    ...
    if (summary.reconstructionStatus === 'divergent') basis.textContent = 'Basis · Inventory cannot be fully reconstructed';
    else if (summary.reconstructionStatus === 'untracked') basis.textContent = 'Basis · No confirmed purchase basis';
    else basis.textContent = `Basis · ${summary.trackedNetQuantity}/${summary.currentQuantity} tracked · Avg. purchase ${average} · Remaining basis ${summary.currentCostBasis ? formatBigIntRatioMoney(summary.currentCostBasis) : 'N/A'}`;
    basis.title = `Item-ID-based evaluation from confirmed Weav3r Arbitrage Helper transactions. Remaining basis uses only safely attributable costs; after outbounds, fully known inbound costs use exact rational WAC accounting. Other inventory changes may not be fully captured.${summary.knownPurchaseQuantity < summary.confirmedInboundQuantity ? ' Not all confirmed inbounds have an unambiguously attributable price.' : ''}`;
    return basis;
  }
  ...
  function renderTornInventoryProfit(block, result) {
    const row = block?.model?.sourceRow; if (!row) return null; let node = row.querySelector?.(':scope [data-wah-item-profit]'); const host = ensureTornInventoryWahRow(block); if (!host) return null;
    ...
    if (!['full', 'partial'].includes(result.status)) node.textContent = 'Profit N/A'; else { const coverage = result.status === 'partial' ? ` on ${result.coveredQuantity}/${result.currentQuantity}` : ` · ${result.coveredQuantity}/${result.currentQuantity} items`; node.textContent = `Profit ${formatBigIntRatioMoney(result.potentialProfit, true)}${coverage} · ROI ${formatInventoryRoi(result.roi)}`; }
  ...
  function renderTornInventoryBestSale(block, result, now = Date.now()) {
    const row = block?.model?.sourceRow; if (!row) return null; let node = row.querySelector?.(':scope [data-wah-item-best-sale]'); const host = ensureTornInventoryWahRow(block); if (!host) return null;
    ...
    if (result.channel === 'stored-trader') node.textContent = `Stored: ${result.trader.traderName} · ${formatLedgerMoney(result.unitPrice)} each · ${formatLedgerMoney(result.totalValue)} total`;
    else if (result.status !== 'available') node.textContent = 'Best Sale: N/A';
  ...
  function renderTornInventoryBazaar(block, result, storedValuation = null, now = Date.now()) {
    const row = block?.model?.sourceRow; if (!row) return null; let node = row.querySelector?.(':scope [data-wah-item-bazaar]'); const host = ensureTornInventoryWahRow(block); if (!host) return null;
    ...
    if (valuation.status !== 'available' && storedValuation?.status === 'sort-available') node.textContent = `Stored: Bazaar ${formatLedgerMoney(storedValuation.recommendedUnitPrice)} · ${formatStoredQuoteAge(storedValuation.lastSeenAt, now)}`; else if (valuation.status === 'loading') node.textContent = 'Bazaar …'; else if (valuation.status !== 'available') node.textContent = 'Bazaar N/A'; else node.textContent = valuation.quantity === 1 ? `Bazaar ${formatLedgerMoney(valuation.recommendedUnitPrice)} · current` : `Bazaar ${formatLedgerMoney(valuation.recommendedUnitPrice)} each · ${formatLedgerMoney(valuation.recommendedTotal)} total · current`;
  ...
  function renderTornInventoryWeav3rLinks(block) {
    const row = block?.model?.sourceRow; const itemId = normalizePositiveInt(block?.model?.itemId); const host = ensureTornInventoryWahRow(block); if (!row || !itemId || !host) return null;
  ...
  function renderInventoryBazaarSellQuantity(record) {
    const row = record?.block?.model?.sourceRow; const host = ensureTornInventoryWahRow(record?.block); if (!host) return null; let node = row.querySelector?.(':scope [data-wah-bazaar-sell-quantity]');
    ...
    node.textContent = `To Bazaar: ${record.sellDecision.sellableQuantity}/${record.sellDecision.inventoryQuantity}`; return node;
  }
  function renderInventoryBazaarSellRule(record) {
    const block = record?.block; const row = block?.model?.sourceRow; const host = ensureTornInventoryWahRow(block); if (!row || !host) return null; let label = row.querySelector?.(':scope [data-wah-inventory-sell-rule-control]');
    ...
    const select = label.querySelector?.('[data-wah-inventory-sell-rule]'); if (select) select.value = record.sellDecision.sellRule; return label;
  }
