...
(function () {
  ...
  function renderTornInventoryBasis(block, summary) {
    ...
    basis.dataset.wahState = ['divergent', 'untracked'].includes(summary.reconstructionStatus) ? 'unavailable' : 'available'; basis.dataset.wahBasisStatus = summary.reconstructionStatus;
  ...
  function renderTornInventoryBestSale(block, result, now = Date.now()) {
    ...
    node.dataset.wahState = result.status === 'available' ? 'available' : result.channel === 'stored-trader' ? 'stored' : 'unavailable'; node.dataset.wahChannel = result.channel || 'unavailable';
    if (result.channel === 'stored-trader') node.textContent = `Stored candidate · ${formatLedgerMoney(result.unitPrice)} each · ${result.trader.traderName}`;
    else if (result.status !== 'available') node.textContent = 'Best Sale —';
    else { const label = result.channel === 'bazaar' ? 'Bazaar' : result.trader.traderName; node.textContent = `Best Sale: ${label} · ${formatLedgerMoney(result.unitPrice)} each · ${formatLedgerMoney(result.totalValue)} total`; }
  ...
  function renderInventoryBazaarSellQuantity(record) {
    ...
    node.dataset.wahState = record.sellDecision.sellableQuantity > 0 ? 'available' : 'unavailable'; node.textContent = `To Bazaar ${record.sellDecision.sellableQuantity}/${record.sellDecision.inventoryQuantity}`; return node;
  }
