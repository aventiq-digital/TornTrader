...
(function () {
  ...
  const state = {
    ...
    historyTransportListenerId: null, inventoryHistoryObserver: null, inventoryHistoryInitialized: false, inventoryBasisRefreshTimer: null, inventoryBazaarResults: new Map(), inventoryBazaarSettingsListenerId: null, inventoryTraderQuoteListenerId: null, inventorySettingsListenerId: null, inventorySortSettings: { criterion: 'original', direction: 'asc' }, inventorySortOriginals: new WeakMap(), inventorySortLastSignatures: new WeakMap(), inventorySortPending: new Map(), inventorySortQuietTimer: 0, inventorySortMaxTimer: 0, inventorySortImmediate: false, inventorySortInternalRows: new WeakSet(), inventoryFullLoad: { container: null, generation: 0, status: 'idle', attempts: 0, startedAt: 0, awaitingProgress: false, snapshot: null, timer: 0 }, inventoryValuationGeneration: 1, inventoryBazaarPending: new Set(),
  ...
  function abortTornInventoryFullLoad(reason) { const load = state.inventoryFullLoad; clearTimeout(load.timer); load.timer = 0; load.awaitingProgress = false; load.status = 'aborted'; logDebug('inventory-full-load-abort', { reason, attempts: load.attempts, generation: load.generation }); scheduleTornInventoryBasisRefresh(); return { status: 'aborted', reason };
  }
  function scheduleTornInventoryFullLoad(delay = INVENTORY_FULL_LOAD_RETRY_MS, progressTimeout = false) { const generation = state.inventoryFullLoad.generation; clearTimeout(state.inventoryFullLoad.timer); state.inventoryFullLoad.timer = setTimeout(() => { state.inventoryFullLoad.timer = 0; if (generation !== state.inventoryFullLoad.generation) return; if (progressTimeout && state.inventoryFullLoad.awaitingProgress) { state.inventoryFullLoad.awaitingProgress = false; logDebug('inventory-full-load-state', { status: 'progress-timeout', attempt: state.inventoryFullLoad.attempts, generation }); } ensureFullTornInventoryLoaded(); }, delay); }
  function ensureFullTornInventoryLoaded(now = Date.now()) {
    if (location.pathname !== '/item.php') return { status: 'ignored' }; const container = getActiveTornInventoryContainer(document); const load = state.inventoryFullLoad; if (!container) return load.container ? abortTornInventoryFullLoad('active-container-missing') : { status: 'waiting-container' };
    if (load.container !== container) { if (load.container) logDebug('inventory-full-load-container-changed', { previousGeneration: load.generation }); clearTimeout(load.timer); state.inventoryFullLoad = { container, generation: load.generation + 1, status: 'loading', attempts: 0, startedAt: now, awaitingProgress: false, snapshot: null, timer: 0 }; logDebug('inventory-full-load-start', { generation: state.inventoryFullLoad.generation }); }
    const current = state.inventoryFullLoad; const snapshot = getTornInventoryLoadState(container); logDebug('inventory-full-load-state', { ...snapshot, status: current.status, attempts: current.attempts, generation: current.generation }); if (!snapshot || !['0', '1'].includes(snapshot.dataAll) || snapshot.dataFrom == null || !snapshot.dataQueue) return abortTornInventoryFullLoad('unsafe-container-state');
    if (snapshot.dataAll === '1') { const changed = current.status !== 'complete'; clearTimeout(current.timer); current.timer = 0; current.awaitingProgress = false; current.status = 'complete'; logDebug('inventory-full-load-complete', { ...snapshot, generation: current.generation }); if (changed) { state.inventoryValuationGeneration += 1; state.inventoryBazaarPending.clear(); scheduleTornInventoryBasisRefresh(); } return { status: 'complete', ...snapshot }; }
    if (current.status === 'aborted') return { status: 'aborted' }; if (now - current.startedAt > INVENTORY_FULL_LOAD_MAX_DURATION_MS) return abortTornInventoryFullLoad('max-duration');
    if (current.awaitingProgress) { const progressed = snapshot.dataAll === '1' || snapshot.dataFrom > current.snapshot.dataFrom || snapshot.rowCount > current.snapshot.rowCount; if (!progressed) return { status: 'awaiting-progress', ...snapshot }; clearTimeout(current.timer); current.timer = 0; current.awaitingProgress = false; logDebug('inventory-full-load-progress', { before: current.snapshot, after: snapshot, generation: current.generation }); scheduleTornInventoryFullLoad(100); return { status: 'progress', ...snapshot }; }
    if (current.attempts >= INVENTORY_FULL_LOAD_MAX_ATTEMPTS) return abortTornInventoryFullLoad('no-progress'); const button = document.querySelector?.('#load-more-items [role="button"]'); const jquery = window.jQuery || window.$; if (!button || typeof jquery !== 'function') { current.attempts += 1; if (current.attempts >= INVENTORY_FULL_LOAD_MAX_ATTEMPTS) return abortTornInventoryFullLoad(!button ? 'load-more-control-missing' : 'jquery-unavailable'); scheduleTornInventoryFullLoad(); return { status: 'waiting-trigger', reason: !button ? 'load-more-control-missing' : 'jquery-unavailable' }; }
    current.attempts += 1; current.snapshot = snapshot; current.awaitingProgress = true; logDebug('inventory-full-load-trigger', { attempt: current.attempts, ...snapshot, generation: current.generation }); jquery(button).trigger('click'); scheduleTornInventoryFullLoad(INVENTORY_FULL_LOAD_PROGRESS_TIMEOUT_MS, true); return { status: 'triggered', attempt: current.attempts, ...snapshot };
  }
  function isTornInventoryFullLoadPending(container) { const load = state.inventoryFullLoad; return load.container === container && load.status === 'loading' && getTornInventoryLoadState(container)?.dataAll === '0'; }
  ...
  function canApplyInventorySortSnapshot(container, criterion = state.inventorySortSettings.criterion) { if (isTornInventoryFullLoadPending(container)) return { allowed: false, reason: 'full-inventory-load' }; if (inventorySortDependsOnBazaar(criterion) && state.inventoryBazaarPending.size) return { allowed: false, reason: 'bazaar-valuation-round' }; return { allowed: true, reason: null }; }
  ...
  function buildInventorySortKey(record, criterion) {
    ...
    if (criterion === 'bazaar-unit') return record.bazaarValuation?.status === 'available' ? inventorySortMoney(record.bazaarValuation.recommendedUnitPrice) : null; if (criterion === 'bazaar-total') return record.bazaarValuation?.status === 'available' ? inventorySortMoney(record.bazaarValuation.recommendedTotal) : null;
  ...
  function refreshTornInventoryBasis(root = document) {
    ...
    for (const container of containers) {
      ...
      for (const block of blocks) {
        ...
        const traderResult = resolveInventoryTraderQuote(block.model.itemId, quoteIndex, state.settings.minimumTraderRating, now, personalRatings); const bazaarValuation = buildInventoryBazaarValuation(priceResult, block.currentQuantity); const traderValuation = buildInventoryTraderValuation(traderResult, block.currentQuantity); const bestSaleResult = resolveInventoryBestSale(priceResult, traderResult, block.currentQuantity); const profitResult = resolveInventoryPotentialProfit({ basisSummary: summary, bestSaleResult, quantity: block.currentQuantity }); renderTornInventoryBazaar(block, priceResult); renderTornInventoryTrader(block, traderResult, now); renderTornInventoryBestSale(block, bestSaleResult, now); renderTornInventoryProfit(block, profitResult); sortRecords.push({ block, summary, bazaarValuation, traderValuation, bestSaleResult, profitResult, originalIndex: getInventoryOriginalIndex(container, block) });
      }
      ensureInventorySortControl(container); for (const itemId of requestedItemIds) state.inventoryBazaarPending.add(`${state.inventoryValuationGeneration}:${itemId}`); const sortReadiness = canApplyInventorySortSnapshot(container); if (sortReadiness.allowed) queueInventoryStableSort(container, sortRecords, scan, { immediate: state.inventorySortImmediate }); else logDebug('inventory-sort-deferred', { reason: sortReadiness.reason, pending: state.inventoryBazaarPending.size, generation: state.inventoryValuationGeneration });
    }
    state.inventorySortImmediate = false;
    for (const itemId of requestedItemIds) { const generation = state.inventoryValuationGeneration; void loadBazaarRecommendedPrice(itemId, { adjustment }).then((result) => { if (location.pathname !== '/item.php') return; const currentAdjustment = normalizeBazaarAddSettings(gmGet(BAZAAR_ADD_UI_SETTINGS_KEY, null)).priceAdjustment; if (result.adjustment !== currentAdjustment) return; state.inventoryBazaarResults.set(itemId, result); }).finally(() => { state.inventoryBazaarPending.delete(`${generation}:${itemId}`); scheduleTornInventoryBasisRefresh(); }); }
