...
(function () {
  ...
  function renderBazaarManage() { if (!isTornBazaarManageRoute()) return teardownBazaarManage(); const panel = findBazaarManagePanel(); if (!panel) { logDebug('Manage panel waiting'); return; } ensureSellingDecisionStyles(); const controls = ensureBazaarManageControls(panel); const context = buildSellingDecisionContext(); const models = collectBazaarManageRows(panel); for (const model of models) { let host = model.element.querySelector('[data-wah-bazaar-manage-row]'); if (!host) { host = document.createElement('div'); host.className = 'wah-selling-decision wah-bazaar-manage-row'; host.dataset.wahBazaarManageRow = String(model.itemId); model.element.appendChild(host); } if (host.dataset.wahBazaarManageRow !== String(model.itemId)) { host.dataset.wahBazaarManageRow = String(model.itemId); host.textContent = ''; } const snapshot = resolveSellingDecisionSnapshot(model.itemId, model.quantity, context); const recommendation = snapshot.bazaarValuation.status === 'available' ? snapshot.bazaarValuation.recommendedUnitPrice : snapshot.storedBazaar.status === 'sort-available' ? snapshot.storedBazaar.recommendedUnitPrice : null; model.pricingStatus = resolveBazaarManagePricingStatus(model.currentPrice, recommendation); const recommendationLabel = recommendation ? `<span>Recommended ${formatLedgerMoney(recommendation)} · ${snapshot.bazaarValuation.status === 'available' ? 'current' : 'stored'}</span>` : ''; const pricingLabel = model.pricingStatus.text ? `<span class="wah-manage-pricing wah-manage-pricing--${model.pricingStatus.status}">${model.pricingStatus.text}</span>` : ''; host.innerHTML = formatSellingDecisionHtml(snapshot, model.quantity, model.itemId, model.itemName, model.currentPrice) + recommendationLabel + pricingLabel + `<button type="button" data-wah-manage-fill="${model.itemId}">Fill</button>`; } const summary = summarizeBazaarManagePricing(models); const summaryNode = controls?.querySelector?.('[data-wah-manage-summary]'); if (summaryNode) summaryNode.textContent = `${summary.total} visible listings · ${summary.atMarket} at market · ${summary.above} above · ${summary.below} below`; tornState.bazaarManage.models = models; return { controls, models, summary }; }
