...
(function () {
  ...
  const state = {
    ...
    sharedHistoryUrl: '',
    nativeNavigationInitialized: false, manualTraderCaptureTimer: 0, routeDomReady: true, routeSnapshotFailed: false, staleRouteTables: new Set(),
  ...
  function snapshotWeav3rItemSourceTables(root = document) { return new Set(Array.from(root?.querySelectorAll?.('table') || []).filter(isWeav3rItemSourceTable)); }
  function beginWeav3rItemRouteTransition() {
    state.staleRouteTables = new Set(); state.routeDomReady = false; state.routeSnapshotFailed = false;
    try { state.staleRouteTables = snapshotWeav3rItemSourceTables(document); state.routeDomReady = state.staleRouteTables.size === 0; logDebug('route-transition-source-snapshot', { tableCount: state.staleRouteTables.size }); if (state.routeDomReady) logDebug('route-transition-ready'); else logDebug('route-transition-wait', { reason: 'stale-source-tables-connected' }); }
    catch (error) { state.routeSnapshotFailed = true; logDebug('route-transition-wait', { reason: 'source-snapshot-failed', error: String(error?.message || error) }); }
  ...
  function updateWeav3rItemRouteDomReadiness() {
    ...
    for (const table of Array.from(state.staleRouteTables)) if (!table.isConnected) state.staleRouteTables.delete(table);
    if (!state.staleRouteTables.size) { state.routeDomReady = true; logDebug('route-transition-ready'); }
    return state.routeDomReady;
  }
  ...
  function renderTornInventoryWeav3rLinks(block) {
    const row = block?.model?.sourceRow; const itemId = normalizePositiveInt(block?.model?.itemId); const host = row?.querySelector?.('.title-wrap') || row?.querySelector?.('.name-wrap'); if (!row || !itemId || !host) return null;
  ...
  function renderInventoryBazaarSellQuantity(record) {
    const row = record?.block?.model?.sourceRow; const host = row?.querySelector?.('.title-wrap') || row?.querySelector?.('.name-wrap'); if (!host) return null; let node = row.querySelector?.(':scope [data-wah-bazaar-sell-quantity]');
