...
(function () {
  ...
  function processAddStep(handoff, route) {
    waitForTornElement(isSupportedAddInventoryReady, async (inventoryReady) => {
      ...
      const discoveryRoot = getInventoryContainer(); const discovery = await findTradeTargetWithProgressiveScroll({ inventory: discoveryRoot, targetItemId: latest.itemId, signal: controller.signal, isContextCurrent: () => tornState.inventoryDiscoveryController === controller && discoveryRoot?.isConnected !== false && parseTradeHash().step === 'add' && parseTradeHash().tradeId === latestRoute.tradeId && readTradeHandoff()?.handoffId === latest.handoffId, onProgress: (progress) => { if (tornState.inventoryDiscoveryController === controller) renderTradeFilterPanel(latest, progress.targets.length, getTradeFilterMode(latest), false, true); } });
      ...
      if (discovery.status !== 'complete') { getInventoryRows().forEach((row) => row.classList.remove(TRADE_FILTER_HIDDEN_CLASS, TRADE_FILTER_TARGET_CLASS)); renderTradeFilterPanel(latest, discovery.targets.length, 'all', false, false, true); showTornStatus('Weav3r: Inventory loading incomplete; target presence was not classified.'); return; }
