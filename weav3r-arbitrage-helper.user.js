...
(function () {
  ...
  function observeTraderQuote(raw, now = Date.now()) {
    const candidate = normalizeTraderQuoteRecord({ ...raw, version: 1, observedAt: Number(raw?.observedAt) || now, lastSeenAt: now, observationCount: 1, source: 'weav3r' }); if (!candidate) { logDebug('Trader quote rejected.', { traderId: raw?.traderId, itemId: raw?.itemId }); return null; } const key = traderQuoteRecordKey(candidate); const collection = readTraderQuotes(); const existing = collection.records[key]; const record = existing ? normalizeTraderQuoteRecord({ ...existing, traderName: candidate.traderName, lastSeenAt: Math.max(existing.lastSeenAt, now), observationCount: existing.observationCount + 1, traderRatingAtObservation: candidate.traderRatingAtObservation, traderActivityTextAtObservation: candidate.traderActivityTextAtObservation, traderActivityLastSeenAt: candidate.traderActivityLastSeenAt }) : candidate; const result = mergeCriticalRecord(TRADER_QUOTES_KEY, { version: 1, records: {} }, key, record, normalizeTraderQuoteCollection); if (!result.ok) { logDebug('Trader quote rejected by durable store.', { key }); return null; } logDebug(existing ? 'Trader quote deduplicated.' : 'Trader quote persisted.', { key, observationCount: result.collection.records[key]?.observationCount }); return result.collection.records[key];
  }
  ...
  function captureVisibleWeav3rTraderQuotes(now = Date.now()) {
    const itemId = resolveWeav3rItemRouteId(location.href); if (!itemId) return { status: 'ignored', itemId: null, parsed: 0, saved: 0 }; logDebug('Weav3r Trader itemId resolved.', itemId); const traders = parseTraderOffers(document, location.href); if (!traders.length) return { status: 'waiting', itemId, parsed: 0, saved: 0 }; logDebug('Weav3r Trader table detected.', { itemId }); logDebug('Weav3r Trader rows parsed.', { itemId, count: traders.length }); const saved = persistObservedTraderQuotes(itemId, traders, location.href, now); logDebug('Weav3r Trader quotes observed.', { itemId, parsed: traders.length, saved }); return { status: saved ? 'persisted' : 'rejected', itemId, parsed: traders.length, saved };
  }
  ...
  function resolveInventoryTraderQuote(itemId, quoteIndex, minimumTraderRating, now = Date.now(), personalRatings = null) {
    ...
    const classified = quotes.map((quote) => ({ quote, eligibility: getStoredTraderQuoteEligibility(quote, minimumTraderRating, now, personalRatings) })); const eligible = classified.filter((entry) => entry.eligibility === 'eligible').sort(compareCurrentTraderQuotes); if (eligible.length) { const result = { status: 'eligible', ...eligible[0], reason: null }; logDebug('item.php Trader quote resolved.', { itemId, status: result.status, traderId: result.quote.traderId }); return result; }
    const unknown = classified.filter((entry) => entry.eligibility === 'unknown').sort((left, right) => right.quote.lastSeenAt - left.quote.lastSeenAt || right.quote.observedAt - left.quote.observedAt || left.quote.traderId - right.quote.traderId); const result = unknown.length ? { status: 'unverified', ...unknown[0], reason: 'current-status-unverified' } : { status: 'unavailable', quote: null, reason: 'no-eligible-quotes' }; logDebug('item.php Trader quote resolved.', { itemId, status: result.status, traderId: result.quote?.traderId || null }); return result;
  }
  ...
  function refreshTornInventoryBasis(root = document) {
    ...
    for (const container of containers) {
      ...
      for (const block of blocks) {
        ...
        const traderResult = resolveInventoryTraderQuote(block.model.itemId, quoteIndex, state.settings.minimumTraderRating, now, personalRatings); const bestSaleResult = resolveInventoryBestSale(priceResult, traderResult, block.currentQuantity); renderTornInventoryBazaar(block, priceResult); renderTornInventoryTrader(block, traderResult, now); renderTornInventoryBestSale(block, bestSaleResult, now); renderTornInventoryProfit(block, resolveInventoryPotentialProfit({ basisSummary: summary, bestSaleResult, quantity: block.currentQuantity }));
      }
    }
