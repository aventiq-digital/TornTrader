...
(function () {
  ...
  function reconcileBazaarAddLifecycle() { if (isTornBazaarAddRoute()) initBazaarAddManager(); else teardownBazaarAddManager(); if (isTornBazaarManageRoute()) initBazaarManage(); else teardownBazaarManage(); }
  ...
  function isTornBazaarManageRoute(pathname = location.pathname, hash = location.hash) { return pathname === '/bazaar.php' && /^#\/manage(?:[/?&]|$)/.test(String(hash || '')); }
  ...
  function renderTradeFilterPanel(handoff, count, mode, notFound = false, searching = false, incomplete = false) {
    ...
    text.textContent = searching ? `Loading full inventory… ${handoff.itemName}` : incomplete ? `Inventory loading incomplete — ${handoff.itemName} could not be verified.` : notFound ? `${handoff.itemName} was not found in the fully loaded trade inventory.` : `${formatTargetFilterLabel(handoff.itemName, count)}${available ? ` · Available: ${available}` : ''}`;
  ...
  async function findTradeTargetWithProgressiveScroll({ inventory, targetItemId, signal, isContextCurrent = () => true }) {
    ...
    try {
      for (let step = 0; step < TRADE_TARGET_DISCOVERY_MAX_STEPS && Date.now() < deadline && !signal?.aborted; step += 1) {
        ...
        if (unchangedAtEnd >= 3) return { status: 'complete', targets: rows.filter((row) => rowHasTradeItemId(row, targetItemId)), rowCount: rows.length };
      ...
      return { status: signal?.aborted ? 'aborted' : 'incomplete', targets, reason: signal?.aborted ? 'aborted' : 'limit-reached' };
    } finally {
      if (isContextCurrent()) {
        ...
        logDebug('inventory scroll restored after full inventory discovery.');
      }
    }
  }
  ...
  function processAddStep(handoff, route) {
    waitForTornElement(isSupportedAddInventoryReady, async (inventoryReady) => {
      ...
      const discovery = await findTradeTargetWithProgressiveScroll({ inventory: getInventoryContainer(), targetItemId: latest.itemId, signal: controller.signal, isContextCurrent: () => tornState.inventoryDiscoveryController === controller && parseTradeHash().step === 'add' && parseTradeHash().tradeId === latestRoute.tradeId && readTradeHandoff()?.handoffId === latest.handoffId });
