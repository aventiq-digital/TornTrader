...
(function () {
  ...
  function updateWeav3rItemRouteDomReadiness(mutations = []) {
    ...
    for (const table of Array.from(state.staleRouteTables)) {
      ...
      if (mutations.some((mutation) => mutation.target === table || table.contains?.(mutation.target) || Array.from(mutation.removedNodes || []).includes(table))) state.staleRouteTables.delete(table);
    }
  ...
  function buildWeav3rItemViewUrl(itemId, view, origin = location.origin) {
