...
(function () {
  ...
  function ensureBazaarManageControls(panel) { let controls = document.getElementById(BAZAAR_MANAGE_TOOLS_ID); if (controls) return controls; controls = document.createElement('section'); controls.id = BAZAAR_MANAGE_TOOLS_ID; controls.className = 'wah-bazaar-manage-controls'; controls.innerHTML = `<strong>Weav3r Bazaar Manager</strong><button type="button" data-wah-manage-fill-prices>Fill prices</button><button type="button" data-wah-manage-remove-all>Remove all</button><span data-wah-manage-status aria-live="polite"></span>`; const anchor = panel.querySelector?.('[aria-label="Confirmation section"]') || panel.querySelector?.('[data-testid="panel-header"]'); anchor?.appendChild?.(controls); return controls; }
  function renderBazaarManage() { if (!isTornBazaarManageRoute()) return teardownBazaarManage(); const panel = findBazaarManagePanel(); if (!panel) { logDebug('Manage panel waiting'); return; } ensureSellingDecisionStyles(); const controls = ensureBazaarManageControls(panel); const context = buildSellingDecisionContext(); const models = collectBazaarManageRows(panel); for (const model of models) { let host = model.element.querySelector('[data-wah-bazaar-manage-row]'); if (!host) { host = document.createElement('div'); host.className = 'wah-selling-decision wah-bazaar-manage-row'; host.dataset.wahBazaarManageRow = String(model.itemId); model.element.appendChild(host); } if (host.dataset.wahBazaarManageRow !== String(model.itemId)) { host.dataset.wahBazaarManageRow = String(model.itemId); host.textContent = ''; } host.innerHTML = formatSellingDecisionHtml(resolveSellingDecisionSnapshot(model.itemId, model.quantity, context), model.quantity, model.itemId, model.itemName, model.currentPrice) + `<button type="button" data-wah-manage-fill="${model.itemId}">Fill</button>`; } tornState.bazaarManage.models = models; return { controls, models }; }
  ...
  function renderTornInventoryBestSale(block, result, now = Date.now(), marketDelta = null) {
    ...
    node.dataset.wahState = result.status === 'available' ? 'available' : result.channel === 'stored-trader' ? 'stored' : 'unavailable'; node.dataset.wahChannel = result.channel || 'unavailable'; node.dataset.wahMarketDelta = marketDelta ? 'verified' : 'none';
    if (result.channel === 'stored-trader') node.textContent = `Stored candidate · ${formatLedgerMoney(result.unitPrice)}${result.quantity === 1 ? '' : ' each'} · ${result.trader.traderName}`;
    else if (result.status !== 'available') node.textContent = 'Best Sale —';
    else { const label = result.channel === 'bazaar' ? 'Bazaar' : 'Trader'; const value = result.quantity === 1 ? formatLedgerMoney(result.unitPrice) : `${formatLedgerMoney(result.unitPrice)} each · ${formatLedgerMoney(result.totalValue)} total`; const delta = marketDelta ? ` · +${formatLedgerMoney(marketDelta.absoluteDelta)} (${formatInventoryAdvantagePercent(marketDelta.percentage)}) vs ${marketDelta.alternative === 'bazaar' ? 'Bazaar' : 'Trader'}` : ''; node.textContent = `Best Sale: ${label} · ${value}${delta}`; }
    node.title = result.channel === 'stored-trader' ? `Stored Trader quote (${formatStoredQuoteAge(result.freshness, now)}) with unverified current eligibility. No claim that the Trader still accepts it. Possible gross value, not guaranteed proceeds.` : result.status === 'available' ? `Currently verified sale channel (${result.channel === 'bazaar' ? 'Bazaar recommendation' : 'eligible Trader'}). Possible gross value, not guaranteed proceeds.` : 'No currently verified sale channel. Gross value and proceeds are not estimated.';
    return node;
  }
  ...
  async function refreshOwnedInventoryBazaar() { if (state.inventoryBazaarManualRefresh) return state.inventoryBazaarManualRefresh.promise; const container = getActiveTornInventoryContainer(document); const latest = state.inventorySortLatest.get(container); const itemIds = [...new Set((latest?.records || []).filter((record) => record.block?.model?.ownership !== 'faction-loan').map((record) => normalizePositiveInt(record.block?.model?.itemId)).filter(Boolean))]; if (!itemIds.length) return { updated: 0, kept: 0 }; const operation = { total: itemIds.length, completed: 0, updated: 0, kept: 0, promise: null }; state.inventoryBazaarManualRefresh = operation; renderInventoryBazaarRefreshState(); const adjustment = normalizeBazaarAddSettings(gmGet(BAZAAR_ADD_UI_SETTINGS_KEY, null)).priceAdjustment; operation.promise = Promise.all(itemIds.map(async (itemId) => { try { const entry = await queueBazaarAddMarketplaceDetail(itemId); const result = resolveBazaarRecommendedPrice(itemId, { adjustment, offers: entry.offers, source: entry.apiSource, fetchedAt: entry.capturedAt }); if (result.status === 'available') { state.inventoryBazaarResults.set(itemId, result); operation.updated += 1; } else operation.kept += 1; } catch (_) { operation.kept += 1; } finally { operation.completed += 1; renderInventoryBazaarRefreshState(); } })).then(() => { if (container) setInventorySortDirty(container, operation.updated > 0 || state.inventorySortDirty.has(container)); scheduleTornInventoryBasisRefresh(); return { updated: operation.updated, kept: operation.kept }; }).finally(() => { const control = document.getElementById(INVENTORY_SORT_CONTROL_ID); state.inventoryBazaarManualRefresh = null; renderInventoryBazaarRefreshState(control); const status = control?.querySelector?.('[data-wah-inventory-refresh-status]'); if (status) { status.hidden = false; status.textContent = operation.kept ? `Bazaar refreshed · ${operation.updated} updated · ${operation.kept} kept from stored values` : 'Bazaar refreshed'; } }); return operation.promise; }
